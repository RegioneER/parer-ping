 ...
 * ({@code BackendHelper}, che è {@code @Stateless} e ha un EM thread-safe); il risultato viene
 * depositato qui con {@link ConcurrentHashMap#putIfAbsent}. In caso di race condition alla prima
 * chiamata su una chiave, al massimo due thread caricano lo stesso dato dal DB; tutte le chiamate
 * successive sono completamente lock-free.
 * </p>
 */
@Singleton
...
public class ObjectStorageConfigCache {
    ...
    private final Map<String, BackendStorage> backendByNameCache = new ConcurrentHashMap<>();
    private final Map<Long, BackendStorage> backendByIdCache = new ConcurrentHashMap<>();
    private final Map<String, ObjectStorageBackend> osConfigCache = new ConcurrentHashMap<>();
    // cache dell'id PK di DecBackend: evita query ripetute nei saveObjectStorageLink*
    private final Map<String, Long> backendIdCache = new ConcurrentHashMap<>();

    // --- BackendStorage (by name) ---

    public BackendStorage getBackend(String nomeBackend) {
        return backendByNameCache.get(nomeBackend);
    }

    /*
     * Deposita il valore in cache solo se la chiave non è già presente. Restituisce il valore
     * effettivamente in cache dopo l'operazione.
     */
    public BackendStorage putBackendIfAbsent(String nomeBackend, BackendStorage value) {
        BackendStorage existing = backendByNameCache.putIfAbsent(nomeBackend, value);
        return existing != null ? existing : value;
    }

    // --- BackendStorage (by id) ---

    public BackendStorage getBackendById(Long id) {
        return backendByIdCache.get(id);
    }

    public BackendStorage putBackendByIdIfAbsent(Long id, BackendStorage value) {
        BackendStorage existing = backendByIdCache.putIfAbsent(id, value);
        return existing != null ? existing : value;
    }
