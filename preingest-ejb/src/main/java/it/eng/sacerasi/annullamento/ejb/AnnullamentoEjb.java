...
public class AnnullamentoEjb {
    ...
    public PigXmlAnnulSessioneIngestRowBean getPigXmlSessioneIngestRowBean(
	    BigDecimal idSessioneIngest, String tiXmlAnnul) throws ParerUserError {
	PigXmlAnnulSessioneIngestRowBean rowBean = null;
	List<PigXmlAnnulSessioneIngest> xmlAnnuls = helper
		.retrievePigXmlAnnulSessioneIngests(idSessioneIngest, tiXmlAnnul);
	if (xmlAnnuls != null && !xmlAnnuls.isEmpty()) {
		...
		try {
		    ...
		    String msg = "Errore inatteso nel recupero dello xml di "
			    + tiXmlAnnul.toLowerCase();
		    logger.error(msg, ex);
		    throw new ParerUserError(msg);
		}
	    } else {
		throw new ParerUserError(
			"Errore inatteso nel recupero dello xml di " + tiXmlAnnul.toLowerCase());
	    }
	}
	return rowBean;
    }
	...
	if (input != null) {
	    ...
	    context.getBusinessObject(AnnullamentoEjb.class)
		    .gestisciRispostaAnnullamento(esitoConnAnnul, idObject);
	...
	PigSessioneIngest sessioneCorrente = helper.findById(PigSessioneIngest.class,
		idSessioneCorrente);
	Date now = new Date();
	PigSessioneIngest nuovaSessione = monitoraggioHelper.creaSessione(sessioneCorrente, object,
		now, Constants.StatoSessioneIngest.DA_TRASFORMARE.name());
	monitoraggioHelper.creaStatoSessione(nuovaSessione,
	...
	if (tiVers.equals(Constants.TipoVersamento.DA_TRASFORMARE.name())) {
		...
		// Aggiorna l'oggetto con stato ANNULLATO e registra un nuovo stato corrente della
		// sessione
	    ...
	    boolean isLastVerificata = monitoraggioHelper.isLastVerificata(idVers,
		...
		if (helper.countPigUnitaDocObject(object.getIdObject(),
		    ...
		    input = generaRichiestaAnnullamentoVersamenti(object, motivazioneAnnullamento,
			    username);
		    logger.debug(
			    "STATO CHIUSO_ERR_VERS, CHIUSO_OK - Salvo il record XML di richiesta annullamento");
		    ...
		    xmlAnnul.setCdVersioneXmlAnnul(configurationHelper
			    .getValoreParamApplicByApplic(Constants.VERSIONE_XML_ANNUL));
	...
	if (esitoConnAnnul.isErroreConnessione()) {
		...
		if (xmlAnnul.getCdVersioneXmlAnnul().equals("1.0")
		    ...
		    // vecchio comportamento (per poter funzionare anche con una versione di SACER
		    // vecchia)
		    helper.updateUnitaDocSessione(object.getIdLastSessioneIngest(),
			    Constants.StatoUnitaDocSessione.IN_CORSO_ANNULLAMENTO.name(),
			    Constants.StatoUnitaDocSessione.ANNULLATA.name());
			...
			String strXml = null;
			List<PigXmlAnnulSessioneIngest> listaAnnul = pigSessioneIngest
				.getPigXmlAnnulSessioneIngests();
		...
		if (tiVers.equals(Constants.TipoVersamento.NO_ZIP.name())
		    ...
		    // L'oggetto da annullare si riferisce a un oggetto padre e tutti i suoi figli
		    // hanno stato ANNULLATO
     ...
     * tiVers = tiVersFile di Tipo Object
     *
     * Annulla un eventuale Sisma legato all'oggetto passato #####DA USARE PER ANULLARE SISMA COME
     * per STRUMENTI URBANISTICI, cercare chiamate a
     * eseguiAnnullamentoEventualeStrumentoUrbanistico!
     */
    // MEV 26398
	...
	// il problema si pone se il sisma è in stato completato e noi annulliamo i pacchetti
    ...
    private RichiestaSacerInput generaRichiestaAnnullamentoVersamenti(PigObject object,
	...
	richAnnulVers.getRichiesta().setCodice("Annullamento oggetto " + cdKeyObject
		+ " e sessione " + idSessioneIngest.toPlainString());
	...
	if (motivazioneAnnullamento != null && !motivazioneAnnullamento.isEmpty()) {
	    richAnnulVers.getRichiesta().setMotivazione("Annullamento in PreIngest dell'oggetto "
		    + cdKeyObject + " - " + motivazioneAnnullamento);
	} else {
	    richAnnulVers.getRichiesta()
		    .setMotivazione("Annullamento in PreIngest dell'oggetto " + cdKeyObject);
	}
	...
	BigDecimal idAmbienteVers = BigDecimal.valueOf(
		object.getPigTipoObject().getPigVer().getPigAmbienteVer().getIdAmbienteVers());
	...
	try {
	    Marshaller recMarshaller = xmlContextCache
		    .getRichAnnVersCtx_RichiestaAnnullamentoVersamenti().createMarshaller();
	    ...
	    throw new ParerUserError(
		    "Errore inatteso nella creazione della richiesta di annullamento");
	}
	...
	input = new RichiestaSacerInput(RichiestaSacerInput.TipoRichiestaSacer.ANNULLAMENTO,
		configurationHelper.getValoreParamApplicByApplic(Constants.VERSIONE_XML_ANNUL),
		configurationHelper.getValoreParamApplicByApplic(Constants.URL_ANNUL_VERS),
		tmpWriter.toString(), null, nmUseridSacer, cdPswSacer, timeout);
	return input;
    }
	...
	RichiestaSacerInput input = context.getBusinessObject(AnnullamentoEjb.class)
		.initVerificaAnnullamento(idObject, username);
	if (input != null) {
	    // E' stato generato un xml di invio richiesta di annullamento a Sacer, eseguo
	    // l'attivazione del servizio
	    // Chiamata a richiesta sacer RecuperoUnitaDocumentariaSync
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public RichiestaSacerInput initVerificaAnnullamento(BigDecimal idObject, String username)
	...
	if (!xmlResps.isEmpty()) {
	    ...
	    throw new ParerUserError(
		    "Errore inatteso nella verifica: xml di richiesta non registrato");
	}
	...
	try {
	    ...
	    XPathExpression expr = xPath
		    .compile("//RichiestaAnnullamentoVersamenti/Richiesta/Motivazione");
		...
		if (motivazioneCustomIndex != -1) {
		    motivazioneAnnullamento = oldMotivazione.substring(motivazioneCustomIndex + 1)
			    .trim();
		}
	    }
	} catch (Exception ex) {
	...
	PigObject object = helper.findByIdWithLock(PigObject.class, idObject);
	PigSessioneIngest lastSessione = helper.findById(PigSessioneIngest.class,
		object.getIdLastSessioneIngest());
	...
	logger.debug(
		"Accetta annullamento fallito - Aggiorno la sessione con lo stato precedente {}",
		lastStato.getTiStato());
	lastSessione
		.setIdStatoSessioneIngestCor(new BigDecimal(lastStato.getIdStatoSessioneIngest()));
	lastSessione.setTiStato(lastStato.getTiStato());
	logger.debug(
		"Accetta annullamento fallito - Aggiorno l'oggetto con lo stato della sessione");
	...
	if (lastSessione.getPigXmlAnnulSessioneIngests() != null) {
	    for (PigXmlAnnulSessioneIngest pigXmlAnnulSessioneIngest : lastSessione
		    .getPigXmlAnnulSessioneIngests()) {
