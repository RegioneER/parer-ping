...
public class AllineamentoEntiConvenzionatiEjb {
     ...
     *
     * @param enteSiamDaAllineaList lista di elementi {@link IamEnteSiamDaAllinea}
     */
	...
	for (IamEnteSiamDaAllinea enteSiamDaAllinea : enteSiamDaAllineaList) {
		...
		if (client != null) {
		    ...
		    amministrazioneHelper.writeEsitoIamEnteSiamDaAllinea(
			    enteSiamDaAllinea.getIdEnteSiamDaAllinea(), esitoServizio,
		    ...
		    String posNeg = esitoServizio.name()
			    .equals(CostantiAllineaEntiConv.EsitoServizio.OK.name()) ? "positiva"
				    : "negativa";
		    log.info(
			    "Allineamento Ente Convenzionato - Risposta WS {0} per l'ente siam {1}",
			    posNeg, idEnteSiam);
		    ...
		    amministrazioneHelper.writeEsitoIamEnteSiamDaAllinea(
			    enteSiamDaAllinea.getIdEnteSiamDaAllinea(),
			    CostantiAllineaEntiConv.EsitoServizio.KO,
		    ...
		    log.error(
			    "Allineamento Ente Convenzionato - Risposta WS negativa per l'ente siam {0}",
			    idEnteSiam);
		    break;
		}
		...
		log.error("Allineamento Ente Convenzionato - Risposta WS negativa per l'ente siam "
			+ idEnteSiam
	...
	if (!arrivoDaOnLine) {
		...
		jobLoggerEjb.writeAtomicLog(Constants.NomiJob.ALLINEA_ENTI_CONVENZIONATI,
			...
			"Errore durante la chiamata al WS di allineamento ente siam");
	    }
	}
    }
}
