...
public class AllineamentoOrganizzazioniEjb {
    ...
    /**
     * Metodo chiamato per la duplicazione su IAM delle organizzazioni inserite, modificate o cancellate su SACER_PING.
     * La lista delle organizzazioni è recuperata dalla tabella IAM_ORGANIZ_DA_REPLIC
     *
    ...
    public String allineamentoOrganizzazioni(List<IamOrganizDaReplic> organizList)
        ...
        for (IamOrganizDaReplic organizDaReplic : organizList) {
            try {
                ...
                if (client != null) {
                    ...
                    EsitoServizio esitoServizio = resp.getCdEsito().name().equals(Constants.EsitoServizio.OK.name())
                            ? Constants.EsitoServizio.OK : Constants.EsitoServizio.KO;
                ...
                log.error("Replica Organizzazioni SACER PREINGEST - Risposta WS negativa per l'organizzazione "
                        + organizDaReplic.getNmOrganiz() + " " + Constants.REPLICA_ORG_001, e);
                replicaOK = false;
                break;
            }
        } // End organizDaReplic
        ...
        if (!arrivoDaOnLine) {
            if (replicaOK) {
                ...
                jobLoggerEjb.writeAtomicLog(Constants.NomiJob.ALLINEAMENTO_ORGANIZZAZIONI,
                        Constants.TipiRegLogJob.ERRORE, "Errore durante la chiamata al WS di replica organizzazione");
            }
        }
    ...
    private ParametriInputOrganizzazioni getParametriInputOrganizzazione(IamOrganizDaReplic organizDaReplic) {
        ...
        parametriInputOrganizzazioni
                .setNmUserid(configurationHelper.getValoreParamApplicByApplic(Constants.USERID_REPLICA_ORG));
        parametriInputOrganizzazioni
                .setCdPsw(configurationHelper.getValoreParamApplicByApplic(Constants.PSW_REPLICA_ORG));
        parametriInputOrganizzazioni.setNmApplic(configurationHelper.getValoreParamApplicByApplic(Constants.NM_APPLIC));
        ...
        if (organizDaReplic.getTiOperReplic().equals(Constants.TiOperReplic.INS.name())
            ...
            /*
             * Devo recuperare l'organizzazione da passare al WS, se non c'è (esempio è stata cancellata da
             * SACER_PREINGEST) non posso passare nulla
             */
            switch (organizDaReplic.getNmTipoOrganiz()) {
            case "AMBIENTE":
                PigAmbienteVers ambiente = aoHelper.getPigAmbienteVers(organizDaReplic.getIdOrganizApplic());
                if (ambiente != null) {
            ...
            case "VERSATORE":
                ...
                if (vers != null) {
                    parametriInputOrganizzazioni
                            .setIdOrganizApplicPadre(vers.getPigAmbienteVer().getIdAmbienteVers().intValue());
            ...
            Integer idEnteFornitEstern = mappa.get("idEnteFornitEstern") != null
                    ? ((BigDecimal) mappa.get("idEnteFornitEstern")).intValue() : null;
