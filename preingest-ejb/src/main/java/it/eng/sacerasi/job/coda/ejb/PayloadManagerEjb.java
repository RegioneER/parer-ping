...
public class PayloadManagerEjb {
    ...
    public void manageMessagePayload(Message message, String consumerName)
            throws JMSException, ParerInternalError {
        ...
        if (message instanceof TextMessage) {
            ...
            try {
                ...
                PigUnitaDocObject unitaDoc = codaHelper
                        .findLockPigUnitaDocObjectById(payload.getUnitaDocId());
                ...
                log.debug(
                        ...
                        uuid, unitaDoc.getTiStatoUnitaDocObject(),
                        unitaDocSessione.getTiStatoUnitaDocSessione(), inCodaVersamento);
                if (inCodaVersamento) {
                    final String infoLog = "[" + uuid + "] " + consumer + " - idObj = "
                            + object.getIdObject() + ", idSes = " + object.getIdLastSessioneIngest()
                            + ", ud = " + unitaDoc.getIdUnitaDocObject() + " :: ";
                    log.debug("{} ricevuta UD '{}' dell'oggetto '{}'", infoLog,
                            payload.getUnitaDocId(), payload.getObjectId());
                    // per evitare problemi di memoria il produttore mette in coda solo gli id degli
                    // xml
                    ...
                    String xmlVersamentoSacer = codaHelper
                            .findXmlSacerUnitaDocById(xmlVersamentoSacerId);
                    ...
                    Integer timeout = Integer.valueOf(configurationHelper
                            .getValoreParamApplicByApplic(Constants.TIMEOUT_VERS_SACER));
                    ...
                    RichiestaSacerInput input = new RichiestaSacerInput(
                            RichiestaSacerInput.TipoRichiestaSacer.VERSAMENTO,
                            unitaDoc.getCdVerWsSacer(), urlVersamento, xmlVersamentoSacer,
                            xmlIndice, userIdSacer, passwordSacer, timeout);
                    // chiamo il Servizio Multimedia di Sacer
                    ...
                    if (esitoConnessione.isErroreConnessione()) {
                        log.debug("{} UD '{}' andata in timeout", infoLog,
                                unitaDoc.getIdUnitaDocObject());
                        unitaDoc.setTiStatoUnitaDocObject(
                                Constants.StatoUnitaDocObject.VERSATA_TIMEOUT.name());
                        // MEV 27407
                        Date dtStato = new Date();
                        unitaDoc.setDtStato(dtStato);
                        unitaDocSessione.setTiStatoUnitaDocSessione(
                                Constants.StatoUnitaDocSessione.VERSATA_TIMEOUT.name());
                    ...
                    log.debug(
                            ...
                            infoLog, payload.getUnitaDocId(), codiceEsitoVersamento,
                    ...
                    if (codiceEsitoVersamento != null) {
                        if (codiceEsitoVersamento.equals(Constants.EsitoVersamento.POSITIVO.name())
                                || codiceEsitoVersamento
                                        .equals(Constants.EsitoVersamento.WARNING.name())) {
                            // se POSITIVO o WARNING modifico lo stato della unità documentaria in
                            // VERSATA_OK
                            this.handlePositivoWarning(codiceEsitoVersamento, unitaDoc,
                                    unitaDocSessione, infoLog);
                            incrementaVersateOk = true;
                        } else {
                            // se NEGATIVO modifico lo stato della unità documentaria in VERSATA_ERR
                            this.handleNegativo(codiceEsitoVersamento, unitaDoc, unitaDocSessione,
                                    infoLog, codiceErroreVersamento, messaggioErroreVersamento);
                            incrementaVersateErrore = true;
                        }
                    }
                    // se il numero delle unità documentarie versate coincide con il numero di unità
                    // documentarie da
                    // versare
                    log.debug(
                            "{} Aggiorno i contatori incrementaVersateOk={},incrementaVersateErrore={},incrementaVersateTimeout={}",
                            infoLog, incrementaVersateOk, incrementaVersateErrore,
                    ...
                    log.debug("{} Versate {} UD; da versare = {}", infoLog,
                            session.getNiUnitaDocVers(), session.getNiUnitaDocDaVers());
                    if (session.getNiUnitaDocVers().intValue() == session.getNiUnitaDocDaVers()
                            .intValue()) {
                        ...
                        long duplicateUd = codaHelper.countUdInObj(object,
                                ...
                                + codaHelper.countUdInObj(object,
                                        Constants.StatoUnitaDocObject.VERSATA_ERR.name(),
                        ...
                        if (session.getNiUnitaDocVersOk().intValue() == session
                            ...
                            log.debug("{} oggetto '{}' => padre '{}'", infoLog,
                                    object.getIdObject(), object.getPigObjectPadre());
                            ...
                            if (object.getPigObjectPadre() != null) {
                                ...
                                definisciStatoOggettoPadre(
                                        object.getPigObjectPadre().getIdObject());
                            }
                        } else {
                            // assegno zero al flag di sessione verificata
                            session.setFlSesErrVerif(Constants.DB_FALSE);
                            log.debug("{} imposto flSessioneErrVerificata '{}'", infoLog,
                                    session.getFlSesErrVerif());
                            // PUNTO c
                            if (timeoutUd > 0 || err666Ud > 0 || err666PUd > 0) {
                                // Esistono UD chiuse con TIMEOUT Imposto oggetto e sessione con
                                // stato
                                // CHIUSO_ERR_RECUPERABILE
                                // MEV 30209 Esistono UD chiuse in VERSATA_ERR con codice d'errore
                                // ERR_666 -> Imposto
                                // oggetto e sessione con stato CHIUSO_ERR_RECUPERABILE
                                log.debug("{} Nello zip sono presenti {} UD versate con TIMEOUT",
                                        infoLog, timeoutUd);
                                log.debug("{} Nello zip sono presenti {} UD versate con ERR_666",
                                        infoLog, err666Ud);
                                log.debug("{} Nello zip sono presenti {} UD versate con ERR_666P",
                                        infoLog, err666PUd);
                                // la sessione viene chiusa negativamente ed assume lo stato
                                // CHIUSO_ERR_RECUPERABILE
                                session.setTiStato(
                                        Constants.StatoSessioneIngest.CHIUSO_ERR_RECUPERABILE
                                                .name());
                                // assegno codice di errore = PING-CONSCODA-002 e setto dlErr
                                session.setCdErr(MessaggiWSBundle.PING_CONSCODA_002);
                                // setto dlErr = Il versamento di almeno una unità documentaria è
                                // fallito
                                session.setDlErr(MessaggiWSBundle
                                        .getString(MessaggiWSBundle.PING_CONSCODA_002));
                                // aggiorno l'oggetto assegnando stato = CHIUSO_ERR_RECUPERABILE
                                object.setTiStatoObject(
                                ...
                                // MEV 35357 adeguato il comportamento al pulsante "IMPOSTA COME
                                // ERRORE RECUPERABILE"
                                ...
                                /*
                                 * se almeno una UD è stata chiusa in errore la sessione viene
                                 * chiusa in errore ed assume lo stato CHIUSO_ERR_VERS (anche se
                                 * potenzialmente alcune UD sono state inviate a SACER)
                                 */
                                session.setTiStato(
                                ...
                                // fallito
                                session.setDlErr(MessaggiWSBundle
                                        .getString(MessaggiWSBundle.PING_CONSCODA_001));
                                ...
                                // non cancello più il file perché adesso questo caso viene trattato
                                // come quello
                                ...
                                if (object.getPigObjectPadre() != null) {
                                    ...
                                    if (pigStrumentiUrbanistici != null) {
                                        ...
                                        if (unitaDoc.getCdErrSacer() != null
                                            ...
                                            errore = messaggiHelper
                                                    .retrievePigErrore("PING-ERRSU28");
                                        }
                                        pigStrumentiUrbanistici = strumentiUrbanisticiHelper
                                                .aggiornaStato(pigStrumentiUrbanistici,
                                        ...
                                        if (pigStrumentiUrbanistici.getCdErr() == null
                                                || pigStrumentiUrbanistici.getCdErr()
                                                        .equals("PING-ERRSU27")) {
                                ...
                                if (object.getPigObjectPadre() != null) {
                                    ...
                                    if (pigSisma != null) {
                                        PigErrore errore = messaggiHelper
                                                .retrievePigErrore("PING-ERRSSISMA27");
                                        pigSisma = sismaHelper.aggiornaStato(pigSisma,
                        ...
                        if (deleteFtp) {
                            ...
                            // MEV25602 e MEV34843 - mi assicuro di cancellare anche il file su OS
                            // se presente.
    ...
    /*
     * MEV#14100 - Metodo riutilizzabile che definisce lo stato dell'oggetto padre e gestisce la
     * cancellazione delle cartelle degli oggetti trasformati e quella del padre
     */
    public void definisciStatoOggettoPadre(Long idOggettoPadre) throws LockingFailedException,
        ...
        if (idOggettoPadre != null) {
            ...
            if (oggettoPadre != null) {
                String statoPadreCalcolato = codaHelper
                        .getCalcoloStatoObjDaTrasf(oggettoPadre.getIdObject());
                if (statoPadreCalcolato != null) {
                    Date now = Calendar.getInstance().getTime();
                    String rootDirTrasf = commonDb.getRootTrasfParam();
                    PigSessioneIngest sessionePadre = codaHelper
                    ...
                    if (statoPadreCalcolato.equals(Constants.StatoOggetto.CHIUSO_OK.name())) {
                        // Ulteriore modifica successiva che evita di creare 'n' volte lo stato
                        // chiuo Ok nella
                        // sessione
                        if (!oggettoPadre.getTiStatoObject()
                                .equals(Constants.StatoOggetto.CHIUSO_OK.name())) {
                            oggettoPadre.setTiStatoObject(Constants.StatoOggetto.CHIUSO_OK.name());
                            sessionePadre
                                    .setTiStato(Constants.StatoSessioneIngest.CHIUSO_OK.name());
                            sessionePadre.setDtChiusura(now);
                            codaHelper.creaStatoSessione(sessionePadre,
                                    Constants.StatoSessioneIngest.CHIUSO_OK.name(), now);
                            // Punto 4) Elimina le cartelle degli oggetti generati da trasformazione
                            // a partire dal
                            // parametro "ROOT_FTP"
                            // MAC#15470
                            if (oggettoPadre.getTiGestOggettiFigli() != null
                                    && oggettoPadre.getTiGestOggettiFigli().equals(
                            ...
                            /*
                             * Punto 5) evo#17165 5. se esiste un PIG_STRUMENTI_URBANISTICI avente
                             * cd_key = oggettoPadre.cd_key_object ed il campo ti_stato di
                             * PIG_STRUMENTI_URBANISTICI = ‘IN_ELABORAZIONE’ allora il sistema apre
                             * una transazione ed aggiorna lo stato di PIG_STRUMENTI_URBANISTICI =
                             * ‘VERSATO’.
                             */
                        ...
                        if (statoPadreCalcolato.equals(Constants.StatoOggetto.ANNULLATO.name())) {
                            ...
                            sessionePadre
                                    .setTiStato(Constants.StatoSessioneIngest.ANNULLATA.name());
                            sessionePadre.setDtChiusura(now);
                            codaHelper.creaStatoSessione(sessionePadre,
                            ...
                            oggettoPadre.setTiStatoObject(
                                    Constants.StatoOggetto.CHIUSO_ERR_VERS.name());
                            sessionePadre.setTiStato(
                                    Constants.StatoSessioneIngest.CHIUSO_ERR_VERS.name());
                            sessionePadre.setDtChiusura(now);
                            codaHelper.creaStatoSessione(sessionePadre,
                                    Constants.StatoSessioneIngest.CHIUSO_ERR_VERS.name(), now);
                            sessionePadre.setCdErr(MessaggiWSBundle.PING_ERR_VERS_001);
                            sessionePadre.setDlErr(
                                    MessaggiWSBundle.getString(MessaggiWSBundle.PING_ERR_VERS_001));
                            ...
                            // Punto 6) Elimina le cartelle degli oggetti generati da trasformazione
                            // a partire dal
                            // parametro "ROOT_FTP"
                            // MAC#15470
                            if (oggettoPadre.getTiGestOggettiFigli() != null
                    ...
                    log.debug(
                            "Il calcolo dello stato dell'oggetto padre [{}] ha determinato uno stato NULLO",
                            idOggettoPadre);
                }
            }
        }
    ...
    public void deleteDir(String infoLog, String rootDir, String ftpPath, String ftpDir)
        ...
        try {
            ...
            log.warn(" {} errore nella cancellazione della cartella {}{}{}: la risorsa non esiste",
                    infoLog, rootDir, ftpPath, ftpDir);
        } finally {
            if (xadConn != null) {
                xadConn.close();
    ...
    /*
     * Funzione di cancellazione (ricorsiva) di una cartella tranne se stessa (cartellaPrincipale)
     * ed il file "cartellaPrincipale".zip contenuto sotto di lei. Il resto cancella tutto.
     */
    public void eliminaCartellaTranneSeStessa(String infoLog, String rootDir, String ftpPath,
            String ftpDir)
            throws InterruptedException, FileUnderUseException, LockingFailedException,
            DirectoryNotEmptyException, InsufficientPermissionOnFileException,
            NoTransactionAssociatedException, ResourceException {
        ...
        try {
            ...
            // per eliminare la dir temporanea estratta dallo zip
            int livello = 0;
            Util.eliminaCartellaRicorsivamenteTranneSeStessa(tmpDirectory, xadConn, ftpDir,
                    livello);
        } catch (FileNotExistsException ex) {
            // la mancata cancellazione della cartella non è un errore.
            log.warn("{} errore nella cancellazione della cartella {}{}{}: la risorsa non esiste",
    ...
    private void handlePositivoWarning(String codiceEsito, PigUnitaDocObject unitaDoc,
        ...
        unitaDocSessione
                .setTiStatoUnitaDocSessione(Constants.StatoUnitaDocSessione.VERSATA_OK.name());
        ...
        log.debug("{} Stato UD '{}' {}", infoLog, unitaDoc.getIdUnitaDocObject(),
                unitaDoc.getTiStatoUnitaDocObject());
        // incremento il numero delle unità documentarie versate con successo
    }

    private void handleNegativo(String codiceEsito, PigUnitaDocObject unitaDoc,
            PigUnitaDocSessione unitaDocSessione, String infoLog, String codiceErrore,
            String messaggioErrore) {
        log.debug(
                "{} Esito versamento UD '{}' {} modifico lo stato della UD SESSIONE in VERSATA_ERR",
                infoLog, unitaDoc.getIdUnitaDocObject(), codiceEsito);
        unitaDocSessione
                .setTiStatoUnitaDocSessione(Constants.StatoUnitaDocSessione.VERSATA_ERR.name());
