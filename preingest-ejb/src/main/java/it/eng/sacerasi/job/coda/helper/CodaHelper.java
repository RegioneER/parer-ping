...
public class CodaHelper {
    ...
    public List<PigVers> retrieveVersatori() {
        ...
        Query q = em.createQuery("SELECT v FROM PigSessioneIngest ses, PigObject obj " + "JOIN   ses.pigVer v "
    ...
    public List<Long> retrieveUnitaDocsIdByIdObjAndState(Long objId, String state) {
        Query q = em.createQuery("SELECT ud.idUnitaDocObject " + "FROM PigUnitaDocObject ud "
                + "where ud.pigObject.idObject = :objId " + "and ud.tiStatoUnitaDocObject = :state");
    ...
    public Long retrieveXmlIdByUdIdAndType(long idUnitaDocObject, String type) {
        Query q = em.createQuery("SELECT xmlUd.idXmlSacerUnitaDoc " + "FROM PigXmlSacerUnitaDoc xmlUd "
                + "where xmlUd.pigUnitaDocObject.idUnitaDocObject = :unitaDocId " + "and xmlUd.tiXmlSacer = :type");
    ...
    /*
     * Il sistema conta il numero di StudiDicom versati nella data odierna + il numero di StudiDicom in stato =
     * IN_CODA_VERS
     */
    public long contaStudiDicomVersatiOggiEInCodaVers() {
        Query q = em.createNativeQuery("SELECT " + "    COUNT(1) " + "FROM " + "    pig_sessione_ingest         ses "
    ...
    public PigSessioneIngest retrieveSessionByObject(PigObject object) {
        Query q = em.createQuery("SELECT ses " + "FROM PigObject obj JOIN obj.pigSessioneIngests ses "
                + "where obj.idObject = :objectId " + "and obj.idLastSessioneIngest = ses.idSessioneIngest");
    ...
    public Long countUdInObj(PigObject object, String state, String errCode) {
        Query q = em.createQuery("SELECT count(ud) " + "FROM PigObject obj JOIN obj.pigUnitaDocObjects ud "
    ...
    public PigUnitaDocSessione findLockPigUnitaDocSessioneById(Long unitaDocSessionId) {
        final LockModeType lockModeType = LockModeType.PESSIMISTIC_WRITE;
        log.debug("Leggo il record PigUnitaDocSessione id={} e faccio lock {}", unitaDocSessionId, lockModeType);
        Map<String, Object> properties = new HashMap<>();
        properties.put(JPA_PORPERTIES_TIMEOUT, 25000);
        /*
         * Attenzione, Oracle usa multi version control quindi il LockModeType.PESSIMISTIC_WRITE impedisce scritture
    ...
    public PigUnitaDocSessione retrievePigUnitaDocSessioneByKeyUD(BigDecimal idSessioneIngest,
        ...
        Query q = em.createQuery("SELECT udSes " + "FROM PigUnitaDocSessione udSes "
                + "WHERE udSes.pigSessioneIngest.idSessioneIngest = :idSessione " + "AND udSes.aaUnitaDocSacer = :aaUd "
                + "AND udSes.cdKeyUnitaDocSacer = :keyUd " + "AND udSes.cdRegistroUnitaDocSacer = :regUd");
    ...
    public Long checkConsumed(String messageSelector, BigDecimal paramToCheck) {
        ...
        if (Constants.TipoSelettore.CODA1.name().equals(messageSelector)
            ...
            Query q = em.createQuery("SELECT count(obj) " + "FROM PigObject obj JOIN obj.pigSessioneIngests ses "
                    + "WHERE obj.idObject = :objId " + "AND ses.idSessioneIngest = obj.idLastSessioneIngest "
    ...
    public String getCalcoloStatoObjDaTrasf(Long idObjectPadre) {
        MonVCalcStatoObjDaTrasf obj = em.find(MonVCalcStatoObjDaTrasf.class, new BigDecimal(idObjectPadre));
        return (obj == null ? null : obj.getTiStatoObjectPadre());
    }

    /**
     * Effettua l'aggiornamento dei contatori in un'operazione singola per demandare al database la gestione della
     * concorrenza. Aggiorna sempre {@link PigSessioneIngest#niUnitaDocVers} ed eventuali altri contatori in base ai
     * parametri che vengono passati.
     *
     * @param idSessioneIngest
     *            primary key di {@link PigSessioneIngest}
     * @param incrementaVersateOk
     *            se true indica di aggiornare {@link PigSessioneIngest#niUnitaDocVersOk}
     * @param incrementaVersateErrore
    ...
    public PigSessioneIngest aggiornaContatori(long idSessioneIngest, boolean incrementaVersateOk,
        ...
        log.debug("Prima del refresh PigSessioneIngest id={} niUnitaDocVers={}", session.getIdSessioneIngest(),
                session.getNiUnitaDocVers());
        em.refresh(session);
        log.debug("Dopo refresh PigSessioneIngest id={} niUnitaDocVers={}", session.getIdSessioneIngest(),
                session.getNiUnitaDocVers());
