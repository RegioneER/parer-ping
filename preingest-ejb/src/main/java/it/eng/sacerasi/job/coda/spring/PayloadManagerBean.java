...
package it.eng.sacerasi.job.coda.spring;
...
@Transactional
public class PayloadManagerBean {

    private static final Logger log = LoggerFactory.getLogger(PayloadManagerBean.class);
    private static final String THREAD_INTERROTTO_CANCELLAZIONE = "Thread interrotto in cancellazione cartella: {}";
    ...
    public void manageMessagePayload(Payload payload, String consumerName)
        ...
        try {
            ...
            if (inCodaVersamento) {
                ...
                RichiestaSacerInput input = new RichiestaSacerInput(
                        ...
                        unitaDoc.getCdVerWsSacer(), urlVersamento, xmlVersamentoSacer, xmlIndice,
                        userIdSacer,
                        passwordSacer, timeout);
                // chiamo il Servizio Multimedia di Sacer
                ...
                if (esitoConnessione.isErroreConnessione()) {
                    log.debug("{} UD '{}' andata in timeout", infoLog,
                            unitaDoc.getIdUnitaDocObject());
                    unitaDoc.setTiStatoUnitaDocObject(StatoUnitaDocObject.VERSATA_TIMEOUT.name());
                    // MEV 27407
                    Date dtStato = new Date();
                    unitaDoc.setDtStato(dtStato);
                    unitaDocSessione.setTiStatoUnitaDocSessione(
                            StatoUnitaDocSessione.VERSATA_TIMEOUT.name());
                ...
                if (session.getNiUnitaDocVers().intValue() == session.getNiUnitaDocDaVers()
                    ...
                    if (session.getNiUnitaDocVersOk().intValue() == session
                        ...
                        if (timeoutUd > 0 || err666Ud > 0 || err666PUd > 0) {
                            ...
                            /*
                             * se almeno una UD è stata chiusa in errore la sessione viene chiusa in
                             * errore ed assume lo stato CHIUSO_ERR_VERS (anche se potenzialmente
                             * alcune UD sono state inviate a SACER)
                             */
                            session.setTiStato(
                            ...
                            if (oggettoPadre.getTiGestOggettiFigli() != null
                                ...
                                for (PigObjectTrasf objectFiglio : oggettoPadre
                                    ...
                                    try {
                                        ...
                                        log.error(THREAD_INTERROTTO_CANCELLAZIONE,
                                                e.getMessage());
                                        Thread.currentThread().interrupt();
                                    }
                                }
