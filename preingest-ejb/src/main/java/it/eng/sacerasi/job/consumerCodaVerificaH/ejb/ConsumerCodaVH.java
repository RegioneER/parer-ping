...
public class ConsumerCodaVH implements MessageListener {
        ...
        try {
            ...
            if (message instanceof TextMessage) {
                ...
                if (controllaLockStatoVerHash(tmpPayloadCdPrepXml.getIdLastSessioneIngest(),
                    ...
                    tmpOggettoInCoda.setRifPigObject(riagganciaPigObject(tmpPayloadCdPrepXml.getIdPigObject()));
    ...
    private void impostaStatoVerHashSuccess(PigObject oggetto) throws ParerInternalError {
        try {
            ...
            if (oggetto.getPigTipoObject().getTiVersFile().equals(Constants.TipoVersamento.NO_ZIP.name())
                ...
                RispostaControlli tmpRispostaControlli = salvataggioDati.modificaSessione(idLastSessioneIngest,
                ...
                tmpRispostaControlli = salvataggioDati.creaStatoSessione(idLastSessioneIngest,
                        Constants.StatoSessioneIngest.IN_ATTESA_SCHED.name(), Calendar.getInstance().getTime());
                if (tmpRispostaControlli.getCodErr() != null) {
                    logError(tmpRispostaControlli);
                    throw new ParerInternalError(tmpRispostaControlli.getCodErr());
                }
                ...
                tmpRispostaControlli.reset();
                tmpRispostaControlli = salvataggioDati.creaStatoSessione(idLastSessioneIngest,
                        Constants.StatoSessioneIngest.DA_TRASFORMARE.name(), Calendar.getInstance().getTime());
                if (tmpRispostaControlli.getCodErr() != null) {
                    logError(tmpRispostaControlli);
                    throw new ParerInternalError(tmpRispostaControlli.getCodErr());
                }
    ...
    private boolean controllaLockStatoVerHash(long idLastSessioneIngest, Constants.StatoVerificaHash stato)
