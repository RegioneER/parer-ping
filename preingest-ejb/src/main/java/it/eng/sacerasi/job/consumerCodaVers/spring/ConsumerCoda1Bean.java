...
public class ConsumerCoda1Bean {

    private static final String DESC_CONSUMER = "CCV1";

    Logger log = LoggerFactory.getLogger(ConsumerCoda1Bean.class);
    ...
    @Autowired
    public ConsumerCoda1Bean(PayloadManagerBean payloadManagerHelper) {
        this.payloadManagerHelper = payloadManagerHelper;
    }

    @KafkaListener(id = "#{systemProperties['kafka.prefix']}.ConsumerGroupCV1", groupId = "#{systemProperties['kafka.prefix']}.spring-sacer_ping.coda1", topics = "#{systemProperties['kafka.prefix']}.sacer_ping.coda1.raw", concurrency = "#{systemProperties['kafka.concurrency.ping.coda1']}")
    @Transactional
    public void listenCCV1(GenericRecord avroRecord) {
