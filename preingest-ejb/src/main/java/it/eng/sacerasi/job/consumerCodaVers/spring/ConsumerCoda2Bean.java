 ...
 * @author DiLorenzo_F
 */
public class ConsumerCoda2Bean {

    private static final String DESC_CONSUMER = "CCV2";

    Logger log = LoggerFactory.getLogger(ConsumerCoda2Bean.class);
    ...
    @Autowired
    public ConsumerCoda2Bean(PayloadManagerBean payloadManagerHelper) {
        this.payloadManagerHelper = payloadManagerHelper;
    }

    @KafkaListener(id = "#{systemProperties['kafka.prefix']}.ConsumerGroupCV2", groupId = "#{systemProperties['kafka.prefix']}.spring-sacer_ping.coda2", topics = "#{systemProperties['kafka.prefix']}.sacer_ping.coda2.raw", concurrency = "#{systemProperties['kafka.concurrency.ping.coda2']}")
    @Transactional
    public void listenCCV2(GenericRecord avroRecord) {
