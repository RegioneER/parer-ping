...
public class ProduzioneXmlEjb {
    ...
    /**
     * Esegue la produzione degli xml di versamento e di indice multimedia con i dati contenuti
     * nell'oggetto in coda
     *
     * @param oggetto l'oggetto in coda
     *
	...
	for (UnitaDocObject udObj : obj.getListaUnitaDocObject()) {
		...
		if (fileUd.getRifPigTipoFileObject().getTiDocSacer()
		    ...
		    doc.setIDDocumento(
			    fileUd.getRifPigTipoFileObject().getTiDocSacer() + allegCounter);
		    ...
		    doc = new DocumentoType();
		    doc.setIDDocumento(
			    fileUd.getRifPigTipoFileObject().getTiDocSacer() + annesCounter);
		    ...
		    doc = new DocumentoType();
		    doc.setIDDocumento(
			    fileUd.getRifPigTipoFileObject().getTiDocSacer() + annotCounter);
    ...
    private void marshallXmlVers(UnitaDocumentaria unitaDoc, UnitaDocObject udObj)
	...
	Marshaller indexMarshaller = xmlContextCache.getVersReqMMCtxforIndiceMM()
		.createMarshaller();
    ...
    private void popolaIntestazione(IntestazioneType intestazione, PigObject pigObject,
	    UnitaDocObject udObject) {
	...
	intestazione.setTipologiaUnitaDocumentaria(
		pigObject.getPigTipoObject().getNmTipoUnitaDocSacer());
    ...
    // Popolo la configurazione con i flag contenuti nella tabella pigTipoObject relativa
    // all'oggetto versato
    private void popolaConfigurazione(ConfigType configurazione, PigObject pigObject) {
	configurazione.setForzaAccettazione(pigObject.getPigTipoObject()
		.getFlForzaAccettazioneSacer().equals(Constants.DB_TRUE));
	configurazione.setForzaCollegamento(
		pigObject.getPigTipoObject().getFlForzaCollegamento().equals(Constants.DB_TRUE));
	configurazione.setForzaConservazione(
		pigObject.getPigTipoObject().getFlForzaConservazione().equals(Constants.DB_TRUE));
	configurazione.setTipoConservazione(
		TipoConservazioneType.valueOf(pigObject.getPigTipoObject().getTiConservazione()));
    }
	...
	if (profiloArchivisticoAsync != null) {
	    ...
	    profiloArchivisticoSacer.getFascicoloPrincipale().setClassifica(
		    profiloArchivisticoAsync.getFascicoloPrincipale().getClassifica());
	    ...
	    profiloArchivisticoSacer.getFascicoloPrincipale()
		    .setSottoFascicolo(new FascicoloType());

	    profiloArchivisticoSacer.getFascicoloPrincipale().getFascicolo()
		    .setIdentificativo(profiloArchivisticoAsync.getFascicoloPrincipale()
	    ...
	    profiloArchivisticoSacer.getFascicoloPrincipale().getSottoFascicolo()
		    .setIdentificativo(profiloArchivisticoAsync.getFascicoloPrincipale()
			    .getSottoFascicolo().getIdentificativo());
	    // Nuovo JAXB
	    JAXBElement<String> str2 = fact.createFascicoloTypeOggetto(profiloArchivisticoAsync
		...
		JAXBElement<String> str3 = fact
			.createFascicoloTypeOggetto(fasc.getFascicolo().getOggetto());
		camic.getFascicolo().setOggetto(str3);
		camic.getSottoFascicolo()
			.setIdentificativo(fasc.getSottoFascicolo().getIdentificativo());
		JAXBElement<String> str4 = fact
			.createFascicoloTypeOggetto(fasc.getSottoFascicolo().getOggetto());
		camic.getSottoFascicolo().setOggetto(str4);
		profiloArchivisticoSacer.getFascicoliSecondari().getFascicoloSecondario()
			.add(camic);
	    }
	}
    }
    ...
    private void popolaProfiloUnitaDocumentaria(ProfiloUnitaDocumentariaType profiloUdSacer,
	    it.eng.sacerasi.ws.xml.invioAsync.ProfiloUnitaDocumentariaType profiloUdAsync,
	    OggettoInCoda obj) {
	if (profiloUdAsync != null) {
	    profiloUdSacer.setData(Utils.getStandardXsDateFormat(
		    profiloUdAsync.getData().toGregorianCalendar().getTime()));
		...
		if (obj.getRifPigObject().getPigInfoDicoms().get(0) != null) {
		    Date dtStudyDate = obj.getRifPigObject().getPigInfoDicoms().get(0)
			    .getDtStudyDate();
		    String modality = obj.getRifPigObject().getPigInfoDicoms().get(0)
			    .getDlListaModalityInStudy();
    ...
    private void popolaDatiSpecifici(DatiSpecificiType datiSpecificiSacer,
	    it.eng.sacerasi.ws.xml.invioAsync.DatiSpecificiType datiSpecificiAsync,
	    OggettoInCoda obj) throws ParerInternalError {
	...
	if (datiSpecificiAsync != null) {
		...
		for (PigXsdDatiSpec xsdDatiSpec : obj.getRifPigObject().getPigTipoObject()
			.getPigXsdDatiSpecs()) {
			...
			try {
				...
				if (att.getFlVersSacer().equals(Constants.DB_TRUE)) {
				    ...
				    char[] delimiters = {
					    '_' };
				    // Istanzio il metodo definito come get + nome della colonna da
				    // estrarre degli
				    // attributi
				    // della PigInfoDicom, sostituendo gli '_' e capitalizzando i
				    // termini
				    // ES: cd_AET_nodo_dicom -> getCdAetNodoDicom
				    Method method = c.getDeclaredMethod("get" + WordUtils
					    .capitalizeFully(att.getNmColDatiSpec(), delimiters)
					    .replace("_", ""));
				    // Invoca il metodo ottenuto sulla tabella PigInfoDicom
				    // dell'oggetto per ottenere il
				    // valore di quell'attributo
				    Object value = method.invoke(
					    obj.getRifPigObject().getPigInfoDicoms().get(0));
				    if (value != null) {
					// NUOVO CODICE JAXB
					// Crea un elemento per il campo Any e lo aggiunge ai dati
					// specifici
					DocumentBuilderFactory dbf = DocumentBuilderFactory
						.newInstance();
					// XXE: This is the PRIMARY defense. If DTDs (doctypes) are
					// disallowed,
					...
					// "XML Schema, DTD, and Entity Attacks" (see reference
					// below)
					dbf.setXIncludeAware(false);
					dbf.setExpandEntityReferences(false);
					// As stated in the documentation, "Feature for Secure
					// Processing (FSP)" is the
					// central mechanism that will
					// help you safeguard XML processing. It instructs XML
					// processors, such as
					// parsers, validators,
					// and transformers, to try and process XML securely, and
					// the FSP can be used as
					// an alternative to
					// dbf.setExpandEntityReferences(false); to allow some safe
					// level of Entity
					// Expansion
					// Exists from JDK6.
					...
					// "If for some reason support for inline DOCTYPEs are a
					// requirement, then
					// ensure the entity settings are disabled (as shown above)
					// and beware that SSRF
					// attacks
					// (http://cwe.mitre.org/data/definitions/918.html) and
					// denial
					// of service attacks (such as billion laughs or
					// decompression bombs via "jar:")
					// are a risk."
					DocumentBuilder docBuilder = dbf.newDocumentBuilder();
					Document doc = docBuilder.newDocument();
					Element elem = doc.createElement(att.getNmAttribDatiSpec());
					if (value instanceof Date && att.getTiDatatypeCol().equals(
						Constants.AttribDatiSpecDataType.DATETIME.name())) {
					    elem.setTextContent(timeStampDf.format(value));
					} else if (value instanceof Date && att.getTiDatatypeCol()
						.equals(Constants.AttribDatiSpecDataType.DATA
							.name())) {
		...
		datiSpecificiSacer
			.setVersioneDatiSpecifici(datiSpecificiAsync.getVersioneDatiSpecifici());
	...
	// Ciclo sulle unita documentarie dell'xml per caricare i dati dell'unita documentaria
	// corretta
	for (it.eng.sacerasi.ws.xml.invioAsync.UnitaDocumentariaType unitaDocumentaria : obj
		...
		String xmlChiave = unitaDocumentaria.getChiave().getTipoRegistro() + "^"
			+ unitaDocumentaria.getChiave().getAnno() + "^"
			+ unitaDocumentaria.getChiave().getNumero();
		String objChiave = udObj.getChiaveUd().getRegistro() + "^"
			+ udObj.getChiaveUd().getAnno() + "^" + udObj.getChiaveUd().getNumero();
    ...
    private void popolaDatiUnitaDoc(
	    it.eng.sacerasi.ws.xml.invioAsync.UnitaDocumentariaType unitaDocumentaria,
	...
	if (unitaDocumentaria.getProfiloArchivistico() != null) {
	    ...
	    popolaProfiloArchivistico(unitaDoc.getProfiloArchivistico(),
		    unitaDocumentaria.getProfiloArchivistico());
	}
	...
	if (unitaDocumentaria.getDatiSpecifici() != null) {
	    ...
	    JAXBElement<DatiSpecificiType> elem = fact
		    .createUnitaDocumentariaDatiSpecifici(new DatiSpecificiType());
	    unitaDoc.setDatiSpecifici(elem);
	    popolaDatiSpecifici(unitaDoc.getDatiSpecifici().getValue(),
		    unitaDocumentaria.getDatiSpecifici().getValue(), obj);
	}
	if (unitaDocumentaria.getDocumentiCollegati() != null) {
	    unitaDoc.setDocumentiCollegati(new DocumentoCollegatoType());
	    popolaDocumentiCollegati(unitaDoc.getDocumentiCollegati(),
		    unitaDocumentaria.getDocumentiCollegati());
	}
    }
	...
	if (documentiCollegatiAsync != null) {
		...
		docSacer.getChiaveCollegamento()
			.setTipoRegistro(doc.getChiaveCollegamento().getTipoRegistro());
	...
	if (fileUd.getParsedFileType() != null
		&& fileUd.getParsedFileType().getProfiloDocumento() != null) {
	    doc.setProfiloDocumento(new ProfiloDocumentoType());
	    doc.getProfiloDocumento()
		    .setAutore(fileUd.getParsedFileType().getProfiloDocumento().getAutore());
	    doc.getProfiloDocumento().setDescrizione(
		    fileUd.getParsedFileType().getProfiloDocumento().getDescrizione());
	} else {
	    if (obj.getTipoOggetto() == Constants.TipiOggetto.STUDIO_DICOM) {
		doc.setProfiloDocumento(new ProfiloDocumentoType());
		if (obj.getRifPigObject().getPigInfoDicoms().get(0) != null) {
		    doc.getProfiloDocumento().setAutore(
			    obj.getRifPigObject().getPigInfoDicoms().get(0).getDsInstitutionName());
		    doc.getProfiloDocumento().setDescrizione(obj.getRifPigObject()
			    .getPigInfoDicoms().get(0).getDlStudyDescription());
		}
	    }
	}
	if (fileUd.getParsedFileType() != null
		&& fileUd.getParsedFileType().getDatiSpecifici() != null) {
	    ...
	    doc.getDatiSpecifici().getValue().setVersioneDatiSpecifici(fileUd.getParsedFileType()
		    .getDatiSpecifici().getValue().getVersioneDatiSpecifici());
	    doc.getDatiSpecifici().getValue().getAny()
		    .addAll(fileUd.getParsedFileType().getDatiSpecifici().getValue().getAny());
	}
	if (fileUd.getParsedFileType() != null
		&& fileUd.getParsedFileType().getDatiFiscali() != null) {
	    doc.setDatiFiscali(new DatiFiscaliType());
	    doc.getDatiFiscali().setCF(fileUd.getParsedFileType().getDatiFiscali().getCF());
	    doc.getDatiFiscali()
		    .setCognome(fileUd.getParsedFileType().getDatiFiscali().getCognome());
	    doc.getDatiFiscali().setDataEmissione(
		    fileUd.getParsedFileType().getDatiFiscali().getDataEmissione());
	    doc.getDatiFiscali().setDataTermineEmissione(
		    fileUd.getParsedFileType().getDatiFiscali().getDataTermineEmissione());
	    doc.getDatiFiscali().setDenominazione(
		    fileUd.getParsedFileType().getDatiFiscali().getDenominazione());
	    doc.getDatiFiscali().setNome(fileUd.getParsedFileType().getDatiFiscali().getNome());
	    doc.getDatiFiscali().setNumeroProgressivo(
		    fileUd.getParsedFileType().getDatiFiscali().getNumeroProgressivo());
	    doc.getDatiFiscali().setPIVA(fileUd.getParsedFileType().getDatiFiscali().getPIVA());
	    doc.getDatiFiscali().setPeriodoFiscale(
		    fileUd.getParsedFileType().getDatiFiscali().getPeriodoFiscale());
	    doc.getDatiFiscali()
		    .setRegistro(fileUd.getParsedFileType().getDatiFiscali().getRegistro());
	}
	doc.setStrutturaOriginale(new StrutturaType());
	doc.getStrutturaOriginale()
		.setTipoStruttura(fileUd.getRifPigTipoFileObject().getNmTipoStrutDocSacer());
	...
	if (fileUd.getParsedFileType() != null) {
	    comp.setUtilizzoDataFirmaPerRifTemp(
		    fileUd.getParsedFileType().isUtilizzoDataFirmaPerRifTemp());
	}
	if (fileUd.getParsedFileType() != null
		&& fileUd.getParsedFileType().getRiferimentoTemporale() != null) {
	...
	if (fileUd.getParsedFileType() != null
	    ...
	    comp.setDescrizioneRiferimentoTemporale(
		    fileUd.getParsedFileType().getDescrizioneRiferimentoTemporale());
	}
    ...
    private void popolaComponenteMMZipConXml(
	    it.eng.parer.ws.xml.versReqMultiMedia.ComponenteType comp, FileUnitaDoc fileUd) {
	comp.setID(fileUd.getIdFile());
	comp.setURNFile(fileUd.getUrnFile());
	if (fileUd.getUrnFileInZip() != null) {
	    // il tag PathOggetto, ignorato da SACER, contiene l'indirizzo orginale del file
	    // all'interno del file ZIP
    ...
    private void popolaComponenteMMNoZipZipNoXml(
	    it.eng.parer.ws.xml.versReqMultiMedia.ComponenteType comp, FileUnitaDoc fileUd) {
	comp.setID(fileUd.getIdFile());
	comp.setURNFile(fileUd.getUrnFile());
	if (fileUd.getUrnFileInZip() != null) {
	...
	boolean calcolaHashSacer = fileUd.getRifPigTipoFileObject().getFlCalcHashSacer()
		.equals(Constants.DB_TRUE);
	...
	if (!verificaFormatiSacer) {
	    ...
	    comp.getForzaFormato()
		    .setFormatoStandard(fileUd.getRifPigTipoFileObject().getNmFmtFileCalcSacer());
	    comp.getForzaFormato().setFormatoRappresentazioneEsteso(
		    fileUd.getRifPigTipoFileObject().getDsFmtRapprEstesoCalcSacer());
	    comp.getForzaFormato().setFormatoRappresentazioneCompatto(
		    fileUd.getRifPigTipoFileObject().getDsFmtRapprCalcSacer());
	}
