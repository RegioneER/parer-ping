...
public class SalvaErrorePrepXml {
    ...
    /*
     * Dalla documentazione: --------------------- 1) il sistema aggiorna la sessione in
     * PIG_SESSIONE_INGEST con dtChiusura pari all?istante corrente, tiStato = CHIUSO_ERR_SCHED,
     * cdErr e dsErr valorizzati dai controlli svolti 2) il sistema aggiorna l?oggetto corrente,
     * assegnando stato = CHIUSO_ERR_SCHED 3) il sistema elimina i file dall?area FTP; i file sono
     * raggiungibili con il percorso definito da ? cartella root specificata dal parametro
     * ?ROOT_FTP? ? cartella definita dal path di input specificato dal versatore ? cartella
     * definita dal codice identificante l?oggetto 4) il sistema elimina la cartella definita dal
     * codice identificante l?oggetto dall?area FTP 5) aggiorna le unità doc object e unità doc
     * sessione allo stato PREPARA_XML_IN_ERRORE
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void chiudiInErrore(String rootDir, OggettoInCoda oggetto, boolean impostaKoHash)
	...
	if (ctx.getRollbackOnly()) {
	    ctx.getBusinessObject(SalvaErrorePrepXml.class).chiudiInErroreNewTx(rootDir, oggetto,
		    impostaKoHash);
	} else {
	    ctx.getBusinessObject(SalvaErrorePrepXml.class).chiudiInErroreReq(rootDir, oggetto,
		    impostaKoHash);
	}
    }
	...
	bulkUpdateUnitaDoc(PigUnitaDocObject.class.getSimpleName(),
		oggetto.getRifPigObject().getIdObject());
	...
	// Cancella tutto nella cartella tranne se stessa ed il file .zip con lo stesso nome
	// (CD_KEY_OBJECT)
	...
	// Verifico se in salvaTutto era stato messo lo stato IN_ATTESA_VERS. NON deve essere
	// persistito, perciò lo
	// elimino
	if (tmpSessioneIngest.getTiStato().equals(Constants.StatoOggetto.IN_ATTESA_VERS.name())) {
	    ...
	    PigStatoSessioneIngest pigStatoSessione = entityManager
		    .find(PigStatoSessioneIngest.class, idStatoSessioneIngestCor.longValue());
	...
	/*
	 * Eseguo una bulk update nativa per poter utilizzare i dati dalla vista a. se la unità doc
	 * è in errore, stato = PREPARA_XML_IN_ERRORE ed il codice e la descrizione di errore
	 * ritornati dalla vista
	 */
