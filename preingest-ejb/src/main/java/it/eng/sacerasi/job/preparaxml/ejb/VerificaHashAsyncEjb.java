...
public class VerificaHashAsyncEjb {
	...
	if (pigObject.getPigFileObjects() != null && !pigObject.getPigFileObjects().isEmpty()) {
		...
		// MEV 24717 - memorizzo gli eventuali parametri per recuperare l'oggetto da Object
		// Storage
	...
	if (str2Bool(pigTipoObject.getFlContrHash())) {
		...
		if (!tmpVerificaOk) {
		    // MAC 28465 -Sostituito PING-PREPXML-FILE-001 con nuovo errore
		    // PING_VERHASH_FILE_001
	...
	if (file.getRifPigFileObject().getDsHashFileVers() != null
		...
		/*
		 * nota per me: dovrei verificare che i parametri relativi all'algoritmo ed alla
		 * codifica coincidano con SHA ed hexBinary. Per ora ignoro la cosa e confronto
		 * l'hash fornito dal versatore con un hash SHA ed hexBinary del file... se non
		 * coincide rilevo l'errore.
		 */
		file.setTipoHashFile(file.getTipoHashFile());
		file.setEncodingFile(file.getEncodingFile());
		file.setHashFile(tmpHash);

		log.debug("hash calcolato: {} algoritmo {}", file.getHashFile(),
			file.getTipoHashFile());

		if (file.getHashFile()
			.equalsIgnoreCase(file.getRifPigFileObject().getDsHashFileVers())) {
		    tmpret = true;
		} else {
     ...
     */
    private String calculateHash(String fileUrn, String tipoHash)
