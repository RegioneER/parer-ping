...
public class ProducerCodaVersamentoEjb {
	...
	final int numMaxUdDaAccodare = Integer.parseInt(configurationHelper
		.getValoreParamApplicByApplic(Constants.NUM_UNITA_DOC_CODA_VERS));
	...
	try (Stream<PigObject> oggetti = codaHelper
		...
		// MEV#18661 - Se il numero di dicom versati nel giorno è minore del massimo
		// configurato prosegue
		...
		if (Constants.STUDIO_DICOM.equals(object.getPigTipoObject().getNmTipoObject())
		    ...
		    log.debug(
			    ...
			    ((object.getDsObject() == null) ? "" : object.getDsObject()),
			    object.getIdObject(), numMaxDicomXgiorno);
		    return;
		}
		...
		// determino, per l'oggetto, l'insieme delle unità documentarie con stato =
		// DA_VERSARE
		...
		log.debug("{} :: trovate {} unita documentarie da processare per l'oggetto {}",
			infoLogObj, unitaDocsIdToProcess.size(), object.getIdObject());
		// se ho delle UD da processare incremento il contatore degli StudiDicom, voglio
		// evitare di conteggiare
		// Oggetti che per qualche anomalia
		// non hanno UD collegate.
		if (!unitaDocsIdToProcess.isEmpty() && Constants.STUDIO_DICOM
			.equals(object.getPigTipoObject().getNmTipoObject())) {
		...
		for (int k = 0; k < unitaDocsIdToProcess.size(); k++) {
			...
			isLastUdInObj = true;
			log.debug("{} :: ultima unità documentaria dell'oggetto {}", infoLogUd,
				object.getIdObject());
		    }
		    // di ogni ud determino l'identificatore, la chiave e gli xml di versamento a
		    // SACER
		    PigUnitaDocObject unitaDoc = codaHelper
			    .findPigUnitaDocObjectById(unitaDocsIdToProcess.get(k));
		    PigUnitaDocSessione unitaDocSessione = codaHelper
			    .retrievePigUnitaDocSessioneByKeyUD(idLastSessioneIngest,
		    ...
		    BigDecimal idAmbienteVers = BigDecimal
			    .valueOf(unitaDoc.getPigObject().getPigTipoObject().getPigVer()
				    .getPigAmbienteVer().getIdAmbienteVers());
		    BigDecimal idVers = BigDecimal.valueOf(
			    unitaDoc.getPigObject().getPigTipoObject().getPigVer().getIdVers());
	    ...
	    log.info(
		    "{} Raggiunto il limite massimo di {} unità documentarie che si possono mettere in coda",
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void manageUnitaDoc(long unitaDocId, long unitaDocSessioneId, String urlServVers,
	...
	log.debug("{} :: dimensione file  = {} coda '{}' selezionata", infoLogUd,
		unitaDoc.getNiSizeFileByte(), queueToUse);
	try {
	...
	// setto dlErr = Fallita registrazione in coda di versamento per unità doc.
	// <registro>-<anno>-<numero>
	session.setDlErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_PRODCODA_001,
		unitaDoc.getCdRegistroUnitaDocSacer(), unitaDoc.getAaUnitaDocSacer(),
		unitaDoc.getCdKeyUnitaDocSacer()));
    ...
    public Payload buildPayload(PigUnitaDocObject unitaDoc, long unitaDocSessioneId,
	    String urlServVers) {
	...
	// la chiave è formata da: CD_REGISTRO_UNITA_DOC_SACER, AA_UNITA_DOC_SACER,
	// CD_KEY_UNITA_DOC_SACER
	...
	payload.setXmlVersamentoSacerId(codaHelper.retrieveXmlIdByUdIdAndType(
		unitaDoc.getIdUnitaDocObject(), Constants.TipiXmlSacer.XML_VERS.name()));
