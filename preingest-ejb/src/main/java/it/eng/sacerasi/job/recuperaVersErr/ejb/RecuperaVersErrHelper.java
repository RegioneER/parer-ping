...
public class RecuperaVersErrHelper {
    ...
    public void elaboraOggetto(PigObject obj) {
        ...
        if (isValidObject) {
            ...
            log.debug(
                    "JOB {} - creo la nuova sessione con stato IN_ATTESA_VERS per l'oggetto {}, id {}",
                    Constants.NomiJob.RECUPERA_VERS_ERR.name(), obj.getCdKeyObject(),
                    obj.getIdObject());
            ...
            log.debug(
                    "JOB {} - creo gli oggetti Xml per la nuova sessione basandomi sui dati della precedente",
                    Constants.NomiJob.RECUPERA_VERS_ERR.name());
            me.creaXmlSessioneIngest(oldSession, sessione);
            log.debug("JOB {} - creati gli oggetti Xml",
                    Constants.NomiJob.RECUPERA_VERS_ERR.name());
            obj.setIdLastSessioneIngest(new BigDecimal(sessione.getIdSessioneIngest()));

            // MEV 37686
            recuperaErroreSismaOSU(obj);
        }
    }

    private boolean checkIsValidObj(PigObject obj) {
        boolean isValidObj = false;
        String queryStr = "SELECT obj FROM PigObject obj "
                + "WHERE obj.idObject = :idObjectToCheck "
        ...
        query.setParameter("statoErrTimeout",
                Constants.StatoOggetto.CHIUSO_ERR_RECUPERABILE.name());
        ...
        PigObject validObject = ((PigObject) query.setLockMode(LockModeType.PESSIMISTIC_READ)
                .getSingleResult());
        if (validObject != null) {
            isValidObj = true;
        }
    ...
    public PigSessioneIngest creaSessione(PigObject obj, PigSessioneIngest oldSession) {
        ...
        /*
         * Il numero di unità documentarie da versare definito nella sessione è pari al conteggio
         * delle unità documentarie dell'oggetto con stato = VERSATA_TIMEOUT; il numero di unità
         * documentarie versate definito nella sessione è pari a 0
         */
        ...
        sessione.setIdStatoSessioneIngestCor(
    ...
    public List<PigObject> getListaObjects(List<Long> idVersatori, String statoTimeout,
            String statoErrVers, int olderThanMinutes) {
        String queryStr = "SELECT DISTINCT obj FROM PigObject obj "
                + "JOIN obj.pigSessioneIngests siIng "
                ...
                + "WHERE ud.pigObject.idObject = obj.idObject "
                + "AND (ud.tiStatoUnitaDocObject = '"
    ...
    public void registraUD(List<PigUnitaDocObject> pigUnitaDocObjects, PigSessioneIngest sessione,
        ...
        for (PigUnitaDocObject pudo : pigUnitaDocObjects) {
            if (pudo.getTiStatoUnitaDocObject()
                    .equals(Constants.StatoUnitaDocObject.VERSATA_TIMEOUT.name())
                    || (pudo.getTiStatoUnitaDocObject()
                            .equals(Constants.StatoUnitaDocObject.VERSATA_ERR.name())
                            && !pudo.getCdErrSacer()
                                    .equals(Constants.COD_VERS_ERR_CHIAVE_DUPLICATA_NEW))) {
                ...
                udSessione.setTiStatoUnitaDocSessione(
                        Constants.StatoUnitaDocObject.DA_VERSARE.name());
                ...
                for (PigUnitaDocSessione oldUdSessione : oldSession.getPigUnitaDocSessiones()) {
                    if (udSessione.getAaUnitaDocSacer().equals(oldUdSessione.getAaUnitaDocSacer())
                            && udSessione.getCdKeyUnitaDocSacer()
                                    .equals(oldUdSessione.getCdKeyUnitaDocSacer())
                            && udSessione.getCdRegistroUnitaDocSacer()
                                    .equals(oldUdSessione.getCdRegistroUnitaDocSacer())) {
                        for (PigXmlSacerUnitaDocSes oldUdSessXml : oldUdSessione
    ...
    // MEV 37686
    public void recuperaErroreSismaOSU(PigObject obj) {
