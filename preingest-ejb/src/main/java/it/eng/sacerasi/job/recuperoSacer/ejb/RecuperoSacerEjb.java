...
public class RecuperoSacerEjb {
    ...
    public void recuperaOggettiSacer() throws ParerInternalError {
        ...
        if (!tmpSessioni.isEmpty()) {
            // Integer timeout = new Integer(helper.getParamFromParamApplic(Constants.TIMEOUT_VERS_SACER));
            Integer timeout = new Integer(
                    configurationHelper.getValoreParamApplicByApplic(Constants.TIMEOUT_VERS_SACER));
            // Ogni sessione è correlata a un oggetto PigObject, che a sua volta contiene un unica unità documentaria
            for (PigSessioneRecup sessioneRec : tmpSessioni) {
                PigUnitaDocObject unitaDocObj = sessioneRec.getPigObject().getPigUnitaDocObjects().get(0);
                // Inizializzo l'oggetto di cui poi dovrò eseguire il marshall per fare la richiesta a Sacer
                ...
                // unitaDocObj.getPigObject().getPigTipoObject().getIdTipoObject(), unitaDocObj.getIdOrganizIam());
                ...
                BigDecimal idAmbienteVers = BigDecimal.valueOf(sessioneRec.getPigObject().getPigTipoObject().getPigVer()
                ...
                nmUseridSacer = configurationHelper.getValoreParamApplicByTipoObj("USERID_USER_VERS", idAmbienteVers,
                        idVers, idTipoObject);
                cdPswSacer = configurationHelper.getValoreParamApplicByTipoObj("PSW_USER_VERS", idAmbienteVers, idVers,
                        idTipoObject);
                ...
                recupero.getChiave().setAnno(BigInteger.valueOf(unitaDocObj.getAaUnitaDocSacer().longValue()));
                ...
                try {
                    Marshaller recMarshaller = xmlContextCache.getVersReqStatoCtx_Recupero().createMarshaller();
                ...
                // Inizializzo l'oggetto con cui dovrò fare il marshall per definire l'output per il recupero
                ...
                // Long idAmbienteVers = sessioneRec.getPigObject().getPigTipoObject().getPigVer().getPigAmbienteVer()
                // .getIdAmbienteVers();
                // Long idVers = sessioneRec.getPigObject().getPigTipoObject().getPigVer().getIdVers();
                // Long idTipoObject = sessioneRec.getPigObject().getPigTipoObject().getIdTipoObject();
                indice.setOutputPath(sessioneRec.getPigObject().getPigTipoObject().getPigVer().getDsPathOutputFtp());
                ...
                try {
                    Marshaller recMarshaller = xmlContextCache.getVersReqStatoMMCtx_IndiceMM().createMarshaller();
                ...
                // String urlRecuperoUdSacer = sessioneRec.getPigVer().getPigAmbienteVer().getDsUrlServRecup();
                String urlRecuperoUdSacer = configurationHelper.getValoreParamApplicByTipoObj("DS_URL_SERV_RECUP",
                        idAmbienteVers, idVers, idTipoObject);
                // Creo l'oggetto contenente i suoi parametri (TipoRichiesta = RECUPERO)
                RichiestaSacerInput input = new RichiestaSacerInput(RichiestaSacerInput.TipoRichiestaSacer.RECUPERO,
                        Constants.VERSIONE_XML_MM, urlRecuperoUdSacer, tmpWriter.toString(), indexWriter.toString(),
                ...
                if (esitoConnRecUD.isErroreConnessione()) {
                    ...
                    jobLoggerEjb.writeAtomicLog(Constants.NomiJob.RECUPERO_SACER, Constants.TipiRegLogJob.ERRORE,
                            errorMessage);
                    break;
                }

                if (codiceEsitoRecupero.equals(Constants.EsitoVersamento.NEGATIVO.name())) {
                    // se il risultato è stato inaspettatamente NEGATIVO registro la sessione con stato di errore e
                    // chiudo il job
                    salvaDati.elaboraErrore(sessioneRec, Constants.ServizioRecupero.RECUPERO_UD_SERVICE, null,
                            codiceErroreRecupero, messaggioErroreRecupero);

                    errorMessage = MessaggiWSBundle.getString(MessaggiWSBundle.ERR_NEGATIVO_RECUPERO_UD,
                            sessioneRec.getPigVer().getPigAmbienteVer().getNmAmbienteVers(),
                            sessioneRec.getPigVer().getNmVers(), unitaDocObj.getCdRegistroUnitaDocSacer(),
                            unitaDocObj.getAaUnitaDocSacer().toString(), unitaDocObj.getCdKeyUnitaDocSacer(),
                            codiceErroreRecupero, messaggioErroreRecupero);
                    ...
                    // In caso la risposta di SACER sia WARNING vado a gestire le sessioni successive
                    continue;
                }
                ...
                // FIXME : Verificare cosa cambia nell'xml tra RecuperoUd e RecProveConserv (se cambia, altrimenti
                // cambia solo l'url e l'xml rimane invariato)
                // String urlRecuperoPCSacer = configurationHelper.getValoreParamApplic("DS_URL_SERV_PROVE",
                ...
                // Constants.VERSIONE_XML_MM, urlRecuperoPCSacer, tmpWriter.toString(), indexWriter.toString(),
                // nmUseridSacer, cdPswSacer, timeout);
                // // Creo l'oggetto contenente i suoi parametri (TipoRichiesta = RECUPERO)
                // // Chiamata a richiesta sacer RecuperoUnitaDocumentariaSync
                ...
                // // se il risultato è stato inaspettatamente NEGATIVO registro la sessione con stato di errore e
                // // chiudo il job
                // salvaDati.elaboraErrore(sessioneRec, Constants.ServizioRecupero.RECUPERO_PC_SERVICE, unitaDocObj,
                // codiceErroreRecupero, messaggioErroreRecupero);
                //
                // errorMessage = MessaggiWSBundle.getString(MessaggiWSBundle.ERR_NEGATIVO_RECUPERO_PC,
                // sessioneRec.getPigVer().getPigAmbienteVer().getNmAmbienteVers(),
                // sessioneRec.getPigVer().getNmVers(), unitaDocObj.getCdRegistroUnitaDocSacer(),
                // unitaDocObj.getAaUnitaDocSacer().toString(), unitaDocObj.getCdKeyUnitaDocSacer(),
                // codiceErroreRecupero, messaggioErroreRecupero);
        ...
        jobLoggerEjb.writeAtomicLog(Constants.NomiJob.RECUPERO_SACER, Constants.TipiRegLogJob.FINE_SCHEDULAZIONE, null);
    }
}
