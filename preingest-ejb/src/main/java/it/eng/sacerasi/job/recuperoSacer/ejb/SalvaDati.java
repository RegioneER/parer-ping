...
public class SalvaDati {
    ...
    public void elabora(PigSessioneRecup sessioneRec, PigUnitaDocObject unitaDocObj)
            throws ParerInternalError {
        // I file da rinominare sono quelli creati da Sacer con le due chiamate di recupero eseguite
        // nel metodo sopra
        ...
        String ftpOutput = rootFtp.concat(
                sessioneRec.getPigObject().getPigTipoObject().getPigVer().getDsPathOutputFtp());
        ...
        File fileUdDest = new File(ftpOutput.concat(Constants.UD_FILE_PREFIX_PREINGEST)
                .concat(sessioneRec.getPigObject().getCdKeyObject())
        ...
        if (this.modificaSessione(sessioneRec, Constants.StatoSessioneRecup.RECUPERATO, null, null,
                null)) {
            log.info("Commit effettuato");
        } else {
            log.info("Rollback effettuato");
            throw new ParerInternalError(
                    "Errore grave nella procedura elabora in recuperoOggettiSacer");
        }
    }
    ...
    public void elaboraErrore(PigSessioneRecup sessioneRec, Constants.ServizioRecupero tipoServizio,
        ...
        if (tipoServizio == Constants.ServizioRecupero.RECUPERO_PC_SERVICE) {
            ...
            // String ftpOutput =
            // rootFtp.concat(configurationHelper.getValoreParamApplic("DS_PATH_OUTPUT_FTP",
            // idAmbienteVers, idVers, idTipoObject, Constants.TipoPigVGetValAppart.TIPOOBJECT));
            String ftpOutput = sessioneRec.getPigObject().getPigTipoObject().getPigVer()
                    .getDsPathOutputFtp();
            String fileSuffix = unitaDocObj.getCdRegistroUnitaDocSacer().concat(FILE_KEY_SEPARATOR)
                    .concat(unitaDocObj.getAaUnitaDocSacer().toString().concat(FILE_KEY_SEPARATOR))
                    .concat(unitaDocObj.getCdKeyUnitaDocSacer().concat(Constants.ZIP_EXTENSION));
        ...
        // - O il servizio è RECUPERO_PC e oltre alla modifica sessione va anche eliminato il file
        // UD creato
        // precedentemente
        if ((tipoServizio == Constants.ServizioRecupero.RECUPERO_UD_SERVICE && this
                .modificaSessione(sessioneRec, Constants.StatoSessioneRecup.CHIUSO_ERR_RECUPERATO,
                        Calendar.getInstance().getTime(), codErr, dsErr))
                || (tipoServizio == Constants.ServizioRecupero.RECUPERO_PC_SERVICE
                        && this.modificaSessione(sessioneRec,
                                Constants.StatoSessioneRecup.CHIUSO_ERR_RECUPERATO,
                                Calendar.getInstance().getTime(), codErr, dsErr)
            ...
            log.info("Rollback effettuato");
            throw new ParerInternalError(
                    "Errore grave nella procedura elaboraErrore in recuperoOggettiSacer");
        }

    }

    private boolean modificaSessione(PigSessioneRecup sessioneRec,
            Constants.StatoSessioneRecup stato, Date dtChiusura, String codErr, String dsErr) {
        boolean tmpRet;
        try {
            PigSessioneRecup tmp = entityManager.find(PigSessioneRecup.class,
                    sessioneRec.getIdSessioneRecup());
    ...
    private boolean rinominaFiles(File fileUd, File fileUdDest, File filePc, File filePcDest) {
        ...
        try {
            ...
            log.error("Interruzione durante la ridenominazione dei file ", ex);
            xadConn.close(); // Assicurati di chiudere la connessione
            log.info("close effettuato");
            // Imposta tmpRet a false per indicare il fallimento
            tmpRet = false;
    ...
    private boolean rimuoviFile(File file) {
        ...
        try {
            ...
            Thread.currentThread().interrupt(); // Ripristina lo stato di interruzione
            log.error("Interruzione durante la rimozione del file ", ex);
            xadConn.close(); // Assicurati di chiudere la connessione
            log.info("close effettuato");
            // Imposta tmpRet a false per indicare il fallimento
            tmpRet = false;
