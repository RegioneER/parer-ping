...
import it.eng.sacerasi.sisma.ejb.InvioSismaEjb;

/**
 *
 * @author Gilioli_P
 */
@Singleton(mappedName = "InvioSismaTimer")
@Lock(LockType.READ)
@LocalBean
public class InvioSismaTimer extends JobTimer {

    private static final Logger logger = LoggerFactory.getLogger(InvioSismaTimer.class);
    @EJB
    private InvioSismaTimer thisTimer;
    @EJB
    private InvioSismaEjb invioSismaEjb;

    public InvioSismaTimer() {
        super(Constants.NomiJob.INVIO_SISMA);
        logger.debug(InvioSismaTimer.class.getName() + " creato");
    }
    ...
    public void startCronScheduled(CronSchedule sched, String applicationName) {
        ...
        if (!isActive()) {
            ...
            logger.info("Lancio il timer InvioSismaTimer...");
    ...
    public void startProcess(Timer timer) {
        try {
            jobLogger.writeAtomicLog(Constants.NomiJob.INVIO_SISMA, Constants.TipiRegLogJob.INIZIO_SCHEDULAZIONE, null);
            ...
            jobLogger.writeAtomicLog(Constants.NomiJob.INVIO_SISMA, Constants.TipiRegLogJob.ERRORE, message);
            logger.error("Errore nell'esecuzione del job di invio sisma", e);
        }
    }
}
