...
import it.eng.sacerasi.job.recuperaErrori.ejb.RecuperaErroriInCodaEjb;
 ...
 * @author Bonora_L
 */
@Singleton(mappedName = "RecuperaErroriInCodaTimer")
@Lock(LockType.READ)
@LocalBean
public class RecuperaErroriInCodaTimer extends JobTimer {

    private static final Logger logger = LoggerFactory.getLogger(RecuperaErroriInCodaTimer.class);
    @EJB
    private RecuperaErroriInCodaTimer thisTimer;
    @EJB
    private RecuperaErroriInCodaEjb recuperaErroriInCodaEjb;

    public RecuperaErroriInCodaTimer() {
        super(Constants.NomiJob.RECUPERA_ERRORI_IN_CODA);
        logger.debug(RecuperaErroriInCodaTimer.class.getName() + " creato");
    }
    ...
    public void startCronScheduled(CronSchedule sched, String applicationName) {
        ...
        if (!isActive()) {
            ...
            logger.info("Lancio il timer RecuperaErroriInCodaTimer...");
    ...
    public void startProcess(Timer timer) {
        try {
            jobLogger.writeAtomicLog(Constants.NomiJob.RECUPERA_ERRORI_IN_CODA,
                    Constants.TipiRegLogJob.INIZIO_SCHEDULAZIONE, null);
            recuperaErroriInCodaEjb.recuperaErrori();
            ...
            jobLogger.writeAtomicLog(Constants.NomiJob.RECUPERA_ERRORI_IN_CODA, Constants.TipiRegLogJob.ERRORE,
                    message);
            logger.error("Errore nell'esecuzione del job di recupero errori in coda", e);
        } catch (Exception e) {
            // questo log viene scritto solo in caso di errore.
            String message = null;
            if (e.getCause() != null) {
