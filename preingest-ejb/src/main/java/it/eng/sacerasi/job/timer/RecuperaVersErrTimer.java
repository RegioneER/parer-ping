...
import it.eng.sacerasi.job.recuperaVersErr.ejb.RecuperaVersErrEjb;
 ...
 * @author Gilioli_P
 */
@Singleton(mappedName = "RecuperaVersErrTimer")
@Lock(LockType.READ)
@LocalBean
public class RecuperaVersErrTimer extends JobTimer {

    private static final Logger logger = LoggerFactory.getLogger(RecuperaVersErrTimer.class);
    @EJB
    private RecuperaVersErrTimer thisTimer;
    @EJB
    ...
    public void startProcess(Timer timer) {
        try {
            jobLogger.writeAtomicLog(Constants.NomiJob.RECUPERA_VERS_ERR, Constants.TipiRegLogJob.INIZIO_SCHEDULAZIONE,
                    null);
            ...
            jobLogger.writeAtomicLog(Constants.NomiJob.RECUPERA_VERS_ERR, Constants.TipiRegLogJob.ERRORE, message);
