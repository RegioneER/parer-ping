...
public class VerificaDocumentiSUEjb {
	...
	List<PigVSuLisDocDaVerif> documentiDaVerificare = verificaDocumentiSUHelper
		.getDocumentiDaVerificare();
	log.info("Recuperati " + documentiDaVerificare.size()
		+ " documenti strumenti urbanistici da elaborare");
	...
	for (PigVSuLisDocDaVerif documentoDaVerificare : documentiDaVerificare) {
	    try {
		PigStrumUrbDocumenti strumUrbDocumenti = genericHelper.findById(
			PigStrumUrbDocumenti.class,
		...
		if (doesObjectExist) {
			...
			} else if (VerificheDocumentiSUSismaEtc.isValidZip(tempFile,
			...
			if (errore == null) {
			    ...
			    VerificaZipFileResponse response = checkTempZipFile(
				    strumUrbDocumenti.getNmFileOrig(), tempFile, strumUrbDocumenti,
				    detectedCharset);
		...
		if (errore == null) {
		    ...
		    registraEsitoDocumentiSU(documentoDaVerificare.getPigVSuLisDocDaVerifId()
			    .getIdStrumUrbDocumenti(), "1", numFiles, null, null, null);
		} else {
		    // MEV25704 - Ho trovato degli errori, registro anche il report
		    registraEsitoDocumentiSU(
			    documentoDaVerificare.getPigVSuLisDocDaVerifId()
				    .getIdStrumUrbDocumenti(),
			    Constants.DB_FALSE, null, errore.getCdErrore(), errore.getDsErrore(),
			    report);
		}
	    } catch (Exception ex) {
	...
	log.info(
		VerificaDocumentiSUEjb.class.getSimpleName() + " --- Inizio chiamata asincrona...");
	try {
	    // Scrivo nella tabella dei log l'inizio del "job" e relativo riferimento allo strumento
	    // urbanistico
	...
	log.info(VerificaDocumentiSUEjb.class.getSimpleName()
		+ " --- Richiesta LOCK su strumento urbanistico " + idStrumentoUrbanistico);
	verificaDocumentiSUHelper.findByIdWithLock(PigStrumentiUrbanistici.class,
	...
	log.info("Recuperati " + documentiDaVerificare.size()
		+ " documenti per lo strumento urbanistico da elaborare");
	...
	PigStrumUrbDocumenti strumUrbDocumenti = verificaDocumentiSUHelper
		.findById(PigStrumUrbDocumenti.class, idStrumUrbDocumenti.longValue());
	...
	if (errore == null) {
	    ...
	    // MEV25704 - Ho trovato degli errori, registro anche il report
	    registraEsitoDocumentiSU(idStrumUrbDocumenti, Constants.DB_FALSE, null,
		    errore.getCdErrore(), errore.getDsErrore(), report);
	}
    }

    public VerificaZipFileResponse checkTempZipFile(String nomeFileZipOriginale, File file,
	    PigStrumUrbDocumenti strumUrbDocumenti, Charset zipCharset)
	...
	if (entries.hasMoreElements()) {
		...
		// MEV#25743 - Modifiche alla verifica dei file caricati - inclusione del nome del
		// file zip nel
		// controllo di lunghezza path
		// Se il nome dello zip originale + tutto il path dell'entry è superiore a 254
		// caratteri da errore.
		if (VerificheDocumentiSUSismaEtc.isLongMoreThan254Chars(nomeFileZipOriginale,
			entry.getName())) {
		...
		if (!entry.getName().matches(FILENAME_FORMAT_REGEXP)) {
		    // MEV 30808
		    int pointOfFailure = VerificheDocumentiSUSismaEtc.firstFailurePoint(
			    Pattern.compile(FILENAME_FORMAT_REGEXP), entry.getName());
	    ...
	    if (strumUrbDocumenti.getPigStrumUrbValDoc().getFlDocPrincipale().equals("1")
		    && numFiles > 1) {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    private void registraEsitoDocumentiSU(BigDecimal idStrumUrbDocumenti, String flEsitoVerifica,
	    Integer numFiles, String cdErr, String dsErr, String report) {
	// Assumo il lock sull'oggetto PIG_STRUM_URB_DOCUMENTI
	PigStrumUrbDocumenti strumUrbDocumenti = genericHelper
		.findByIdWithLock(PigStrumUrbDocumenti.class, idStrumUrbDocumenti.longValue());
    ...
    // Controlla se per lo strumento urbanistico in questione esistono ancora documenti da
    // verificare
    ...
    public boolean existsDocumentiDaVerificarePerStrumentoUrbanistico(
	    BigDecimal idStrumentoUrbanistico) {
	return verificaDocumentiSUHelper
		.existsDocumentiDaVerificarePerStrumentoUrbanistico(idStrumentoUrbanistico);
    }
    ...
    public boolean existsDocumentiDaVerificareConErrorePerStrumentoUrbanistico(
	    BigDecimal idStrumentoUrbanistico) {
	return verificaDocumentiSUHelper
    ...
    public boolean existsDocumentiDaVerificareSenzaErrorePerStrumentoUrbanistico(
	    BigDecimal idStrumentoUrbanistico) {
	return verificaDocumentiSUHelper
		.existsDocumentiDaVerificareSenzaErrorePerStrumentoUrbanistico(
			idStrumentoUrbanistico);
    }
