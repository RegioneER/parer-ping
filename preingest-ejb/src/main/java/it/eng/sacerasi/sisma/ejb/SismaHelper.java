...
public class SismaHelper extends GenericHelper {
	...
	return EnumSet.of(TiStato.BOZZA, TiStato.DA_VERIFICARE, TiStato.DA_RIVEDERE,
		TiStato.VERIFICATO, TiStato.RICHIESTA_INVIO, TiStato.INVIO_IN_CORSO, TiStato.ERRORE,
		TiStato.IN_ELABORAZIONE, TiStato.IN_TRASFORMAZIONE, TiStato.IN_VERSAMENTO,
		TiStato.IN_ELABORAZIONE_SA, TiStato.IN_TRASFORMAZIONE_SA, TiStato.IN_VERSAMENTO_SA,
		TiStato.VERSATO, TiStato.COMPLETATO, TiStato.ANNULLATO);
    }
	...
	EnumSet<TiStato> set = EnumSet.of(TiStato.BOZZA, TiStato.DA_VERIFICARE, TiStato.DA_RIVEDERE,
		TiStato.VERIFICATO, TiStato.RICHIESTA_INVIO, TiStato.INVIO_IN_CORSO, TiStato.ERRORE,
		TiStato.IN_ELABORAZIONE, TiStato.IN_TRASFORMAZIONE, TiStato.IN_VERSAMENTO,
		TiStato.IN_ELABORAZIONE_SA, TiStato.IN_TRASFORMAZIONE_SA, TiStato.IN_VERSAMENTO_SA,
		TiStato.VERSATO, TiStato.COMPLETATO, TiStato.ANNULLATO);
	...
	EnumSet<TiStato> elencoStati = EnumSet.of(TiStato.BOZZA, TiStato.DA_VERIFICARE,
		TiStato.DA_RIVEDERE, TiStato.VERIFICATO, TiStato.RICHIESTA_INVIO,
		TiStato.INVIO_IN_CORSO, TiStato.ERRORE, TiStato.IN_ELABORAZIONE,
		TiStato.IN_TRASFORMAZIONE, TiStato.IN_VERSAMENTO, TiStato.IN_ELABORAZIONE_SA,
		TiStato.IN_TRASFORMAZIONE_SA, TiStato.IN_VERSAMENTO_SA, TiStato.VERSATO,
		TiStato.COMPLETATO, TiStato.ANNULLATO);
    ...
    public Query findSismaByVersAndStates(RicercaSismaDTO rDTO, BigDecimal idVers,
	    Set<TiStato> set) {
	String queryStr = "SELECT s, s.pigVer , fin, prog, fase, atto, stato FROM PigSisma s "
		+ "JOIN s.pigSismaProgettiAg prog "
		+ "JOIN s.pigSismaProgettiAg.pigSismaFinanziamento fin "
	...
	if (rDTO.isIdLineaFin()) {
	    query.setParameter(COSTANTE_ID_SISMA_FINANZIAMENTO,
		    HibernateUtils.longFrom(rDTO.getIdLineaFin()));
	}
	...
	String queryStr = "SELECT s, s.pigVer, fin, prog, fase, atto, stato FROM PigSisma s "
		...
		+ "JOIN s.pigSismaProgettiAg.pigSismaFinanziamento fin "
		+ "JOIN s.pigSismaFaseProgetto fase " + "JOIN s.pigSismaValAtto atto "
		+ "JOIN s.pigSismaStatoProgetto stato "
		+ "WHERE s.tiStato IN :set ORDER BY s.dtCreazione DESC";
    ...
    public List<PigSismaStatoProgetto> findPigSismaStatoProgettoByIdSismaFaseProgetto(
    ...
    public List<PigSismaFaseProgetto> getFasiInseriteFinVers(BigDecimal idSismaFinanziamento,
	    BigDecimal idVers) {
	...
	query1.setParameter(COSTANTE_ID_SISMA_FINANZIAMENTO,
		HibernateUtils.longFrom(idSismaFinanziamento));
	return query1.getResultList();
    }

    /* Estrae le fasi per il tipo di finanziamento */
    public List<PigSismaFaseProgetto> findPigSismaFaseProgettoByFin(
	    BigDecimal idSismaFinanziamento) {
	String strQuery2 = "SELECT p FROM PigSismaFaseProgetto p "
		+ "WHERE p.pigSismaFinanziamento.idSismaFinanziamento=:idSismaFinanziamento "
		+ "ORDER BY p.niOrd ";
	Query query2 = getEntityManager().createQuery(strQuery2);
	query2.setParameter(COSTANTE_ID_SISMA_FINANZIAMENTO,
		HibernateUtils.longFrom(idSismaFinanziamento));
	return query2.getResultList();
    }

    public List<PigSismaProgettiAg> getPigSismaProgettiAgByIdEnteFinanziamento(
	    BigDecimal idEnteSiam, BigDecimal idSismaFinanziamento) {
    ...
    public PigSismaPianoDocReq getPigSismaPianoDocReqByValDoc(
	    PigSismaFaseProgetto pigSismaFaseProgetto, PigSismaValDoc pigSismaValDoc) {
    ...
    public PigSismaDocumenti getPigSismaDocumentiBySismaNmFileOrig(PigSisma pigSisma,
	    String nmFileOrig) {
    ...
    // MEV27950
    public List<PigSismaPianoDocReq> findPigSismaPianoDocReq(
	    PigSismaFaseProgetto pigSismaFaseProgetto) {
	...
	if (pigSisma != null) {
	    ...
	    dto.setSoggettoATutela(
		    soggettoATutela != null && soggettoATutela.equals(Constants.DB_TRUE));
	...
	String queryStr = "select su, str, en, amb, udo "
		...
		+ "and    orgIam.idOrganizIam = udo.idOrganizIam "
		+ "and    son.pigObjectPadre.idObject = po.idObject "
		+ "and    str.idStrut = orgiam.idOrganizApplic "
		+ "and    en.idEnte = str.orgEnte.idEnte "
    ...
    public DatiRecuperoDto findDatiPerRecuperoByIdSisma(BigDecimal id, boolean estraiAgenzia,
	    BigDecimal idAgenzia) {
	...
	String queryStr = "select sisma, ogg, udo, vers, orgIam, str, en, amb "
		...
		+ "and    ogg.idObject = udo.pigObject.idObject "
		+ "and    ogg.pigVer.idVers = vers.idVers " + "and    sisma.idSisma=:id "
		+ "and    udo.idOrganizIam=orgIam.idOrganizIam "
		+ "and    orgIam.idOrganizApplic=str.idStrut "
		+ "and    en.idEnte = str.orgEnte.idEnte "
	...
	if (!l.isEmpty()) {
		...
		dto.setNomeTipoRegistro(pigSisma.getPigSismaProgettiAg().getPigSismaFinanziamento()
			.getDsTipoRegistroAgenzia());
		dto.setNumero(pigSisma.getRegistroAg() + "_" + pigSisma.getNumeroAg());
		dto.setAnno(pigSisma.getAnnoAg().longValueExact());
	    } else {
		// Il rapporto di versamento è quello dell'ente perché è stato inviato all'ente
		// appunto
		dto.setNomeTipoRegistro(pigSisma.getPigSismaProgettiAg().getPigSismaFinanziamento()
			.getDsTipoRegistroSaPubblico());
	...
	Query query = getEntityManager()
		.createQuery("SELECT s FROM PigSisma s WHERE s.idSisma = :idSisma "
    ...
    public List<PigSisma> findPigSismaByVersAndDatiAgenzia(BigDecimal idSisma, PigVers agenzia,
	...
	for (TiStato stato : stati) {
	    // MAC 27254 - Agenzia non può cercare i sisma in stato bozza. Non lo inseriamo nella
	    // decode map
    ...
    // MEV 30936
    public void creaStatoStorico(PigSisma pigSisma, String stato, Date dtRegStato,
	    String descrizione) {
	...
	Query query = getEntityManager().createQuery(
		"SELECT psud FROM PigSismaDocumenti psud WHERE psud.pigSisma.idSisma = :idSisma AND"
			+ " psud.nmFileOrig = :nomefile AND psud.flDeleted = '0'");
	query.setParameter("idSisma", HibernateUtils.longFrom(idSisma));
