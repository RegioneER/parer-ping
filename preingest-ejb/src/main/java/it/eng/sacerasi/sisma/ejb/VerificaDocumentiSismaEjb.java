...
public class VerificaDocumentiSismaEjb {
	...
	try {
	    ...
	    sessionContext.getBusinessObject(VerificaDocumentiSismaEjb.class)
		    .verificaDocumentiAsync(idSisma);
	} catch (Exception e) {
	    // INUTILI in quanto intercettati
	}
	log.info(VerificaDocumentiSismaEjb.class.getSimpleName()
		+ " --- Fine chiamata asincrona...");
    }

    @Asynchronous
    public void verificaDocumentiAsync(BigDecimal idSisma) throws IOException {
	// Locko sisma che sto considerando
	log.info(VerificaDocumentiSismaEjb.class.getSimpleName() + " --- Richiesta LOCK su sisma "
		+ idSisma);
	...
	List<PigSismaDocumenti> documentiDaVerificare = verificaDocumentiSismaHelper
		.getDocumentiDaVerificare(idSisma);
	log.info(
		"Recuperati " + documentiDaVerificare.size() + " documenti per sisma da elaborare");
	...
	for (PigSismaDocumenti documentoDaVerificare : documentiDaVerificare) {
		...
		registraEsitoDocumentiSisma(
			new BigDecimal(documentoDaVerificare.getIdSismaDocumenti()),
	...
	log.info(VerificaDocumentiSismaEjb.class.getSimpleName()
		+ " --- FINE chiamata per verifica documenti sisma");
    }
	...
	PigSismaDocumenti sismaDocumenti = verificaDocumentiSismaHelper
		.findById(PigSismaDocumenti.class, idSismaDocumenti.longValue());
	...
	if (doesObjectExist) {
		...
		String dsPathInputFtp = sismaDocumenti.getPigSisma().getPigVer()
			.getDsPathInputFtp();
		...
		if (errore == null) {
		    ...
		    VerificaZipFileResponse response = checkTempZipFile(
			    sismaDocumenti.getNmFileOrig(), tempFile, sismaDocumenti,
			    detectedCharset);
	...
	if (errore == null) {
	    ...
	    registraEsitoDocumentiSisma(idSismaDocumenti, Constants.DB_TRUE, numFiles, null, null,
		    null);
	} else {
	    // MEV26267 - Ho trovato degli errori, registro anche il report
	    registraEsitoDocumentiSisma(idSismaDocumenti, Constants.DB_FALSE, null,
	...
	if (entries.hasMoreElements()) {
	    ...
	    if (sismaDocumenti.getPigSismaValDoc().getFlDocPrincipale().equals("1")
		    && numFiles > 1) {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    private void registraEsitoDocumentiSisma(BigDecimal idSismaDocumenti, String flEsitoVerifica,
