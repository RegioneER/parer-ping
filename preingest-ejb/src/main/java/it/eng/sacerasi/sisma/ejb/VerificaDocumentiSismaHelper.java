...
public class VerificaDocumentiSismaHelper extends GenericHelper {
    ...
    public boolean isVerificaTerminata(BigDecimal idSisma) {
        Query q = entityManager.createQuery("SELECT COUNT(logJob) FROM PigLogJob logJob WHERE "
                + "(logJob.idRecord = :idSisma " + "AND logJob.tiRegLogJob = 'INIZIO_SCHEDULAZIONE')"
                + "AND (logJob.idRecord = :idSisma " + "AND logJob.tiRegLogJob != 'FINE_SCHEDULAZIONE') ");
    ...
    public boolean verificaInCorso(BigDecimal idSisma) {
        ...
        Query qFine = entityManager.createQuery("SELECT COUNT(logJob) FROM PigLogJob logJob WHERE "
                + "logJob.idRecord = :idSisma " + "AND logJob.tiRegLogJob IN ('FINE_SCHEDULAZIONE', 'ERRORE') ");
    ...
    public boolean existsDocumentiDaVerificareSenzaErrorePerSisma(BigDecimal idSisma) {
        Query q = entityManager.createQuery("SELECT COUNT(lisDocDaVerif) FROM PigSismaDocumenti lisDocDaVerif "
                + "WHERE lisDocDaVerif.pigSisma.idSisma = :idSisma " + "AND lisDocDaVerif.flEsitoVerifica = '0' "
    ...
    public boolean existsDocumentiVerificatiConErrorePerSisma(BigDecimal idSisma) {
        Query q = entityManager.createQuery("SELECT COUNT(lisDocDaVerif) FROM PigSismaDocumenti lisDocDaVerif "
                + "WHERE lisDocDaVerif.pigSisma.idSisma = :idSisma " + "AND lisDocDaVerif.cdErr IS NOT NULL ");
        q.setParameter("idSisma", HibernateUtils.longFrom(idSisma));
        return ((Long) q.getSingleResult()) > 0;
    }

    public List<String> getDocumentiVerificatiConErrorePerSisma(BigDecimal idSisma) {
        Query q = entityManager.createQuery("SELECT lisDocDaVerif.nmFileOrig FROM PigSismaDocumenti lisDocDaVerif "
                + "WHERE lisDocDaVerif.pigSisma.idSisma = :idSisma " + "AND lisDocDaVerif.cdErr IS NOT NULL "
