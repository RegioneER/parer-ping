...
public class StrumentiUrbanisticiEjb {
	...
	if (sus != null && !sus.isEmpty()) {
		...
		riga.setBigDecimal("anno",
			new BigDecimal(pigStrumentiUrbanistici.getAnno().longValueExact()));
		...
		riga.setBigDecimal("dimensione", strumentiUrbanisticiHelper
    ...
    public List<DocStrumDto> findDocumentiCaricatiPerIdSUDto(
	    PigStrumentiUrbanistici pigStrumentiUrbanistici) {
	ArrayList<DocStrumDto> al = new ArrayList<>();
	PigStrumUrbPianoStato pigStrumUrbPianoStato = pigStrumentiUrbanistici
		.getPigStrumUrbPianoStato();
	...
	for (PigStrumUrbDocumenti pigStrumUrbDocumenti : listaDoc) {
		...
		dto.setObbligatorio(
			pigStrumUrbPianoDocReq.getFlDocObbligatorio().equals(Constants.DB_TRUE));
		dto.setCdErr(pigStrumUrbDocumenti.getCdErr());
		dto.setDsErr(pigStrumUrbDocumenti.getDsErr());
		dto.setFlEsitoVerifica(
			pigStrumUrbDocumenti.getFlEsitoVerifica().equals(Constants.DB_TRUE));
	...
	PigStrumUrbPianoStato pigStrumUrbPianoStato = pigStrumentiUrbanistici
		.getPigStrumUrbPianoStato();
	List<DocUploadDto> listaUpload = findPigVSuLisDocsPianoByTipoStrumentoFase(
		pigStrumentiUrbanistici, pigStrumUrbPianoStato.getNmTipoStrumentoUrbanistico(),
		pigStrumUrbPianoStato.getTiFaseStrumento());
	...
	for (DocUploadDto docUploadDto : listaUpload) {
		...
		// MEV25704 - se è presente un report della verifica fatta sui file, permettine il
		// download
	...
	List<PigStrumUrbPianoStato> valori = strumentiUrbanisticiHelper
		.findPigStrumUrbPianoStatoByNomeTipo(nomeTipo);
	...
	PigStrumUrbPianoStato p = strumentiUrbanisticiHelper
		.getPigStrumUrbPianoStatoByNomeTipoByTipoAndFase(nmTipoStrumentoUrbanistico,
			tiFaseStrumento);
	if (p != null) {
	...
	PigStrumentiUrbanistici su = strumentiUrbanisticiHelper
		.findById(PigStrumentiUrbanistici.class, id);
	...
	PigStrumUrbPianoStato pigStrumUrbPianoStato = strumentiUrbanisticiHelper
		.getPigStrumUrbPianoStatoByNomeTipoByTipoAndFase(
			dto.getNmTipoStrumentoUrbanistico(), dto.getTiFaseStrumento());
	...
	PigStrumentiUrbanistici esisteAltroSu = strumentiUrbanisticiHelper
		.getSUByVersAndCdKey(pigVers, pigStrumentiUrbanistici.getCdKey());
	if (esisteAltroSu != null) {
	    dto.addWarnMessage(StringUtils.replace(
		    messaggiHelper.retrievePigErrore("PING-ERRSU20").getDsErrore(), "{0}",
		    esisteAltroSu.getCdKey()));
	} else {
	    ...
	    pigStrumentiUrbanistici
		    .setCdKeyOs(String.format("%s/%s/%s", pigAmbienteVers.getNmAmbienteVers(),
			    pigVers.getNmVers(), pigStrumentiUrbanistici.getCdKey()));
	    pigStrumentiUrbanistici
		    .setOggetto(String.format("Strumento urbanistico %s - %s di %s del %s",
			    dto.getNmTipoStrumentoUrbanistico(), pigStrumentiUrbanistici.getCdKey(),
			    pigVers.getNmVers(),
	...
	String newCdKey = String.format("%d_%s_%s", dto.getAnno().longValueExact(),
		dto.getTiAtto().getCdNome(), Utils.normalizzaNomeFile(dto.getNumero()));
	...
	if (!oldCdKey.equals(newCdKey)
		&& strumentiUrbanisticiHelper.getSUByVersAndCdKey(pigVers, newCdKey) != null) {
	    dto.addWarnMessage(messaggiHelper.retrievePigErrore("PING-ERRSU20").getDsErrore());
	}

	if (!dto.existsMessages()) {
	    // MEV 27352 - cancella i documenti se tipo progetto o fase progetto sono stati
	    // modificati
	    ...
	    pigStrumentiUrbanistici.setPigStrumUrbPianoStato(pigStrumUrbPianoStato);
	    if (pigStrumentiUrbanistici.getTiStato()
		    .equals(PigStrumentiUrbanistici.TiStato.ERRORE)) {
		// MEV 31096
	    ...
	    List<PigStrumUrbCollegamenti> cols = pigStrumentiUrbanistici
		    .getPigStrumUrbCollegamentis();
	...
	SIOrgEnteSiam sIOrgEnteSiam = strumentiUrbanisticiHelper.findById(SIOrgEnteSiam.class,
		idEnteSiam);
	return String.format("%s - %s %s del %s", sIOrgEnteSiam.getNmEnteSiam(),
		nomeStrumentoUrbanistico, pigStrumentiUrbanistici.getCdKey(),
		DateUtil.formatDateWithSlash(pigStrumentiUrbanistici.getData()));
    }
	...
	if (dto.getAnnoCollegato1() != null) {
	    ...
	    PigStrumUrbPianoStato piano = strumentiUrbanisticiHelper
		    .getPigStrumUrbPianoStatoByNomeTipoByTipoAndFase(
			    pigStrumUrbPianoStato.getNmTipoStrumentoUrbanistico(),
	...
	for (PigStrumUrbDocumenti doc : docs) {
	    strumentiUrbanisticiHelper.findByIdWithLock(PigStrumUrbDocumenti.class,
		    doc.getIdStrumUrbDocumenti());
	}
	PigVSuCheck pigVSuCheck = strumentiUrbanisticiHelper
		.getDatiNavigazionePerSU(idStrumentoUrbanistico);
	if (pigVSuCheck.getFlFileMancante().equals(Constants.DB_FALSE)
		...
		pigStrumentiUrbanistici.setIamUser(
			strumentiUrbanisticiHelper.findById(IamUser.class, idUserIamCorrente));
	    }
	    retOk = true;
	}
	...
	List<PigStrumentiUrbanistici> strumenti = strumentiUrbanisticiHelper
		.findNumeriByVersAnnoTipoSUFase(pigVers, anno, nmTipoStrumento, fase);
	...
	if (pigStrumentiUrbanistici.getTiStato().equals(PigStrumentiUrbanistici.TiStato.BOZZA)
		|| pigStrumentiUrbanistici.getTiStato()
			.equals(PigStrumentiUrbanistici.TiStato.ERRORE)) {
	    ...
	    pigStrumUrbDocumenti = strumentiUrbanisticiHelper.findByIdWithLock(
    ...
    public List<DocUploadDto> findPigVSuLisDocsPianoByTipoStrumentoFase(
	    PigStrumentiUrbanistici pigStrumentiUrbanistici, String nmTipoStrumento,
	    String tiFaseStrumento) {
	List<PigStrumUrbDocumenti> listaDoc = pigStrumentiUrbanistici.getPigStrumUrbDocumentis();
	...
	for (PigVSuLisDocsPiano pigVSuLisDocsPiano : l) {
	    ...
	    dto.setNmTipoDocumento(
		    pigVSuLisDocsPiano.getPigVSuLisDocsPianoId().getNmTipoDocumento());
	    dto.setObbligatorio(
		    pigVSuLisDocsPiano.getFlDocObbligatorio().equals(Constants.DB_TRUE));
	    dto.setPrincipale(pigVSuLisDocsPiano.getFlDocPrincipale().equals(Constants.DB_TRUE));
	    al.add(dto);
	    // determina se esiste un doc già inserito
	    for (PigStrumUrbDocumenti pigStrumUrbDocumenti : listaDoc) {
		if (pigStrumUrbDocumenti.getPigStrumUrbValDoc().getNmTipoDocumento()
		    ...
		    dto.setDataDoc(DateUtil
			    .formatDateWithSlashAndTime(pigStrumUrbDocumenti.getDtCaricamento()));
		    if (pigStrumUrbDocumenti.getFlEsitoVerifica() != null) {
			dto.setFlEsitoVerifica(
				(pigStrumUrbDocumenti.getFlEsitoVerifica().equals("S")));
		    }
		    break;
		}
	    }
    ...
    public List<DocUploadDto> findPigVSuLisDocsPianoByTipoStrumentoFase(
	    BigDecimal idStrumentiUrbanistici, String nmTipoStrumento, String tiFaseStrumento) {
	PigStrumentiUrbanistici pigStrumentiUrbanistici = strumentiUrbanisticiHelper
		.findById(PigStrumentiUrbanistici.class, idStrumentiUrbanistici);
	return findPigVSuLisDocsPianoByTipoStrumentoFase(pigStrumentiUrbanistici, nmTipoStrumento,
		tiFaseStrumento);
    }
	...
	PigStrumentiUrbanistici pigStrumentiUrbanistici = strumentiUrbanisticiHelper
		.findById(PigStrumentiUrbanistici.class, idStrumentiUrbanistici);
	return pigStrumentiUrbanistici.getCdKeyOs() + "_"
		+ Utils.eliminaPunteggiatureSpaziNomeFile(nomeFileOriginale);
    }
	...
	psuds.setNmTenant(configurationHelper.getValoreParamApplicByApplic(
    ...
    // MEV31096
    public PigStrumUrbStoricoStatiTableBean getPigStrumUrbStoricoStatiFromPigObjectTableBean(
	...
	return (strumentiUrbanisticiHelper.findById(PigStrumentiUrbanistici.class,
		idStrumentoUrbanistico)).getOggetto();
    }

    public boolean existsCondizioniInvio(BigDecimal idStrumentoUrbanistico) {
	PigVSuCheck check = strumentiUrbanisticiHelper.findViewById(PigVSuCheck.class,
