...
public class AmministrazioneHelper extends GenericHelper {
     ...
     * AMBIENTI
     */
    public List<PigAmbienteVers> getPigAmbienteVersAbilitatiList(String nmAmbienteVers,
	    Long idUtente, String nmApplic) {

	StringBuilder queryStr = new StringBuilder("SELECT DISTINCT vers.pigAmbienteVer "
		+ "FROM UsrVAbilOrganiz abilOrganiz, PigVers vers "
		+ "WHERE abilOrganiz.idOrganizApplic = vers.idVers "
		+ "AND abilOrganiz.id.idUserIam = :idUtente "
		+ "AND abilOrganiz.nmApplic = :nmApplic ");

	if (nmAmbienteVers != null) {
	    queryStr.append(
		    "AND UPPER(vers.pigAmbienteVer.nmAmbienteVers) LIKE :ambiente ESCAPE :char ");
	}
    ...
    public List<PigAmbienteVers> getPigAmbienteVersList(String nmAmbienteVers, Long idUtente,
	    String nmApplic) {
	StringBuilder queryStr = new StringBuilder("SELECT DISTINCT vers.pigAmbienteVer "
		+ "FROM UsrVAbilOrganiz abilOrganiz, PigVers vers "
		+ "WHERE abilOrganiz.idOrganizApplic = vers.idVers "
	...
	queryStr = new StringBuilder(
		"SELECT DISTINCT amb FROM PigAmbienteVers amb WHERE amb.pigVers IS EMPTY");
	...
	String queryStr = "SELECT organizNolastLiv FROM UsrVAbilOrganizNolastLiv organizNoLastliv "
		...
		+ "AND organizNoLastliv.nmTipoOrganiz = 'AMBIENTE' "
	...
	if (idAmbienteVers != null) {
	    queryStr.append(whereWord)
		    .append("vers.pigAmbienteVer.idAmbienteVers = :idAmbienteVers ");
	    whereWord = " AND ";
	}
	queryStr.append(whereWord)
		.append(":dtOdierna BETWEEN vers.dtIniValVers AND vers.dtFineValVers ");
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT vers " + "FROM IamAbilOrganiz iao, PigVers vers "
			+ "WHERE iao.idOrganizApplic = vers.idVers "
			+ "AND iao.iamUser.idUserIam = :idUserIam ");
	...
	if (vers.getPigAmbienteVer() != null
		&& vers.getPigAmbienteVer().getNmAmbienteVers() != null) {
	    queryStr.append(whereWord);
	...
	if (vers.getPigAmbienteVer() != null
		&& vers.getPigAmbienteVer().getNmAmbienteVers() != null) {
	    query.setParameter("nmAmbienteVers",
		    vers.getPigAmbienteVer().getNmAmbienteVers().toUpperCase());
	}
    ...
    public List<PigVRicVers> getPigVRicVersList(Long idVers, String nmVers, String nmAmbienteVers,
	    ...
	    String nmAmbienteEnteConvenz, String nmEnteConvenz, Long idUserIam,
	    String nmTipoVersatore) {
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT DISTINCT new it.eng.sacerasi.viewEntity.PigVRicVers(ricVers.idAmbienteVers, "
			...
			+ "WHERE iao.idOrganizApplic = ricVers.id.idVers "
			+ "AND iao.iamUser.idUserIam = :idUserIam ");
	...
	String queryStr = "SELECT DISTINCT b.nmAmbienteEnteConvenz FROM SIOrgEnteSiam a JOIN SIOrgAmbienteEnteConvenz b ON (a.siOrgAmbienteEnteConvenz.idAmbienteEnteConvenz = b.idAmbienteEnteConvenz)"
		...
		+ " WHERE iao.idOrganizApplic = vers.idVers"
		+ " AND iao.iamUser.idUserIam = :idUserIam)" + " ORDER BY b.nmAmbienteEnteConvenz";
	...
	StringBuilder queryStr = new StringBuilder("SELECT DISTINCT ricVers.nmEnteConvenz "
		+ "FROM IamAbilOrganiz iao, PigVRicVers ricVers "
		+ "WHERE iao.idOrganizApplic = ricVers.id.idVers "
		+ "AND iao.iamUser.idUserIam = :idUserIam "
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT vers  FROM PigVers vers  WHERE vers.nmVers= :nmVers ");
	if (idAmb != null) {
	...
	Query query = getEntityManager()
		.createQuery("SELECT vers FROM PigVers vers  WHERE vers.idVers= :idVers");
    ...
    // MEV25814
    public boolean existsPigTipoFileObjectByTipoObjectAndName(BigDecimal idTipoObj,
	    String pigTipoFileName) {
	StringBuilder queryStr = new StringBuilder(
		"SELECT tipoFileObj FROM PigTipoFileObject tipoFileObj ");
	queryStr.append(
     ...
     * @deprecated
     *
     * @param idVersDaTrasf versatore da
     * @param idVersTrasf   versatore
     *
    ...
    /**
     * Ritorna la lista dei versatori per cui generare oggetti, escludendo il versatore del
	...
	String queryStr = "SELECT organiz FROM UsrVAbilOrganiz organiz "
		+ "WHERE organiz.idOrganizApplic = :idVers "
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT tipoObject FROM PigTipoObject tipoObject ");
	if (idVers != null) {
    ...
    public List<PigTipoObject> getPigTipoObjectNoDaTrasfAbilitatiList(BigDecimal idVers,
	    long idUtente) {
    ...
    public PigDichVersSacerTipoObj getPigDichVersSacerTipoObj(BigDecimal idTipoObject,
	...
	try {
	    Query query = getEntityManager().createNamedQuery("PigObject.findByIdTipoOggetto",
		    PigObject.class);
	    ...
	    log.error("Errore nell'estrazione di PigObject per idTipoOggetto [{" + idTipoOggetto
		    + "}]", ex);
	    throw ex;
	}
	return esiste;
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT tipoFileObject FROM PigTipoFileObject tipoFileObject ");
    ...
    public PigTipoFileObject getPigTipoFileObjectByName(String nmTipoFileObj,
     ...
     * SOP CLASS
     */
    public List<PigSopClassDicom> getPigSopClassDicomList(String cdSopClassDicom,
	    String dsSopClassDicom) {
	StringBuilder queryStr = new StringBuilder(
		"SELECT sopClass FROM PigSopClassDicom sopClass ");
	String whereWord = "WHERE ";
     ...
     * SOPCLASSDICOMVERS
     */
    public List<PigSopClassDicomVers> getPigSopClassDicomVersList(BigDecimal idSopClassDicom,
	    BigDecimal idVers) {
	StringBuilder queryStr = new StringBuilder(
	...
	if (idSopClassDicom != null) {
	    queryStr.append(whereWord);
	    queryStr.append(
		    " sopClassDicomVers.pigSopClassDicom.idSopClassDicom = :idSopClassDicom");
	    whereWord = " AND ";
	}
	if (idVers != null) {
	    queryStr.append(whereWord);
    ...
    public PigSopClassDicomVers getPigSopClassDicomVers(BigDecimal idSopClassDicom,
	    BigDecimal idVers) {
	StringBuilder queryStr = new StringBuilder(
		"SELECT sopClassVers FROM PigSopClassDicomVers sopClassVers ");
    ...
    public List<PigXsdDatiSpec> getPigXsdDatiSpecList(BigDecimal idTipoObj,
	    BigDecimal idTipoFileObj) {
	StringBuilder queryStr = new StringBuilder(
		"SELECT xsdDatiSpec FROM PigXsdDatiSpec xsdDatiSpec ");
	if (idTipoObj != null) {
	    queryStr.append(" WHERE xsdDatiSpec.pigTipoObject.idTipoObject = :idTipoObj");
	} else if (idTipoFileObj != null) {
	    queryStr.append(
		    " WHERE xsdDatiSpec.pigTipoFileObject.idTipoFileObject = :idTipoFileObj");
	}
	Query query = getEntityManager().createQuery(queryStr.toString());
	if (idTipoObj != null) {
	    query.setParameter("idTipoObj", HibernateUtils.longFrom(idTipoObj));
    ...
    public List<PigXsdDatiSpec> getOrdPigXsdDatiSpecList(BigDecimal idTipoObj,
	    BigDecimal idTipoFileObj) {
	StringBuilder queryStr = new StringBuilder(
		"SELECT xsdDatiSpec FROM PigXsdDatiSpec xsdDatiSpec ");
	if (idTipoObj != null) {
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT attribDatiSpec FROM PigAttribDatiSpec attribDatiSpec ");
	...
	if (idTipoObj != null) {
	    PigTipoObject tipoObj = getEntityManager().find(PigTipoObject.class,
		    idTipoObj.longValue());
	    // Controllo se devo eliminare le corrispondenze o i versatori a seconda della modifica
	    // del tipo versamento
	    // file
	    if (tipoObj.getTiVersFile()
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT dichVersSacerTipoObj, treeOrganizIam.dlCompositoOrganiz "
     ...
     */
    public List<PigVersTipoObjectDaTrasf> retrievePigVersTipoObjectDaTrasfList(
	    BigDecimal idTipoObject) {
	StringBuilder queryStr = new StringBuilder(
		"SELECT versTipoObjectDaTrasf FROM PigVersTipoObjectDaTrasf versTipoObjectDaTrasf ");
	if (idTipoObject != null) {
	    queryStr.append(
		    "WHERE versTipoObjectDaTrasf.pigTipoObjectDaTrasf.idTipoObject = :idTipoObject");
	}
    ...
    public PigVersTipoObjectDaTrasf getPigVersTipoObjectDaTrasf(BigDecimal idTipoObjectDaTrasf,
	    String cdVersGen, BigDecimal idVersGen) {
	StringBuilder queryStr = new StringBuilder(
	...
	String queryStr = "SELECT COUNT(obj) FROM PigObject obj "
		+ "WHERE obj.pigTipoObject.idTipoObject = :idTipoObject "
		+ "AND obj.pigVer.idVers = :idVers ";
	...
	Query query = getEntityManager().createQuery(
		"SELECT COUNT(objs) FROM PigVersTipoObjectDaTrasf versTipoObjectDaTrasf "
     ...
     */
    public List<PigVValoreSetParamTrasf> getPigVValoreSetParamTrasfList(
	...
	if (idVersTipoObjectDaTrasf != null) {
	    queryStr.append(
		    "WHERE valoreSetParamTrasf.id.idVersTipoObjectDaTrasf = :idVersTipoObjectDaTrasf");
	}
	Query query = getEntityManager().createQuery(queryStr.toString());
	if (idVersTipoObjectDaTrasf != null) {
    ...
    public List<PigVValoreSetParamTrasf> getPigVValoreSetParamTrasfListByIdSetParamTrasf(
    ...
    public List<PigVValParamTrasfDefSpec> getPigVValParamTrasfDefSpecList(
    ...
    public String getPigVValParamTrasfDefSpecTypeByName(BigDecimal idSetParamTrasf,
	...
	Query q = getEntityManager()
		.createQuery("SELECT trasf FROM XfoTrasf trasf ORDER BY trasf.cdTrasf");
    ...
    public UsrVAbilStrutSacerXping getAmbienteFromUsrVAbilStrutSacerXping(long idUserIam,
	    String nmAmbiente) {
	TypedQuery<UsrVAbilStrutSacerXping> q = getEntityManager().createQuery(
    ...
    public List<UsrVAbilStrutSacerXping> getEntiFromUsrVAbilStrutSacerXping(long idUserIam,
	...
	String query = "SELECT DISTINCT new it.eng.sacerasi.viewEntity.UsrVAbilStrutSacerXping(organiz.id.idUserIam, 'ENTE', organiz.idEnte, organiz.nmEnte) FROM UsrVAbilStrutSacerXping organiz WHERE organiz.id.idUserIam = :idUserIam "
		+ (idAmbiente != null ? "AND organiz.idAmbiente = :idAmbiente" : "")
		+ " ORDER BY organiz.nmEnte";
	TypedQuery<UsrVAbilStrutSacerXping> q = getEntityManager().createQuery(query,
		UsrVAbilStrutSacerXping.class);
	q.setParameter("idUserIam", HibernateUtils.bigDecimalFrom(idUserIam));
	if (idAmbiente != null) {
    ...
    public List<UsrVAbilStrutSacerXping> getStruttureFromUsrVAbilStrutSacerXping(long idUserIam,
	    BigDecimal idEnte) {
	String query = "SELECT DISTINCT new it.eng.sacerasi.viewEntity.UsrVAbilStrutSacerXping(organiz.id.idUserIam, 'STRUTTURA', organiz.id.idStrut, organiz.nmStrut) FROM UsrVAbilStrutSacerXping organiz WHERE organiz.id.idUserIam = :idUserIam "
		+ (idEnte != null ? "AND organiz.idEnte = :idEnte" : "")
    ...
    public List<UsrVAbilStrutSacerXping> getStruttureFromUsrVAbilStrutSacerXping(long idUserIam,
	    String nmAmbiente, String nmEnte) {
	String query = "SELECT DISTINCT new it.eng.sacerasi.viewEntity.UsrVAbilStrutSacerXping(organiz.id.idUserIam, 'STRUTTURA', organiz.id.idStrut, organiz.nmStrut) FROM UsrVAbilStrutSacerXping organiz WHERE organiz.id.idUserIam = :idUserIam "
		+ (StringUtils.isNotBlank(nmAmbiente) ? "AND organiz.nmAmbiente = :nmAmbiente "
			: "")
		+ (StringUtils.isNotBlank(nmEnte) ? "AND organiz.nmEnte = :nmEnte" : "")
		+ " ORDER BY organiz.nmStrut";
	TypedQuery<UsrVAbilStrutSacerXping> q = getEntityManager().createQuery(query,
		UsrVAbilStrutSacerXping.class);
	q.setParameter("idUserIam", HibernateUtils.bigDecimalFrom(idUserIam));
	...
	Query query = getEntityManager()
		.createQuery("SELECT DISTINCT(strutVersSacer.nmUseridSacer) "
			+ "FROM PigVLisStrutVersSacer strutVersSacer "
			+ "ORDER BY strutVersSacer.nmUseridSacer ");
	return query.getResultList();
    }

    public List<UsrVAbilStrutSacerXping> getOrganizIamFromUsrVAbilStrutSacerXping(long idUserIam,
	    BigDecimal idEnte) {
	String query = "SELECT DISTINCT organiz FROM UsrVAbilStrutSacerXping organiz "
		+ "WHERE organiz.id.idUserIam = :idUserIam "
		+ (idEnte != null ? "AND organiz.idEnte = :idEnte" : "") + " "
		+ "ORDER BY organiz.nmStrut";
	TypedQuery<UsrVAbilStrutSacerXping> q = getEntityManager().createQuery(query,
		UsrVAbilStrutSacerXping.class);
	...
	TypedQuery<UsrVAbilStrutSacerXping> q = getEntityManager().createQuery("SELECT DISTINCT "
		...
		+ "FROM UsrVAbilStrutSacerXping organiz "
		+ "WHERE organiz.id.idUserIam = :idUserIam " + "ORDER BY organiz.nmAmbiente ",
		UsrVAbilStrutSacerXping.class);
	q.setParameter("idUserIam", HibernateUtils.bigDecimalFrom(idUserIam));
	return q.getResultList();
    }
	...
	TypedQuery<UsrVAbilStrutSacerXping> q = getEntityManager().createQuery("SELECT DISTINCT "
		...
		+ "FROM UsrVAbilStrutSacerXping organiz "
		+ "WHERE organiz.id.idUserIam = :idUserIam ", UsrVAbilStrutSacerXping.class);
	q.setParameter("idUserIam", HibernateUtils.bigDecimalFrom(idUserIam));
	return q.getResultList().stream()
		.sorted(Comparator.comparing(UsrVAbilStrutSacerXping::getDlCompositoOrganiz))
    ...
    public boolean existPigValoreSetParamTrasf(BigDecimal idVersTipoObjectDaTrasf,
	    BigDecimal idSetParamTrasf) {
	Query query = getEntityManager()
		.createQuery("SELECT COUNT(p) FROM PigValoreSetParamTrasf p "
    ...
    public boolean existPigValoreParamTrasf(BigDecimal idValoreSetParamTrasf,
	    BigDecimal idParamTrasf) {
    ...
    public boolean existPigVValParamTrasfDefSpec(BigDecimal idValoreSetParamTrasf,
	    String nmParamTrasf) {
	Query query = getEntityManager()
		.createQuery("SELECT COUNT(p) FROM PigVValParamTrasfDefSpec p "
			+ "WHERE p.idSetParamTrasf = :idValoreSetParamTrasf "
			+ "AND p.nmParamTrasf = :nmParamTrasf");
     ...
     * @param cdKeyObjectTrasf chiave oggetto
     *
     ...
     *
     * @param idObject       id oggetto
     * @param pgOggettoTrasf progressivo
     *
     * @return entity {@link PigObjectTrasf}
     */
	...
	Query query = getEntityManager()
		.createQuery("SELECT COUNT(dichVersSacer) " + "FROM PigDichVersSacer dichVersSacer "
			+ "WHERE dichVersSacer.pigVer.idVers = :idVers "
	...
	Query query = getEntityManager().createQuery("SELECT COUNT(dichVersSacerTipoObj) "
		+ "FROM PigDichVersSacerTipoObj dichVersSacerTipoObj "
     ...
     *
     * @param tiParamApplic       tipo parametro
	...
	Query q = getEntityManager()
		.createQuery("SELECT paramApplic FROM PigParamApplic paramApplic "
	...
	Query q = getEntityManager()
		.createQuery("SELECT valoreParamApplic FROM PigValoreParamApplic valoreParamApplic "
	...
	if (idAmbienteVers != null) {
	    queryStr.append(
		    "AND valoreParamApplic.pigAmbienteVer.idAmbienteVers = :idAmbienteVers ");
	}
	if (idVers != null) {
	...
	Query query = getEntityManager()
		.createQuery("SELECT accordoEnte FROM SIOrgAccordoEnte accordoEnte "
    ...
    /**
     * Controlla che nel periodo compreso tra data di inizio e di fine validità l'ente convenzionato
     * associato al versatore abbia un accordo valido (compreso tra data di inizio e data di fine
     * validità)
     *
     ...
     */
    public boolean checkEsistenzaPeriodoValiditaAssociazioneEnteConvenzVers(
    ...
    public boolean existsPeriodoValiditaAssociazioneEnteConvenzVersAccordoValido(
	    BigDecimal idEnteConvenz, Date dtIniVal, Date dtFineVal) {
	Query query = getEntityManager().createQuery("SELECT accordo FROM SIOrgAccordoEnte accordo "
		+ "JOIN  accordo.siOrgEnteConvenz enteSiam "
		+ "WHERE enteSiam.idEnteSiam = :idEnteConvenz "
    ...
    /**
     * Controlla che nel periodo compreso tra data di inizio e di fine validità il versatore non sia
     * già associato ad un altro ente convenzionato ricercando sulla tabella dell'associazione
     * ovvero SIOrgEnteConvenzOrg
     *
     ...
     */
    public boolean checkEsistenzaAssociazioneEnteConvenzVers(String nmApplic, BigDecimal idVers,
	    Date dtIniVal, Date dtFineVal, BigDecimal idEnteConvenzOrg) {
	String queryStr = "SELECT enteConvenzOrg FROM SIOrgEnteConvenzOrg enteConvenzOrg "
		...
		// Oppure ancora entrambe le date sono esterne all'intervallo ma si sovrappongono ad
		// un altro periodo
	...
	if (idEnteConvenzOrg != null) {
	    query.setParameter("idEnteConvenzOrgDaEscludere",
		    HibernateUtils.longFrom(idEnteConvenzOrg));
	}
	List<SIOrgEnteConvenzOrg> list = query.getResultList();
	return !list.isEmpty();
    }

    public SIOrgEnteConvenzOrg getSIOrgEnteConvenzOrg(BigDecimal idVers, BigDecimal idEnteConvenz,
	    Date dtIniVal) {
	Query query = getEntityManager()
		.createQuery("SELECT enteOrg FROM SIOrgEnteConvenzOrg enteOrg "
			+ "WHERE enteOrg.siUsrOrganizIam.idOrganizApplic = :idVers "
	...
	if (esitoServizio != null) {
	    ...
	    case OK:
		enteSiamDaAllinea.setTiStatoAllinea(
			CostantiAllineaEntiConv.TiStatoAllinea.ALLINEA_OK.name());
		...
		case CostantiAllineaEntiConv.ERR_666:
		    enteSiamDaAllinea.setTiStatoAllinea(
			    CostantiAllineaEntiConv.TiStatoAllinea.ALLINEA_IN_ERRORE.name());
	    ...
	    case NO_RISPOSTA:
		enteSiamDaAllinea.setTiStatoAllinea(
			CostantiAllineaEntiConv.TiStatoAllinea.ALLINEA_IN_TIMEOUT.name());
		enteSiamDaAllinea.setCdErr(cdErr);
		enteSiamDaAllinea.setDsMsgErr(dsErr);
		enteSiamDaAllinea.setDtErr(new Date());
		break;
     ...
     *
     * @param idUserIamCor          user
     * @param idAmbienteEnteConvenz ambiente convenzionato
     *
    ...
    public List<OrgVRicEnteConvenzByEsterno> retrieveEntiConvenzAbilitatiAmbiente(
	    BigDecimal idUserIamCor, BigDecimal idAmbienteEnteConvenz, String flNonConvenz) {
    ...
    public boolean isStoricoPresente(long idVers, long idAmbienteVersExcluded,
     ...
     */
    public List<SIOrgEnteSiam> getOrgEnteConvenzCollegUserAbilList(BigDecimal idUserIamCor,
	    BigDecimal idEnteConvenz) {
	...
	Query query = getEntityManager().createQuery(
		"SELECT s FROM SIOrgAccordoEnte s WHERE s.siOrgEnteConvenz.idEnteSiam = :idEnteConvenz "
    ...
    public List<UsrVAbilStrutSacerXping> getOrganizzazioniSacerFromUsrVAbilStrutSacerXping(
	    long idUserIam, String tiDichVers) {
	String chiave = tiDichVers.equals("STRUTTURA") ? "organiz.idOrganizIamStrut"
		: tiDichVers.equals("ENTE") ? "organiz.idOrganizIamEnte"
			: "organiz.idOrganizIamAmbiente";
	StringBuilder queryStr = new StringBuilder(
	...
	final Query query = getEntityManager().createQuery(
		"SELECT DISTINCT new it.eng.sacerasi.grantEntity.OrgVRicEnteConvenzByEsterno(ente.id.idEnteConvenz, ente.nmEnteConvenz,"
			+ " ente.idCategEnte," + "ente.idAmbitoTerrit," + "ente.dtCessazione,"
			+ "ente.dtIniVal," + "ente.flRecesso," + "ente.enteAttivo,"
			+ "ente.idAmbienteEnteConvenz," + "ente.dtDecAccordo,"
	...
	if (list.size() == 1) {
		...
		throw new RuntimeException(
			"Sono presenti più record OrgVRicEnteConvenzByEsterno per idEnteConvenz "
				+ idEnteConvenz);
	    }
	}
    }
