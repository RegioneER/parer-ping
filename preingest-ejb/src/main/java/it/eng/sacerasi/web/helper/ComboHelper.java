...
public class ComboHelper {
    ...
    public PigAmbienteVersTableBean getAmbienteVersatoreFromUtente(long idUtente) {
        String queryStr = "SELECT DISTINCT ambienteVer "
                ...
                + " WHERE iamAbilOrgs.iamUser.idUserIam = :idUtente " + " AND iamAbilOrgs.idOrganizApplic = ver.idVers "
        ...
        try {
            if (!ambienteList.isEmpty()) {
                ...
                ambienteTableBean = (PigAmbienteVersTableBean) Transform.entities2TableBean(ambienteList);
            }
        } catch (Exception e) {
    ...
    public PigVersTableBean getVersatoreFromAmbienteVersatore(Long idUtente, BigDecimal idAmbienteVers) {
    ...
    public PigTipoObjectTableBean getTipoObjectFromVersatore(long idUtente, BigDecimal idVers) {
        StringBuilder queryStr = new StringBuilder("SELECT DISTINCT u from PigTipoObject u, IamAbilTipoDato iatd "
                + "WHERE iatd.idTipoDatoApplic = u.idTipoObject " + "AND iatd.nmClasseTipoDato = 'TIPO_OBJECT' "
        ...
        try {
            if (!tipoObjectList.isEmpty()) {
                // trasformo la lista di entity (risultante della query) in un tablebean
                tipoObjectTableBean = (PigTipoObjectTableBean) Transform.entities2TableBean(tipoObjectList);
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
    ...
    public PigTipoObjectTableBean getTipoObjectFromVersatore(long idUtente, BigDecimal idVers, String... tipoVers) {
     ...
     */
    public PigTipoObjectTableBean getTipoObjectFromVersatoreNoFleggati(long idUtente, BigDecimal idVers,
            String... tipoVers) {
        return getTipoObjectFromVersatore(idUtente, idVers, true, tipoVers);
    }

    /*
     * Se viene passato il flag a vero non vengono estratti i tipi oggetto con il flag di visibilità alzato
     */
    private PigTipoObjectTableBean getTipoObjectFromVersatore(long idUtente, BigDecimal idVers, boolean escludiFleggati,
            String... tipoVers) {
        StringBuilder queryStr = new StringBuilder("SELECT DISTINCT u from PigTipoObject u, IamAbilTipoDato iatd "
                + "WHERE iatd.idTipoDatoApplic = u.idTipoObject " + "AND iatd.nmClasseTipoDato = 'TIPO_OBJECT' "
                + "AND iatd.iamAbilOrganiz.iamUser.idUserIam = :idUtente ");
    ...
    /**
     * Recupera i versatori da mostrare nella combo della pagina di scelta versatori dopo aver effettuato il login
     *
     * @param idUtente
     *            id utente
     * 
    ...
    public Object[] getVersatori(long idUtente) {
        ...
        return new Object[] { versatoriTableBean, idDefVers };
    }
