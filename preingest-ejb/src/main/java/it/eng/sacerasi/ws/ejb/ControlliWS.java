...
public class ControlliWS {
     ...
     * @param codErr      il codice di errore del messaggio da inviare in caso di errore
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    ...
    public RispostaControlli verificaCdKeyObjectLength(String cdKeyObject, String codErr) {
        ...
        // MEV 27880 - i figli generati da trasformazione hanno un nome più lungo di 38 caratteri
        // rispetto al padre
     ...
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaCdKeyObject(String nmAmbiente, String nmVersatore,
            String cdKeyObject) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(false);
    ...
    /**
     * Verifica che il nome del tipo oggetto sia definito nell'ambito del versatore; In caso di
     * errore setta il codice errore e la descrizione in base al codice passato come parametro
     *
     * @param idVers       idVersatore
     * @param nmTipoObject nome tipo oggetto
     * @param codErr       codice errore
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaNomeTipoObject(long idVers, String nmTipoObject,
            String codErr) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(false);
        try {
    ...
    /**
     * Verifica che l'algoritmo sia SHA-1/256/etc In caso di errore setta il codice errore e la
     * descrizione in base al codice passato come parametro
     *
     * @param hashAlgo  algoritmo hash
    ...
    /**
     * Verifica che l'encoding sia HexBinary In caso di errore setta il codice errore e la
     * descrizione in base al codice passato come parametro
     *
     * @param hashEncoding encoding hash
     * @param errorCode    codice di errore
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaEncoding(String hashEncoding, String errorCode) {
        ...
        if (Constants.TipiEncBinari.evaluateByDesc(hashEncoding)
    ...
    private String encodePassword(String password) {
        ...
        try {
            ...
            throw new SacerPingRuntimeException("Algoritmo SHA-1 non supportato", ex,
                    PingErrorCategory.INTERNAL_ERROR);
        } catch (UnsupportedEncodingException ex) {
            log.error("Algoritmo UTF-8 non supportato");
            throw new SacerPingRuntimeException("Algoritmo UTF-8 non supportato", ex,
                    PingErrorCategory.INTERNAL_ERROR);
        }
    ...
    public RispostaControlli verificaStatoOggetto(Long idObject, StatoOggetto stato,
            String errorCode) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
    ...
    /*
     * Gruppo di controlli inseriti per la MAC #14809 - WS invio oggetto: non viene calcolato il
     * versatore per cui generare oggetti
     */
    public RispostaControlli verificaCdVersGenCasoNonEsteso(
        ...
        try {
            PigVersTipoObjectDaTrasfTableBean tb = amministrazioneEjb
                    .getPigVersTipoObjectDaTrasfTableBean(
                            new BigDecimal(invioOggettoAsincronoExt.getIdTipoObject()));
            if (tb == null || tb.size() == 0) {
                ...
                if (StringUtils.isBlank(regExp)) {
                    ...
                    rispostaControlli
                            .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_021,
                    ...
                    if (matcher.matches()) {
                        ...
                        rispostaControlli.setDsErr(
                                MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_022));
                    }
                }
            }
            // Se il cdVersGen è stato determinato passa di qui altrimenti esce ma avrà sicuramente
            // impostato un errore
            // in uscita!
            if (!StringUtils.isBlank(invioOggettoAsincronoExt.getCdVersGen())) {
                rispostaControlli = controlliInvioOggettoAsincrono.verificaCdVersGen(
                        ...
                        invioOggettoAsincronoExt.getCdVersGen());
                if (!rispostaControlli.isrBoolean()) {
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_023);
                    rispostaControlli.setDsErr(
                            MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_023));
                } else {
                    rispostaControlli.setrBoolean(true);
                }
            }
     ...
     *
     * Se il cdVersGen non è valorizzato, se il tipo vers file = "DA_TRASFORMARE" e la regExpr non è
     * definita verifica se esistono tipi oggetto da trasformare; se ne esiste uno solo estrae il
     * suo cdVersGen e valorizza il parametro di input con quello estratto.
     *
     * @param invOgg oggetto contentnte tutti i parametri di input
     */
        ...
        if (invOgg.getInvioOggettoAsincronoInput() instanceof InvioOggettoAsincronoEstesoInput) {
            ...
            if (cdVersGen == null || cdVersGen.trim().equals("")) {
                if (invOgg.getTiVersFile().equals(Constants.TipoVersamento.DA_TRASFORMARE.name())
                        && (invOgg.getDsRegExpCdVers() == null
                                || invOgg.getDsRegExpCdVers().equals(""))) {
                    PigVersTipoObjectDaTrasfTableBean tipoObjDaTrasfTB = amministrazioneEjb
                            .getPigVersTipoObjectDaTrasfTableBean(
                                    new BigDecimal(invOgg.getIdTipoObject()));
