...
public class SalvataggioDati {
	...
	javax.persistence.Query q = entityManager.createQuery(
		"DELETE FROM PigFileObject fileObj WHERE fileObj.pigObject.idObject = :objId");
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    private RispostaControlli salvaFileObject(NotificaTrasferimentoExt nte,
	    FileDepositatoRespType fileDepRest, long size, BackendStorage backend) {
	...
	// Verifico che il file object non esista già a causa di un precedente versamento (Caso
	// warning)
	...
	fiObj.setPigTipoFileObject(entityManager.find(PigTipoFileObject.class,
		nte.getTipoFileObjects().get(fileDepRest.getNmNomeFile())));
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli modificaSessione(Long idLastSession, StatoSessioneIngest stato,
	...
	try {
	    ...
	    if (dsErr != null) {
		// In caso di errore, setto la descrizione e il flag di sessione errata verificata a
		// false
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli modificaSessione(Long idLastSession, StatoOggetto stato,
	    String dlMotivoChiusoWarning, boolean setDtChius) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	try {
	    PigSessioneIngest sessione = entityManager.find(PigSessioneIngest.class, idLastSession);
	...
	try {
		...
		PigTipoObject pigTipoObject = entityManager.find(PigTipoObject.class,
			idTipoOggetto);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli creaSessione(NotificaTrasferimentoExt nte, String cdErr,
	    String dsErr) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	...
	sessione.setFlFileCifrato(
		ioae.getInvioOggettoAsincronoInput().isFlFileCifrato() ? Constants.DB_TRUE
			: Constants.DB_FALSE);
	sessione.setFlForzaAccettazione(
		ioae.getInvioOggettoAsincronoInput().isFlForzaAccettazione() ? Constants.DB_TRUE
			: Constants.DB_FALSE);
	sessione.setFlForzaWarning(
		ioae.getInvioOggettoAsincronoInput().isFlForzaWarning() ? Constants.DB_TRUE
			: Constants.DB_FALSE);
	sessione.setDlMotivoForzaAccettazione(
		ioae.getInvioOggettoAsincronoInput().getDlMotivazione());
	...
	/*
	 * Il parametro di tipo gestione oggetti figli, dato che un oggetto DA_TRASFORMARE può
	 * essere inviato anche da InvioOggettoAsincrono standard, deve essere gestito fuori dal
	 * controllo di sopra
	 */
    ...
    /*
     * MEV #12941 - WS Invio oggetto: estendere controlli semantici in riferimento al Tipo
     * oggetto/Tipo SIP versato Imposta gli oppurtuni flag della sessione secondo quanto richiesto
     * dalla MEV.
     */
    private void controlliMev12941(PigSessioneIngest sessione, String tiVersFile,
	    String nmTipoObject) {
	if (tiVersFile != null) {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli creaStatoSessione(long idSessioneIngest, String statoSessione,
	    Date dtRegStato) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	PigStatoSessioneIngest pigStatoSessione = new PigStatoSessioneIngest();
	final PigSessioneIngest pigSessioneIngest = entityManager.find(PigSessioneIngest.class,
		idSessioneIngest);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli creaOggetto(InvioOggettoAsincronoExt ioaExt, Long idSessione,
	    String username) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	...
	// NO_ZIP
	// Se la priorità di versamento viene passata in input usa quella per l'oggetto che si sta
	// creando
	...
	if (ioaExt.getInvioOggettoAsincronoInput().getTiPrioritaVersamento() == null) {
	    ...
	    obj.impostaPrioritaVersamento(
		    it.eng.sacerasi.web.util.Constants.ComboFlagPrioVersType.getValueByEnumName(
			    ioaExt.getInvioOggettoAsincronoInput().getTiPrioritaVersamento()),
		    username);
	}
	...
	if (ioaExt.getInvioOggettoAsincronoInput() instanceof InvioOggettoAsincronoEstesoInput) {
		...
		obj.setPigObjectPadre(
			entityManager.find(PigObject.class, ioaExt.getIdOggettoPadre()));
	    }
	    ...
	    // RAMO INSERITO PER LA MAC #14809 - WS invio oggetto: non viene calcolato il versatore
	    // per cui generare
	    // oggetti
	    // Anche nel caso di invio oggetto ridotto quando il CD_GEN viene calcolato deve essere
	    ...
	    } else if (ioaExt.getTiVersFile()
		    .equals(Constants.TipoVersamento.DA_TRASFORMARE.name())) {
		// MAC #20989: Se la priporità non è specificata controllare che esista sul
		// tipoOggetto e settare
		// quella,
		...
		if (obj.getPigTipoObject().getTiPriorita() != null) {
		    ...
		    risp.setrBoolean(false);
		    risp.setCodErr(MessaggiWSBundle.ERR_666);
		    risp.setDsErr(MessaggiWSBundle
			    .getString(MessaggiWSBundle.ERR_TIPOOBJECT_PRIORITA_CHECK));
		}
	    } else {
		/*
		 * SE LA PRIORITA' NON VIENE PASSATA DALL'ESTERNO IL SISTEMA LA PRELEVA DAL TIPO
		 * OGGETTO!
		 */
	...
	if (userList != null && !userList.isEmpty()) {
	    ...
	    throw new ParerInternalError(
		    "Errore inatteso: Impossibile trovare l'utente " + username);
	}
	return user;
    }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli salvaInfoDicom(Long idObject, DatiSpecificiType datiSpec,
	    Long idXsdDatiSpec) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	...
	infoDicom.setBlDcmHashTxt(new String(Base64.decodeBase64(datiSpec.getDCMHashDescrizione()),
		StandardCharsets.UTF_8));
	infoDicom.setBlGlobalHashTxt(new String(
		Base64.decodeBase64(datiSpec.getGLOBALHashDescrizione()), StandardCharsets.UTF_8));
	...
	infoDicom.setDlListaModalityInStudy(datiSpec.getModalityInStudyList() != null
		...
		&& !datiSpec.getModalityInStudyList().getModalityInStudy().isEmpty()
			? buildDatiSpecLists(datiSpec.getModalityInStudyList().getModalityInStudy())
			: null);
	infoDicom.setDlListaSopClass(datiSpec.getSOPClassList() != null
		&& datiSpec.getSOPClassList().getSOPClass() != null
		&& !datiSpec.getSOPClassList().getSOPClass().isEmpty()
			? buildDatiSpecLists(datiSpec.getSOPClassList().getSOPClass())
			: null);
	...
	infoDicom.setDtPatientBirthDate(datiSpec.getPatientBirthDate() != null
		? datiSpec.getPatientBirthDate().toGregorianCalendar().getTime()
		: null);
	infoDicom.setDtPresaInCarico(
		datiSpec.getDataPresaInCarico().toGregorianCalendar().getTime());
	...
	infoDicom.setTiPatientSex(
		datiSpec.getPatientSex() != null ? datiSpec.getPatientSex() : null);
	...
	try {
		...
		log.error("Eccezione nella persistenza dell'oggetto : \n"
			+ "CD_AET_NODO_DICOM: {} \n" + "CD_PATIENT_ID: {} \n"
			...
			+ "ID_OBJECT: {} \n" + "ID_XSD_SPEC: {} \n" + "TI_PATIENT_SEX: {} \n"
			+ "ID_INFO_DICOM: {} \n" + "BL_DCM_HASH_TXT: {} \n"
			+ "BL_GLOBAL_HASH_TXT: {}", infoDicom.getCdAetNodoDicom(),
			infoDicom.getCdPatientId(), infoDicom.getCdPatientIdIssuer(),
			infoDicom.getCdVersioneDatiSpecDicom(),
			infoDicom.getDlListaModalityInStudy(), infoDicom.getDlListaSopClass(),
			infoDicom.getDlStudyDescription(), infoDicom.getDsAccessionNumber(),
			infoDicom.getDsDcmHash(), infoDicom.getDsFileHash(),
			infoDicom.getDsGlobalHash(), infoDicom.getDsInstitutionName(),
			infoDicom.getDsPatientName(), infoDicom.getDsRefPhysicianName(),
			infoDicom.getDsStudyId(), infoDicom.getDsStudyInstanceUid(),
			infoDicom.getDtPatientBirthDate(), infoDicom.getDtPresaInCarico(),
			infoDicom.getDtStudyDate(), infoDicom.getIdVers(),
			infoDicom.getNiStudyRelatedImages(), infoDicom.getNiStudyRelatedSeries(),
			idObject, idXsdDatiSpec, infoDicom.getTiPatientSex(),
			infoDicom.getIdInfoDicom(), infoDicom.getBlDcmHashTxt(),
			infoDicom.getBlGlobalHashTxt());
	    }
	}
	return risp;
    }
	...
	return (builder.toString().length() > Constants.MAX_BYTES_DICOM_SIZE)
		? builder.substring(0, Constants.MAX_BYTES_DICOM_SIZE)
		: builder.toString();
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli creaSessioneRecupero(RichiestaRestituzioneOggettoExt rroExt,
	    String cdErr, String dsErr) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli creaSessioneRecupero(NotificaPrelievoExt npExt, String errorCode,
	    String errorMessage) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	PigSessioneRecup sessione = new PigSessioneRecup();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli modificaSessioneRecupero(NotificaPrelievoExt npExt, String cdErr,
	    String dsErr) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	try {
	    PigSessioneRecup sessione = entityManager.find(PigSessioneRecup.class,
		    npExt.getIdSessioneRecup());
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli creaSessioneRecupero(NotificaInAttesaPrelievoExt niapExt,
    ...
    // FIXME: Utilizzare la funzione generica sottostante che prende in input idSessione, stato,
    // cdErr e dsErr
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli modificaSessioneRecupero(NotificaInAttesaPrelievoExt niapExt,
	    String cdErr, String dsErr) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	try {
	    PigSessioneRecup sessione = entityManager.find(PigSessioneRecup.class,
		    niapExt.getIdSessioneRecup());
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli modificaSessioneRecupero(Long idSessioneRecup,
	    Constants.StatoSessioneRecup stato, String cdErr, String dsErr) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	try {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli creaSessioneRecupero(PuliziaNotificatoExt pnExt, String errorCode,
	    String errorMessage) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	PigSessioneRecup sessione = new PigSessioneRecup();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli updateSessioniIngestErrate(String nmVers, String cdKeyObject,
	    Long idObject) {
	RispostaControlli risp = new RispostaControlli();
	risp.setrBoolean(true);
	try {
	    ...
	    updateQuery.setParameter("idSessioneIngest",
		    HibernateUtils.longFrom(object.getIdLastSessioneIngest()));
    ...
    private void deleteDir(String rootDir, String ftpPath, String ftpDir)
	    throws InterruptedException, FileUnderUseException, LockingFailedException,
	    DirectoryNotEmptyException, InsufficientPermissionOnFileException,
	    NoTransactionAssociatedException, ResourceException {
	...
	try {
	    ...
	    log.warn(
		    " [SalvataggioDati] errore nella cancellazione della cartella {}{}{}: la risorsa non esiste",
