...
public class ControlliInvioOggettoAsincrono {
    ...
    /**
     * Verifica che la versione dell'XML sia pari ad una delle versioni supportate da SACER Pre
     * Ingest
     *
     * @param cdVersioneXml codice versione xml
     * @param compat        lista elementi di tipo String
     *
    ...
    public RispostaControlli verificaVersioneXML(String cdVersioneXml, String[] compat) {
        ...
        if (!rispostaControlli.isrBoolean()) {
            ...
            rispostaControlli.setDsErr(
                    MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_005, cdVersioneXml));
        }
        return rispostaControlli;
    }

    /**
     * Verifica se è settato il flag di forza accettazione in input e se è definita la motivazione
     * dell'accettazione
     *
     * @param idTipoObject        id tipo oggetto
     * @param flForzaAccettazione flag 1/0 (true/false)
     * @param dlMotivazione       descrizione motivazione
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaAccettazioneMotivazione(Long idTipoObject,
            boolean flForzaAccettazione, String dlMotivazione) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
        /*
         * MEV#12941 - WS Invio oggetto: estendere controlli semantici in riferimento al Tipo
         * oggetto/Tipo SIP versato
         */
        ...
        if (to.getTiVersFile().equals(TipoVersamento.NO_ZIP.name())
            ...
            if (flForzaAccettazione && StringUtils.isBlank(dlMotivazione)) {
                ...
                rispostaControlli
                        .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_010));
            }
        }
        return rispostaControlli;
    }
     ...
     *
     * @param cdVersioneXml codice versione
     * @param xml           contenuto xml
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaPresenzaXml(String cdVersioneXml, String xml) {
        ...
        if ((StringUtils.isBlank(cdVersioneXml) && StringUtils.isNotBlank(xml))
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_004));
        }
        return rispostaControlli;
    }
            ...
            if (to.getTiVersFile().equals(TipoVersamento.ZIP_CON_XML_SACER.name())
                ...
                if (pigXsdDatiSpecs.isEmpty() && StringUtils.isNotBlank(xml)) {
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle
                            .getString(MessaggiWSBundle.PING_SENDOBJ_008, to.getNmTipoObject()));
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle
                            .getString(MessaggiWSBundle.PING_SENDOBJ_020, to.getNmTipoObject()));
                }
                ...
                if (StringUtils.isBlank(xml)) {
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle
                            .getString(MessaggiWSBundle.PING_SENDOBJ_009, to.getNmTipoObject()));
                }
            }
        } catch (Exception e) {
     ...
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaStudioDicom(Long idTipoObject, String xml,
            String cdKeyObject) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
        PigTipoObject tipoObj = entityManager.find(PigTipoObject.class, idTipoObject);
        if (tipoObj.getNmTipoObject().equalsIgnoreCase(Constants.STUDIO_DICOM)
                && StringUtils.isBlank(xml)) {
            rispostaControlli.setrBoolean(false);
            rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_011);
            rispostaControlli.setDsErr(
                    MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_011, cdKeyObject));
        }
     ...
     *
     * @param idTipoObject          id tipo oggetto
     * @param versioneDatiSpecifici versione dati specifici
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
        ...
        if (tipoObject.getPigXsdDatiSpecs() != null && !tipoObject.getPigXsdDatiSpecs().isEmpty()) {
            if (versioneDatiSpecifici == null) {
                ...
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                        MessaggiWSBundle.PING_SENDOBJ_XML_002, tipoObject.getNmTipoObject()));
            } else {
                boolean vrsDatiSpecFound = false;
                for (int j = 0; j < tipoObject.getPigXsdDatiSpecs().size(); j++) {
                    if (tipoObject.getPigXsdDatiSpecs().get(j).getCdVersioneXsd()
                            .equals(versioneDatiSpecifici)) {
                        vrsDatiSpecFound = true;
                        // Mi faccio ritornare il blocco xsd
                        rispostaControlli
                                .setrString(tipoObject.getPigXsdDatiSpecs().get(j).getBlXsd());
                        rispostaControlli
                                .setrLong(tipoObject.getPigXsdDatiSpecs().get(j).getIdXsdSpec());
                        break;
                    }
                }
                if (!vrsDatiSpecFound) {
                    ...
                    rispostaControlli.setDsErr(
                            MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_003,
    ...
    private PigTipoFileObject getTipoFileObject(long idTipoObject, String nmTipoFileObject) {
        ...
        try {
            ...
            log.error("Eccezione nella lettura della tabella dei tipi file oggetto {}",
                    e.getMessage());
        }
        return ptfo;
    }
     ...
     *
     * @param idTipoObject              id tipo oggetto
     * @param nmTipoFileObject          nome tipo file
     * @param versioneDatiSpecificiFile versione dati specifici
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaTipoFileObject(Long idTipoObject, String nmTipoFileObject,
        ...
        if (tipoFileObject == null) {
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_005,
            ...
            if (tipoFileObject.getPigXsdDatiSpecs() != null
                    && !tipoFileObject.getPigXsdDatiSpecs().isEmpty()) {
                if (versioneDatiSpecificiFile == null) {
                    rispostaControlli.setrBoolean(false);
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_XML_006);
                    rispostaControlli.setDsErr(MessaggiWSBundle
                            .getString(MessaggiWSBundle.PING_SENDOBJ_XML_006, nmTipoFileObject));
                } else {
                    boolean vrsDatiSpecFound = false;
                    for (int j = 0; j < tipoFileObject.getPigXsdDatiSpecs().size(); j++) {
                        if (tipoFileObject.getPigXsdDatiSpecs().get(j).getCdVersioneXsd()
                            ...
                            rispostaControlli.setrString(
                                    tipoFileObject.getPigXsdDatiSpecs().get(j).getBlXsd());
                            break;
                        }
                    }
                    if (!vrsDatiSpecFound) {
                        ...
                        rispostaControlli.setDsErr(
                                MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_007,
     ...
     *
     * @param idTipoObject  id tipo oggetto
     * @param sizeUnitaDocs numero documento unita doc
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaUdInXml(Long idTipoObject, int sizeUnitaDocs) {
        ...
        if (tipoObj.getTiVersFile().equals(TipoVersamento.NO_ZIP.name()) && sizeUnitaDocs > 1) {
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_010));
        }
     ...
     *
     * @param idTipoObject id tipo oggetto
     * @param chiave       valore chiave
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaChiave(Long idTipoObject,
            it.eng.sacerasi.ws.xml.invioAsync.ChiaveType chiave) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);

        PigTipoObject tipoObj = entityManager.find(PigTipoObject.class, idTipoObject);
        if (tipoObj.getTiVersFile().equals(TipoVersamento.NO_ZIP.name())
                && (tipoObj.getTiCalcKeyUnitaDoc().equals(TipoCalcolo.XML_VERS.name()))
                && chiave == null) {
            rispostaControlli.setrBoolean(false);
            rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_XML_009);
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_009));
        }
     ...
     *
     * @param idTipoObject id tipo oggetto
     * @param files        file di tipo
     *                     {@link it.eng.sacerasi.ws.xml.invioAsync.UnitaDocumentariaType.Files}
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaFiles(Long idTipoObject,
        ...
        if (tipoObj.getTiVersFile().equals(TipoVersamento.NO_ZIP.name())
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_012));
        }
     ...
     *
     * @param sopClassList elemento di tipo {@link SOPClassList}
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaSopClassList(SOPClassList sopClassList) {
        ...
        if (checkDatiSpecLists(sopClassList.getSOPClass())) {
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_DICOM_001));
        }
     ...
     *
     * @param modalityInStudyList elemento di tipo {@link ModalityInStudyList}
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaModalityStudio(ModalityInStudyList modalityInStudyList) {
        ...
        if (checkDatiSpecLists(modalityInStudyList.getModalityInStudy())) {
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_DICOM_002));
        }
     ...
     * @param tiVersFile          tipo file versamento
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaStatoOggetto(Long idObject, boolean flForzaAccettazione,
            String ftpPath, String tiVersFile) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
        ...
        if (obj.getTiStatoObject().equals(StatoOggetto.CHIUSO_ERR_VERS.name())
                && Arrays.asList(Constants.TipoVersamento.getTipoVersamentoNoTrasf())
                        .contains(tipoVersamentoEnum)) {
            ...
            try {
                ...
                if (xadConn.fileExistsAndIsDirectory(path)) {
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                            MessaggiWSBundle.PING_SENDOBJ_OBJ_011, obj.getCdKeyObject()));
                }
            } catch (InterruptedException ie) {
                Thread.currentThread().interrupt(); // Ripristino lo stato di interruzione
                rispostaControlli.setrBoolean(false);
                rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
                        String.join("\n", ExceptionUtils.getRootCauseStackTrace(ie))));
                log.error("Eccezione nella gestione della risorsa XADisk: " + ie.getMessage(), ie);
            ...
            rispostaControlli.setDsErr(MessaggiWSBundle
                    .getString(MessaggiWSBundle.PING_SENDOBJ_OBJ_001, obj.getCdKeyObject()));
        } else if (obj.getTiStatoObject().equals(StatoOggetto.IN_CODA_HASH.name())
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_OBJ_002,
            ...
            rispostaControlli.setDsErr(MessaggiWSBundle
                    .getString(MessaggiWSBundle.PING_SENDOBJ_OBJ_010, obj.getCdKeyObject()));
        } else if (obj.getPigTipoObject().getNmTipoObject()
                .equalsIgnoreCase(Constants.STUDIO_DICOM)) {
            if (obj.getTiStatoObject().equals(StatoOggetto.WARNING.name())
                    && !flForzaAccettazione) {
                rispostaControlli.setrBoolean(false);
                rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_OBJ_003);
                rispostaControlli.setDsErr(
                        MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_OBJ_003));
                ...
                rispostaControlli.setDsErr(
                        MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_OBJ_004));
            }
        }
        return rispostaControlli;
    }
     ...
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaModificaTipoOggetto(Long idObject, String nmTipoObject,
            String tiVersFile) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
        ...
        if (obj.getTiStatoObject().equals(StatoOggetto.CHIUSO_ERR_NOTIF.name())
                ...
                || obj.getTiStatoObject()
                        .equals(StatoOggetto.CHIUSO_ERR_VERSAMENTO_A_PING.name())) {
            if (!obj.getPigTipoObject().getNmTipoObject().equals(nmTipoObject)) {
                ...
                // MEV 34105 - disabilitato controllo su cambiamento tipo oggetto anche per oggetti
                // generati da una
                ...
                if (tiVersFile != null && tiVersFile.equals(tipoVersEsistente)
                    ...
                    // Se il tipoVersFile dell'oggetto precedente e quello che si sta versando sono
                    // uguali e
                    // DA_TRASFORMARE
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                            MessaggiWSBundle.PING_SENDOBJ_OBJ_012, obj.getCdKeyObject()));
                }
            }
        }
        return rispostaControlli;
     ...
     *
     * @param idTipoObj      id tipo oggetto
     * @param dcmHash        hash
     * @param flForzaWarning flag 1/0 (true/false)
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaOggettoDicom(Long idTipoObj, String dcmHash,
            boolean flForzaWarning) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
        ...
        if (!flForzaWarning) {
            ...
            if (obj != null) {
                if (obj.getTiStatoObject().equals(StatoOggetto.CHIUSO_OK.name())) {
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                            MessaggiWSBundle.PING_SENDOBJ_OBJ_005, obj.getCdKeyObject()));
                } else if (obj.getTiStatoObject().equals(StatoOggetto.IN_ATTESA_FILE.name())
                        || obj.getTiStatoObject().equals(StatoOggetto.IN_ATTESA_SCHED.name())
                        || obj.getTiStatoObject().equals(StatoOggetto.IN_ATTESA_VERS.name())
                        || obj.getTiStatoObject().equals(StatoOggetto.IN_CODA_VERS.name())
                        || obj.getTiStatoObject().equals(StatoOggetto.CHIUSO_ERR_CODA.name())
                        || obj.getTiStatoObject()
                                .equals(StatoOggetto.CHIUSO_ERR_RECUPERABILE.name())) {
                    rispostaControlli.setrBoolean(false);
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_OBJ_006);
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                            MessaggiWSBundle.PING_SENDOBJ_OBJ_006, obj.getCdKeyObject()));
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                            MessaggiWSBundle.PING_SENDOBJ_OBJ_007, obj.getCdKeyObject()));
                } else if (obj.getTiStatoObject().equals(StatoOggetto.CHIUSO_ERR_NOTIF.name())
                        || obj.getTiStatoObject()
                                .equals(StatoOggetto.CHIUSO_ERR_VERIFICA_HASH.name())
                        || obj.getTiStatoObject().equals(StatoOggetto.CHIUSO_ERR_SCHED.name())
                        || obj.getTiStatoObject().equals(StatoOggetto.CHIUSO_ERR_VERS.name())) {
                    rispostaControlli.setrBoolean(false);
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_OBJ_008);
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                            MessaggiWSBundle.PING_SENDOBJ_OBJ_008, obj.getCdKeyObject()));
                }
            }
        } else {
            rispostaControlli.setrBoolean(false);
            ...
            rispostaControlli
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_OBJ_009));
        }
        return rispostaControlli;
    }
     ...
     *
     * @param idObject id oggetto
     * @param stato    stato oggetto
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaUltimaSessioneOggetto(Long idObject,
            Constants.StatoOggetto stato) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(false);
    ...
    /**
     * Verifica l'esistenza di un oggetto figlio dell'oggetto padre specificato con progressivo pari
     * a pgObjectFiglio
     *
     * @param idObjectPadre  id padre
     * @param pgObjectFiglio id figlio
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaOggettoFiglio(Long idObjectPadre, BigDecimal pgObjectFiglio) {
        ...
        query.setParameter("statiObject", Arrays.asList(StatoOggetto.CHIUSO_ERR_NOTIF.name(),
                StatoOggetto.CHIUSO_ERR_SCHED.name(), StatoOggetto.CHIUSO_ERR_VERIFICA_HASH.name(),
                ...
                StatoOggetto.CHIUSO_ERR_CRASH_FS_PRIM.name(),
    ...
    /**
     * Verifica che, se l'oggetto è 'DA_TRASFORMARE', per gli oggetti figli esista una unità doc con
    ...
    public RispostaControlli verificaOggettoPadre(Long idObject, Long idOggettoPadre) {
        ...
        try {
            PigObject obj = entityManager.find(PigObject.class, idObject);
            if (obj.getPigObjectPadre() != null && (idOggettoPadre == null
                    || obj.getPigObjectPadre().getIdObject().compareTo(idOggettoPadre) != 0)) {
                rispostaControlli.setrBoolean(false);
                rispostaControlli.setCodErr(MessaggiWSBundle.PING_SENDOBJ_OBJ_013);
                rispostaControlli.setDsErr(MessaggiWSBundle
                        .getString(MessaggiWSBundle.PING_SENDOBJ_OBJ_013, obj.getCdKeyObject()));
            }
        } catch (Exception ex) {
            rispostaControlli.setrBoolean(false);
            rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
