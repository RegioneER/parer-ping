...
public class DatiSpecificiParserEjb {
	...
	if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
		...
		Unmarshaller tmpUnmarshaller = xmlContextCache
			.getVersReqAsyncDSDicomCtx_DatiSpecifici().createUnmarshaller();
		...
		JAXBElement<DatiSpecificiType> elemento = (JAXBElement<DatiSpecificiType>) tmpUnmarshaller
			.unmarshal(Utils.getSaxSourceForUnmarshal(
				invioOggettoAsincronoExt.getDatiSpecDicom()));
		...
		rispostaWs.getInvioOggettoAsincronoRisposta()
			.setCdErr(MessaggiWSBundle.PING_SENDOBJ_XML_004);
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    ...
	    rispostaControlli = controlliInvioOggettoAsincrono
		    .verificaSopClassList(parsedDatiSpec.getSOPClassList());
