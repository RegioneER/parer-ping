...
public class InvioOggettoAsincronoEjb {
    ...
    public InvioOggettoAsincronoRisposta invioOggettoAsincrono(String userName, String nmAmbiente,
	    String nmVersatore, String cdKeyObject, String nmTipoObject, boolean flFileCifrato,
	...
	if (log.isDebugEnabled()) {
	    log.debug(
		    "Ricevuta richiesta di InvioOggettoAsincrono con i parametri : nmAmbiente = {} , "
			    ...
			    + "flForzaWarning = {} , flForzaAccettazione = {} , "
			    + "dlMotivazione = {}, "
			    + "cdVersioneXml = {} , tiPrioritaVersamento =  {} , " + "xml = {}",
		    nmAmbiente, nmVersatore, cdKeyObject, nmTipoObject, flFileCifrato,
		    flForzaWarning, flForzaAccettazione, dlMotivazione, cdVersioneXml,
		    tiPrioritaVersamento, StringUtils.abbreviate(xml, 10000));
	}
	RispostaWSInvioOggettoAsincrono rispostaWs = initRispostaWS(
		new InvioOggettoAsincronoRisposta());
	// Istanzio l'oggetto che contiene i parametri ricevuti
	InvioOggettoAsincronoInput inputParameters = new InvioOggettoAsincronoInput(nmAmbiente,
		nmVersatore, cdKeyObject, nmTipoObject, flFileCifrato, flForzaWarning,
	 ...
	 *
	 * Nel caso in cui il parametro ACCETTA_STUDI_IN_WARNING (sul versatore o sull'ambiente o
	 * sull'applicazione) sia impostato a TRUE, se la prima chiamata genera un warning allora
	 * riverso lo stesso oggetto con una forzatura e una motivazione. La stessa cosa che
	 * dovrebbe fare il DPI facendolo a mano dall'applicazione.
	 */
	log.debug("Esito risposta: {}", rispostaWs.getInvioOggettoAsincronoRisposta().getCdEsito());
	if (rispostaWs.getInvioOggettoAsincronoRisposta().getCdEsito()
		.equals(Constants.EsitoServizio.WARN)) {
	    ...
	    String parametroAccettaStudi = configurationHelper.getValoreParamApplicByIdVers(
		    Constants.ACCETTA_STUDI_IN_WARNING,
		    new BigDecimal(ambienteVers.getIdAmbienteVers()),
		...
		String parametroMotivazione = configurationHelper.getValoreParamApplicByIdVers(
			"MOTIVAZIONE_ACCETTA_STUDI_WARNING",
			new BigDecimal(ambienteVers.getIdAmbienteVers()),
			new BigDecimal(pigVers.getIdVers()));
		log.debug("Parametro MOTIVAZIONE_ACCETTA_STUDI_WARNING: {}", parametroAccettaStudi);
		inputParameters = new InvioOggettoAsincronoInput(nmAmbiente, nmVersatore,
			cdKeyObject, nmTipoObject, flFileCifrato, false, true, parametroMotivazione,
    ...
    public InvioOggettoAsincronoEstesoRisposta invioOggettoAsincronoEsteso(String userName,
	    String nmAmbiente, String nmVersatore, String cdKeyObject, String dsObject,
	    String nmTipoObject, boolean flFileCifrato, boolean flForzaWarning,
	    ...
	    BigDecimal niTotObjectFigli, BigDecimal pgObjectFiglio, BigDecimal niUnitaDocAttese,
	    String cdVersGen, String tiGestOggettiFigli, String tiPriorita,
	    String tiPrioritaVersamento) {
	//
	if (log.isDebugEnabled()) {
	    log.debug(
		    "Ricevuta richiesta di InvioOggettoAsincronoEsteso con i parametri : nmAmbiente = {}  , "
			    ...
			    + "nmAmbienteObjectPadre = {} , nmVersatoreObjectPadre = {} , "
			    + "cdKeyObjectPadre = {} , "
			    + "niTotObjectFigli = {}  , pgObjectFiglio = {} , niUnitaDocAttese = {} "
			    + " , " + "cdVersGen = {} , "
			    + "tiGestOggettiFigli = {} , tiPriorita = {} , tiPrioritaVersamento = {}",
		    nmAmbiente, nmVersatore, cdKeyObject, nmTipoObject, dsObject, flFileCifrato,
		    flForzaWarning, flForzaAccettazione, dlMotivazione, cdVersioneXml,
		    StringUtils.abbreviate(xml, 100), nmAmbienteObjectPadre, nmVersatoreObjectPadre,
		    cdKeyObjectPadre, niTotObjectFigli, pgObjectFiglio, niUnitaDocAttese, cdVersGen,
		    tiGestOggettiFigli, tiPriorita, tiPrioritaVersamento);
	}
	RispostaWSInvioOggettoAsincrono rispostaWs = initRispostaWS(
		new InvioOggettoAsincronoEstesoRisposta());

	// Istanzio l'oggetto che contiene i parametri ricevuti
	InvioOggettoAsincronoInput inputParameters = new InvioOggettoAsincronoEstesoInput(
		nmAmbiente, nmVersatore, cdKeyObject, dsObject, nmTipoObject, flFileCifrato,
		flForzaWarning, flForzaAccettazione, dlMotivazione, cdVersioneXml, xml,
		nmAmbienteObjectPadre, nmVersatoreObjectPadre, cdKeyObjectPadre, niTotObjectFigli,
		pgObjectFiglio, niUnitaDocAttese, cdVersGen, tiGestOggettiFigli, tiPriorita,
    ...
    private InvioOggettoAsincronoExt initInvioOggettoAsincronoExt(
	    InvioOggettoAsincronoInput inputParameters) {
	...
	// Chiamo la classe (EJB) InvioOggettoAsincronoCheck che gestisce i controlli di sessione e
	// di oggetto e popola
	// la
	// rispostaWs
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR
	    ...
	    // Chiamo la classe InvioOggettoAsincronoParser che esegue il parse dell'xml, se
	    // presente
	...
	if (StringUtils.isNotBlank(ioaExt.getInvioOggettoAsincronoInput().getNmAmbiente())
		...
		&& StringUtils
			.isNotBlank(ioaExt.getInvioOggettoAsincronoInput().getCdKeyObject())) {
	    ...
	    if (log.isDebugEnabled()) {
		log.debug(
			"Sessione in creazione con Data apertura : {} Data chiusura : {}  Stato : {}",
			String.valueOf(ioaExt.getDtApertura()),
			String.valueOf(ioaExt.getDtChiusura()), ioaExt.getStatoSessione().name());
	    }
		...
		tmpRispostaControlli = salvataggioDati.creaStatoSessione(idSessione,
			ioaExt.getStatoSessione().name(), ioaExt.getDtApertura());
		if (tmpRispostaControlli.getCodErr() != null) {
		    prosegui = false;
		    setRispostaWsError(rispostaWs, tmpRispostaControlli, SeverityEnum.ERROR,
			    Constants.EsitoServizio.KO);
		...
		if (ioaExt.getIdOggettoPadre() != null && prosegui) {
		    ...
		    tmpRispostaControlli = salvataggioDati
			    .updateOggettoPadre(ioaExt.getIdOggettoPadre(),
				    ((InvioOggettoAsincronoEstesoInput) ioaExt
					    .getInvioOggettoAsincronoInput())
					    .getNiTotObjectFigli());
		    if (tmpRispostaControlli.getCodErr() != null) {
			prosegui = false;
		...
		if (ioaExt.isFlRegistraObject()) {
		    if (idObject != null) {
			/*
			 * Caso in cui l'oggetto ha stato CHIUSO_WARNING e viene nuovamente versato,
			 * oppure ha stato WARNING e viene nuovamente versato forzando, oppure ha
			 * stato CHIUSO_ERR_NOTIF o CHIUSO_ERR_SCHED o CHIUSO_ERR_VERS o
			 * CHIUSO_ERR_CRASH_DPI o CHIUSO_ERR_CRASH_FTP o CHIUSO_ERR_CRASH_FS_PRIM o
			 * CHIUSO_ERR_CRASH_FS_SECOND o ANNULLATO o CHIUSO_ERR_XFORMER o
			 * CHIUSO_ERR_VESAMENTO_A_PING e viene nuovamente versato
			 */

			/*
			 * Modifica in InvioOggettoPreIngest per gestire come errore il caso in cui
			 * l'oggetto inviato esista già ed è IN_ATTESA_FILE (la modifica si rende
			 * necessaria perché non si spiega la ragione per cui non dovesse essere un
			 * errore l'esistennza di un oggetto con stato = IN_ATTESA_FILE e perché la
			 * sua gestione come un non errore provoca concorrenza sul file nel DPI fra
			 * il job che fa invio dell'oggetto e quello che fa la notifica
			 * dell'avvenuto trasferimento file
			 */
			// verificaFtpDir = true;
			...
			if (tipoVersamentoFile
			    ...
			    List<PigObject> oggettiFigli = monitoraggioHelper
				    .getTuttiFigli(idObject);
			...
			tmpRispostaControlli.reset();
			// Punto 3
			tmpRispostaControlli = controlliInvioOggettoAsincrono
				.verificaUltimaSessioneOggetto(idObject,
					Constants.StatoOggetto.WARNING);
			if (tmpRispostaControlli.isrBoolean()) {
			    /*
			     * se sessione identificata da id_last_sessione_ingest specificato
			     * dall'oggetto ha stato = WARNING, aggiorna tale sessione assegnando
			     * ti_stato = CHIUSO_FORZATA
			     */
			    log.debug(
			    ...
			    tmpRispostaControlli.reset();
			    tmpRispostaControlli = salvataggioDati.modificaSessione(
				    idLastSessionObject,
				    Constants.StatoSessioneIngest.CHIUSO_FORZATA, null, null);
			    if (tmpRispostaControlli.getCodErr() != null) {
				prosegui = false;
				setRispostaWsError(rispostaWs, tmpRispostaControlli,
					SeverityEnum.ERROR, Constants.EsitoServizio.KO);
			    }
			    Date now = tmpRispostaControlli.getrDate();
			    log.debug("Creazione stato sessione");
			    tmpRispostaControlli.reset();
			    tmpRispostaControlli = salvataggioDati.creaStatoSessione(
				    idLastSessionObject,
			...
			if (prosegui) {
			    log.debug(
				    "Modifico lo stato oggetto in {} e gli assegno la sessione creata",
				    ioaExt.getStatoSessione());
			    tmpRispostaControlli.reset();
			    tmpRispostaControlli = salvataggioDati.modificaOggetto(idObject, ioaExt,
				    idSessione, userName, pgOggettoTrasf, niUnitaDocAttese,
				    dsObject);
			    if (tmpRispostaControlli.getCodErr() != null) {
				prosegui = false;
				setRispostaWsError(rispostaWs, tmpRispostaControlli,
			...
			// Punto 5
			if (StringUtils.isNotBlank(xml) && ioaExt.isFlRegistraXMLObject()
				&& prosegui) {
			    log.debug("Modifico lo xml object");
			    tmpRispostaControlli.reset();
			    tmpRispostaControlli = salvataggioDati.modificaXmlObject(idObject, xml,
				    cdVersioneXml);
			    if (tmpRispostaControlli.getCodErr() != null) {
				prosegui = false;
				setRispostaWsError(rispostaWs, tmpRispostaControlli,
			...
			// Punto 6
			if (ioaExt.getInvioOggettoAsincronoInput().getNmTipoObject()
				.equalsIgnoreCase(Constants.STUDIO_DICOM)
				&& ioaExt.isFlRegistraDatiSpecDicom() && prosegui) {
			...
			if (ioaExt.getTiVersFile()
				.equals(Constants.TipoVersamento.DA_TRASFORMARE.name())) {
			    tmpRispostaControlli.reset();
			    /*
			     * se l'oggetto versato ha tipo che prevede tipo versamento =
			     * DA_TRASFORMARE e per gli oggetti figli non esiste una unità doc con
			     * stato = VERSATA_OK o VERSATA_TIMEOUT
			     */
			    /*
			     * MEV#14652 - Eliminazione punto 7 dell'analisi tmpRispostaControlli =
			     * checker.checkOggettiFigli(idObject); if
			     * (!tmpRispostaControlli.isrBoolean()) { // Se non esistono, pulisco
			     * l'oggetto DA_TRASFORMARE salvataggioDati.cleanOggettoPadre(idObject);
			     * }
			     */
			}
		    } else {
			tmpRispostaControlli = salvataggioDati.creaOggetto(ioaExt, idSessione,
				userName);
			if (tmpRispostaControlli.getCodErr() != null) {
			    prosegui = false;
			    setRispostaWsError(rispostaWs, tmpRispostaControlli, SeverityEnum.ERROR,
			...
			if (StringUtils.isNotBlank(xml) && ioaExt.isFlRegistraXMLObject()
			    ...
			    tmpRispostaControlli = salvataggioDati.creaXmlObject(idObject, xml,
				    cdVersioneXml);
			    if (tmpRispostaControlli.getCodErr() != null) {
				prosegui = false;
				setRispostaWsError(rispostaWs, tmpRispostaControlli,
			...
			if (nmTipoObject.equalsIgnoreCase(Constants.STUDIO_DICOM)
				&& ioaExt.isFlRegistraDatiSpecDicom() && prosegui) {
			    tmpRispostaControlli.reset();
			    log.debug("Salvataggio dati DICOM");
			    tmpRispostaControlli = salvataggioDati.salvaInfoDicom(idObject,
			...
			// Punto IV
			tmpRispostaControlli = salvataggioDati.modificaSessione(idSessione,
				idObject);
			if (tmpRispostaControlli.getCodErr() != null) {
			    prosegui = false;
			    setRispostaWsError(rispostaWs, tmpRispostaControlli, SeverityEnum.ERROR,
    ...
    private void setRispostaWsError(RispostaWSInvioOggettoAsincrono rispostaWs,
	    RispostaControlli rispostaControlli, SeverityEnum sev, Constants.EsitoServizio esito) {
	/*
	 *
	 ...
	 * ioaExt.setStatoSessione(Constants.StatoSessioneIngest.IN_ATTESA_FILE);
	 * ioaExt.setDtChiusura(null); break; case WARNING:
	 * ioaExt.setStatoSessione(Constants.StatoSessioneIngest.WARNING);
	 * ioaExt.setDtChiusura(null); break;
	 */
	...
	log.debug("Errore Invio : {} - {}", rispostaControlli.getCodErr(),
