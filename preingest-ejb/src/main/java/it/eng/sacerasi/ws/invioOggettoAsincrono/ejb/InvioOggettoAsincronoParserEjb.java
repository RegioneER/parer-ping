...
public class InvioOggettoAsincronoParserEjb {
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		if (invioOggettoAsincronoExt.getTiVersFile()
		    ...
		    tmpUnmarshaller = xmlContextCache.getVersReqAsyncCtx_ListaUnitaDocumentarie()
			    .createUnmarshaller();
		    ...
		    JAXBElement<ListaUnitaDocumentarieType> elemento = (JAXBElement<ListaUnitaDocumentarieType>) tmpUnmarshaller
			    .unmarshal(Utils.getSaxSourceForUnmarshal(invioOggettoAsincronoExt
				    .getInvioOggettoAsincronoInput().getXml()));
			...
			tmpUnmarshaller = xmlContextCache.getInvioSUCtx_InvioSU()
				.createUnmarshaller();
			...
			tmpUnmarshaller = xmlContextCache.getInvioSismaCtx_InvioSisma()
				.createUnmarshaller();
			tmpUnmarshaller.setSchema(xmlContextCache.getInvioSismaSchema());
			tmpUnmarshaller.unmarshal(Utils.getSaxSourceForUnmarshal(
				invioOggettoAsincronoExt.getInvioOggettoAsincronoInput().getXml()));
		    } else {
			// MEV 27034 - provo a parsare l'xml per vedere se è ben formato. Non entro
			// nel dettaglio della
			// sua forma.
			...
			// tmpUnmarshaller =
			// xmlContextCache.getVersReqAsyncDaTrasfCtx_Oggetto().createUnmarshaller();
		...
		rispostaWs.setErrorMessage(
			MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_001));
		rispostaWs.getInvioOggettoAsincronoRisposta()
			.setCdEsito(Constants.EsitoServizio.KO);
		rispostaWs.getInvioOggettoAsincronoRisposta()
			.setCdErr(MessaggiWSBundle.PING_SENDOBJ_XML_001);
		...
		rispostaWs.setErrorMessage(MessaggiWSBundle
			.getString(MessaggiWSBundle.ERR_XML_MALFORMED, e.getMessage()));
		rispostaWs.getInvioOggettoAsincronoRisposta()
			.setCdEsito(Constants.EsitoServizio.KO);
		rispostaWs.getInvioOggettoAsincronoRisposta()
	 ...
	 */
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR && (invioOggettoAsincronoExt
		.getTiVersFile().equals(Constants.TipoVersamento.NO_ZIP.name())
		|| invioOggettoAsincronoExt.getTiVersFile()
			.equals(Constants.TipoVersamento.ZIP_NO_XML_SACER.name()))) {
		...
		// Prima di ciclare le unità documentarie verifico che per il tipo di versamento sia
		// definito il numero
		...
		rispostaControlli = controlliInvioOggettoAsincrono.verificaUdInXml(
			...
			parsedListaUnitaDoc.getUnitaDocumentaria().size());
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			...
			String versioneDatiSpecifici = parsedListaUnitaDoc.getUnitaDocumentaria()
				.get(i).getDatiSpecifici() != null
					? parsedListaUnitaDoc.getUnitaDocumentaria().get(i)
						.getDatiSpecifici().getValue()
						.getVersioneDatiSpecifici()
					: null;
			// Ricavo l'insieme degli elementi File
			Files insiemeFile = parsedListaUnitaDoc.getUnitaDocumentaria().get(i)
				.getFiles();
			...
			if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			    ...
			    if (!rispostaControlli.isrBoolean()) {
				setRispostaWsError(invioOggettoAsincronoExt, rispostaWs,
					rispostaControlli);
				break;
			    }
			}
			...
			if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
				...
				invioOggettoAsincronoExt
					.setIdXsdDatiSpec(rispostaControlli.getrLong());
			    }
			}

			// Valido i dati specifici con l'xsd ritornato (Solo se il tipo oggetto è
			// NON STUDIODICOM)
			if (rispostaWs.getSeverity() != SeverityEnum.ERROR
				...
				rispostaControlli.setDsErr(MessaggiWSBundle.getString(
					MessaggiWSBundle.PING_SENDOBJ_XML_004,
				...
				try {
				    ...
				    Marshaller marshaller = xmlContextCache
					    .getVersReqAsyncCtx_ListaUnitaDocumentarie()
					    .createMarshaller();
				    marshaller.marshal(parsedListaUnitaDoc.getUnitaDocumentaria()
					    .get(i).getDatiSpecifici(), sw);
				    sw.flush();
				    sw.close();
				    XmlFragmentValidator validator = new XmlFragmentValidator(
					    rispostaWs, invioOggettoAsincronoExt);
				    ...
				    String msg = MessaggiWSBundle
					    .getString(MessaggiWSBundle.ERR_666, String.join("\n",
			...
			if (rispostaWs.getSeverity() != SeverityEnum.ERROR && insiemeFile != null) {
				...
				if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
				    ...
				    versioneDatiSpecificiFile = singoloFile
					    .getDatiSpecifici() != null
						    ? singoloFile.getDatiSpecifici().getValue()
							    .getVersioneDatiSpecifici()
						    : null;

				    // Verifico se il tipo file specificato è definito per il tipo
				    // di oggetto
				    rispostaControlli.reset();
				    rispostaControlli = controlliInvioOggettoAsincrono
					    .verificaTipoFileObject(
						    invioOggettoAsincronoExt.getIdTipoObject(),
						    nmTipoFileObject, versioneDatiSpecificiFile);
				    if (!rispostaControlli.isrBoolean()) {
					setRispostaWsError(invioOggettoAsincronoExt, rispostaWs,
						rispostaControlli);
					break;
				...
				if (rispostaWs.getSeverity() != SeverityEnum.ERROR
					...
					XmlFragmentValidator validator = new XmlFragmentValidator(
						rispostaWs, invioOggettoAsincronoExt);
					validator.validateXmlFragment(xsdString, sw.toString(),
						MessaggiWSBundle.PING_SENDOBJ_XML_008,
						nmTipoFileObject, versioneDatiSpecificiFile);
				    } catch (JAXBException | IOException ex) {
					rispostaWs.setSeverity(SeverityEnum.ERROR);
					rispostaWs.setErrorCode(MessaggiWSBundle.ERR_666);
					String msg = MessaggiWSBundle.getString(
						MessaggiWSBundle.ERR_666, ex.getMessage());
					rispostaWs.setErrorMessage(msg);
					rispostaWs.getInvioOggettoAsincronoRisposta()
						.setCdEsito(Constants.EsitoServizio.KO);
					rispostaWs.getInvioOggettoAsincronoRisposta()
				...
				// verifico che non esista un altro tag File con stesso valore del
				// tag TipoFile di
				// quello corrente
				if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
				    rispostaControlli.reset();
					...
					rispostaControlli.setrBoolean(false);
					rispostaControlli
						.setCodErr(MessaggiWSBundle.PING_SENDOBJ_XML_011);
					rispostaControlli.setDsErr(MessaggiWSBundle
						.getString(MessaggiWSBundle.PING_SENDOBJ_XML_011));
					setRispostaWsError(invioOggettoAsincronoExt, rispostaWs,
						rispostaControlli);
					break;
				    }
			...
			if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			    rispostaControlli.reset();
			    // Nel caso il tipo oggetto sia STUDIODICOM, istanzio il parser
			    // specifico
			    if (invioOggettoAsincronoExt.getTiVersFile()
				    .equals(Constants.TipoVersamento.NO_ZIP.name())
				    && invioOggettoAsincronoExt.getNmTipoObject()
				...
				invioOggettoAsincronoExt
					.setVersioneDatiSpecifici(versioneDatiSpecifici);
				//
	...
	} else if (rispostaWs.getSeverity() != SeverityEnum.ERROR && (invioOggettoAsincronoExt
		.getTiVersFile().equals(Constants.TipoVersamento.ZIP_CON_XML_SACER.name())
		|| invioOggettoAsincronoExt.getTiVersFile()
			.equals(Constants.TipoVersamento.DA_TRASFORMARE.name()))) {
	    rispostaControlli.reset();
	    ...
	    // e in caso verifico se una di esse è prevista
	    String xsdString = null;
	    // MEV27034 - l'indice xml è validato nella sua interezza, non più solo la speicifica
	    // parte dei dati
	    // specifici.
	    String xml = invioOggettoAsincronoExt.getInvioOggettoAsincronoInput().getXml();
	    String cdVersioneXml = invioOggettoAsincronoExt.getInvioOggettoAsincronoInput()
		    .getCdVersioneXml();
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    rispostaControlli = controlliInvioOggettoAsincrono.verificaVersioneXsd(
			    invioOggettoAsincronoExt.getIdTipoObject(), cdVersioneXml);
		    if (!rispostaControlli.isrBoolean()) {
			setRispostaWsError(invioOggettoAsincronoExt, rispostaWs, rispostaControlli);
		    } else {
			xsdString = rispostaControlli.getrString();
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			...
			rispostaControlli.setDsErr(
				MessaggiWSBundle.getString(MessaggiWSBundle.PING_SENDOBJ_XML_004,
					oggetto.getDatiSpecifici().getVersioneDatiSpecifici(),
					"XSD non definito"));
			setRispostaWsError(invioOggettoAsincronoExt, rispostaWs, rispostaControlli);
		    } else {
			XmlFragmentValidator validator = new XmlFragmentValidator(rispostaWs,
				invioOggettoAsincronoExt);
			validator.validateXmlFragment(xsdString, xml,
				MessaggiWSBundle.PING_SENDOBJ_XML_004, cdVersioneXml);
		    }
		}
	    }
	}
