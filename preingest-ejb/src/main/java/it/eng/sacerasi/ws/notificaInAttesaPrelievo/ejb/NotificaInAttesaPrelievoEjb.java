...
public class NotificaInAttesaPrelievoEjb {
    ...
    public NotificaInAttesaPrelievoRisposta notificaInAttesaPrelievo(String nmAmbiente, String nmVersatore,
            String cdKeyObject) {
        ...
        // NotificaInAttesaPrelievoInput inputParameters = new NotificaInAttesaPrelievoInput(nmAmbiente, nmVersatore,
        ...
        // Chiamo la classe NotificaInAttesaPrelievoCheck che gestisce i controlli e popola la rispostaWs
        ...
        try {
            ...
            tmpRispCon.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_NOTIF_IN_ATTESA_PREL));
        ...
        if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
            ...
            if (niapExt.getIdSessioneRecup() != null
                    && niapExt.getStatoSessione().equals(Constants.StatoSessioneRecup.IN_ATTESA_RECUP)) {
                ...
                tmpRispCon = salvataggioDati.modificaSessioneRecupero(niapExt, rispostaWs.getErrorCode(),
                ...
                tmpRispCon = salvataggioDati.creaSessioneRecupero(niapExt, rispostaWs.getErrorCode(),
                        rispostaWs.getErrorMessage());
                if (tmpRispCon.getCodErr() != null) {
                    setRispostaWsError(rispostaWs, tmpRispCon);
                }
            ...
            if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
                if (niapExt.getFtpOutput() != null) {
                    File udFile = new File(rootFtp.concat(niapExt.getFtpOutput()).concat(File.separator).concat("UD_")
                            .concat(cdKeyObject).concat(Constants.ZIP_EXTENSION));
                    File pcFile = new File(rootFtp.concat(niapExt.getFtpOutput()).concat(File.separator).concat("PC_")
                            .concat(cdKeyObject).concat(Constants.ZIP_EXTENSION));
        ...
        if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
            ...
            if (rispostaWs.getNotificaInAttesaPrelievoRisposta().getCdErr() != null
                    ...
                    && (niapExt.getStatoSessione().equals(Constants.StatoSessioneRecup.IN_ATTESA_PRELIEVO)
                            || niapExt.getStatoSessione().equals(Constants.StatoSessioneRecup.CHIUSO_OK))) {
    ...
    private void setRispostaWsError(RispostaWSNotificaInAttesaPrelievo risp, RispostaControlli tmpRispostaControlli) {
