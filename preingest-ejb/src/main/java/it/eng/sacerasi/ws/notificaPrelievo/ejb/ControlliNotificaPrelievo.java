 ...
 * @author Bonora_L
 */
@Stateless(mappedName = "ControlliNotificaPrelievo")
@LocalBean
public class ControlliNotificaPrelievo {

    private static final Logger log = LoggerFactory.getLogger(ControlliNotificaPrelievo.class);
    ...
    public RispostaControlli verificaOggetto(Long idObject) {
        ...
        // Eseguo una select prendendo, delle sessioni di quell'oggetto, quella che ha l'id maggiore tra tutte
        // Sicuramente questa è la più recente sessione di recupero.
        try {
            String queryStr = "SELECT ses FROM PigSessioneRecup ses " + "WHERE ses.idSessioneRecup = ("
            ...
            if (!sessioni.isEmpty()) {
                ...
                rispostaControlli.setCodErr(MessaggiWSBundle.PING_NOTIFPREL_006);
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOTIFPREL_006));
            }
        } catch (Exception e) {
            rispostaControlli.setrBoolean(false);
        ...
        if (rispostaControlli.isrBoolean()) {
            ...
            if (!sessione.getTiStato().equals(Constants.StatoSessioneRecup.IN_ATTESA_PRELIEVO.name())) {
                rispostaControlli.setrBoolean(false);
                rispostaControlli.setCodErr(MessaggiWSBundle.PING_NOTIFPREL_007);
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOTIFPREL_007));
            }
        }
