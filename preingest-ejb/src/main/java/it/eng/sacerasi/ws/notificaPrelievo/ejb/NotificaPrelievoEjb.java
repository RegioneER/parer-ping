...
public class NotificaPrelievoEjb {
    ...
    public NotificaPrelievoRisposta notificaPrelievo(String nmAmbiente, String nmVersatore,
            String cdKeyObject) {
        ...
        NotificaPrelievoInput inputParameters = new NotificaPrelievoInput(nmAmbiente, nmVersatore,
                cdKeyObject);
        ...
        // Chiamo la classe RichiestaRestituzioneOggettoCheck che gestisce i controlli e popola la
        // rispostaWs
        ...
        if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
            ...
            if (npExt.getIdSessioneRecup() != null) {
                ...
                tmpRispCon = salvataggioDati.modificaSessioneRecupero(npExt,
        ...
        if (npExt.getFtpOutput() != null) {
            File udFile = new File(rootFtp.concat(npExt.getFtpOutput()).concat(File.separator)
                    .concat("UD_").concat(cdKeyObject).concat(Constants.ZIP_EXTENSION));
            File pcFile = new File(rootFtp.concat(npExt.getFtpOutput()).concat(File.separator)
                    .concat("PC_").concat(cdKeyObject).concat(Constants.ZIP_EXTENSION));
    ...
    private RispostaControlli rimuoviFile(File... files) {
        ...
        try {
            ...
            log.error("Errore nella rimozione dei file ", ie);
    ...
    private void setRispostaWsError(RispostaWSNotificaPrelievo risp,
