...
public class ControlliNotificaTrasferimento {
     ...
     */
    public RispostaControlli verificaStatoOggetto(String nmAmbiente, String nmVersatore, String cdKeyObject) {
        ...
        try {
            ...
            if (!objs.isEmpty()) {
                ...
                if (!objs.get(0).getTiStatoObject().equalsIgnoreCase(Constants.StatoOggetto.IN_ATTESA_FILE.name())) {
                    rispostaControlli.setrBoolean(false);
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_NOT_005);
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_005));
                }
            }
        } catch (Exception e) {
    ...
    public RispostaControlli verificaPresenzaDirFtp(String ftpPath) {
        ...
        try {
            ...
            if (file.exists() && file.isDirectory()) {
                ...
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_006));
                // registro sul log applicativo questo evento come un errore; pur non essendo un'eccezione
     ...
     * @param cdKeyObject
     *            numero oggetto
     * @param listaFileDepositati
     *            lista file depositati di tipo {@link FileDepositatoType}
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaCoerenzaTipoVersamentoFile(String nmAmbiente, String nmVersatore,
            String cdKeyObject, List<FileDepositatoType> listaFileDepositati) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(false);
        try {
            String queryStr = "SELECT obj FROM PigObject obj INNER JOIN obj.pigVer vers "
            ...
            if (!lista.isEmpty()) {
                ...
                if (!tipo.equals(Constants.TipoVersamento.NO_ZIP.name())
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_007));
                } else {
                    rispostaControlli.setrBoolean(true);
    ...
    /**
     * Verifica la coerenza del numero di file contenuti nella directory ftp rispetto ai file dichiarati
     *
     * @param ftpPath
     *            path ftp
     * @param size
    ...
    public RispostaControlli verificaCoerenzaNumeroFile(String ftpPath, int size) {
        ...
        if (file.exists() && file.isDirectory()) {
            ...
            if (list.length == size) {
                ...
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_008));
            }
        }
     ...
     *            id oggetto
     * @param listaFiles
     *            file di tipo {@link ListaFileDepositatoType}
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaDocPrincipaleSuNoZip(Long idObj, ListaFileDepositatoType listaFiles) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
        ...
        if (obj.getPigTipoObject().getTiVersFile().equals(Constants.TipoVersamento.NO_ZIP.name())) {
            ...
            if (docPrincCounter != 1) {
                ...
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_010));
            }
        }
     ...
     *            id oggetto
     * @param listaFileDepositati
     *            lista file di tipo {@link FileDepositatoType}
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaTipoFile(Long idObj, List<FileDepositatoType> listaFileDepositati) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(false);
     ...
     *            id tipo file
     * @param fileDep
     *            file depositato {@link FileDepositatoType}
     *
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaHash(Long idObj, Long idTiFileObj, FileDepositatoType fileDep) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(false);
        ...
        if (tiFileObj.getTiCalcHashSacer() != null) {
            if (tiFileObj.getTiCalcHashSacer().equals(Constants.HashCalcType.FILE_HASH_DICOM.name())) {
                ...
                if (!infoDicom.isEmpty()) {
                    ...
                    if (StringUtils.isNotBlank(info.getDsFileHash())
                        ...
                        if (obj.getPigTipoObject().getFlContrHash().equals(Constants.DB_TRUE)) {
                            ...
                            rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_012));
                        } else if (tiFileObj.getFlCalcHashSacer().equals(Constants.DB_FALSE)) {
                            rispostaControlli.setCodErr(MessaggiWSBundle.PING_NOT_013);
                            rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_013));
                        }
                    }
                }
            } else if (tiFileObj.getTiCalcHashSacer().equals(Constants.HashCalcType.NOTIFICATO.name())) {
                if (StringUtils.isNotBlank(fileDep.getDsHashFile()) && StringUtils.isNotBlank(fileDep.getCdEncoding())
    ...
    public RispostaControlli verificaNomeFileFtp(String ftpPath, String nomeFile) {
        ...
        if (file.exists() && file.isFile()) {
            ...
            rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_014, nomeFile));
        }
     ...
     * @return RispostaControlli.isrBoolean() == true in caso la verifica dia esito positivo
     */
    public RispostaControlli verificaDataCreazioneFile(String ftpPath, String nomeFile, Long idSessione) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(false);
        ...
        if (file.exists() && file.isFile()) {
            ...
            if (creationDate.after(sessione.getDtApertura()) && creationDate.before(now)) {
                ...
                log.debug("Data in sessione: " + sessione.getDtApertura() + " ; Data creazione file : " + creationDate
                        + " ; Data attuale: " + now);
                rispostaControlli.setCodErr(MessaggiWSBundle.PING_NOT_015);
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_NOT_015));
            }
        }
