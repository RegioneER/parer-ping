...
public class PuliziaNotificatoCheckHelper {
    ...
    public void checkRichiesta(PuliziaNotificatoExt pnExt, RispostaWSPuliziaNotificato rispostaWs) {
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            rispostaControlli = controlliWS.verificaNomeAmbiente(pnExt.getPuliziaNotificatoInput().getNmAmbiente());
            if (!rispostaControlli.isrBoolean()) {
                if (rispostaControlli.getCodErr() == null) {
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_ELIMINAPREL_001);
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_ELIMINAPREL_001,
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            rispostaControlli.reset();
            rispostaControlli = controlliWS.verificaNomeVersatore(pnExt.getPuliziaNotificatoInput().getNmAmbiente(),
                    pnExt.getPuliziaNotificatoInput().getNmVersatore());
            if (!rispostaControlli.isrBoolean()) {
                if (rispostaControlli.getCodErr() == null) {
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_ELIMINAPREL_002);
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_ELIMINAPREL_002,
                            pnExt.getPuliziaNotificatoInput().getNmVersatore()));
                    setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO, rispostaControlli);
                } else {
                    // Errore 666
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            rispostaControlli = controlliWS.verificaNotBlankCdKeyObject(
                    pnExt.getPuliziaNotificatoInput().getCdKeyObject(), MessaggiWSBundle.PING_ELIMINAPREL_004);
            if (!rispostaControlli.isrBoolean()) {
                setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO, rispostaControlli);
            }
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            rispostaControlli = controlliWS.verificaCdKeyObjectLength(
                    pnExt.getPuliziaNotificatoInput().getCdKeyObject(), MessaggiWSBundle.PING_ELIMINAPREL_008);
            if (!rispostaControlli.isrBoolean()) {
                setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO, rispostaControlli);
            }
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            rispostaControlli.reset();
            rispostaControlli = controlliWS.verificaCdKeyObject(pnExt.getPuliziaNotificatoInput().getNmAmbiente(),
                    pnExt.getPuliziaNotificatoInput().getNmVersatore(),
                    pnExt.getPuliziaNotificatoInput().getCdKeyObject());
            if (!rispostaControlli.isrBoolean()) {
                rispostaControlli.setCodErr(MessaggiWSBundle.PING_ELIMINAPREL_005);
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_ELIMINAPREL_005,
                        pnExt.getPuliziaNotificatoInput().getCdKeyObject()));
                setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO, rispostaControlli);
            } else {
                rispostaWs.getPuliziaNotificatoRisposta()
