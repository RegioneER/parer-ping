...
public class RecuperoStatoOggettoHelper {
    ...
    public Object[] getStatoOggetto(RecuperoStatoOggettoExt rsoExt, RispostaWSRecuperoStatoOggetto rispostaWs)
            throws ParerInternalError {
        Object[] result = null;
        try {
            Query q = entityManager.createQuery("SELECT obj.tiStatoObject, statoObj.dsTiStatoObject "
                    + "FROM PigObject obj, PigStatoObject statoObj "
                    + "WHERE obj.tiStatoObject = statoObj.tiStatoObject " + "AND obj.idObject = :idObject ");
            q.setParameter("idObject", rsoExt.getIdObject());
            List<Object[]> resultList = q.getResultList();
            if (!resultList.isEmpty()) {
                result = resultList.get(0);
            } else {
                throw new Exception("Impossibile trovare PigObject con idObject " + rsoExt.getIdObject());
            }
        } catch (Exception e) {
            ...
            throw new ParerInternalError(ParerErrorSeverity.ERROR, "Eccezione nel recupero dell'oggetto "
