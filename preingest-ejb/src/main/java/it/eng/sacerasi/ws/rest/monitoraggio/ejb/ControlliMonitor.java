...
public class ControlliMonitor {
    ...
    public RispostaControlli leggiUltimaChiamataWS() {
        ...
        try {
            String queryStr = "select max(lj.dtRegLogJob) " + "from PigLogJob lj " + "where lj.nmJob = :nmJob "
    ...
    public RispostaControlli leggiElencoJob() {
        ...
        try {
            ...
            rispostaControlli.setDsErr(
                    MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666, "Eccezione ControlliMonitor.leggiElencoJob "
                            + String.join("\n", ExceptionUtils.getRootCauseStackTrace(e))));
    ...
    public RispostaControlli leggiAllarmiInIntervallo(String jobName, Date dataInizio, Date ultimaAttivitaDelJob) {
        ...
        try {
            String queryStr = "select count(j) from PigLogJob j " + "where j.dtRegLogJob >= :dataInizio "
