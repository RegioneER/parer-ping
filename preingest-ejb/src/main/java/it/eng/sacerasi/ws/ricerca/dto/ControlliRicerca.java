...
public class ControlliRicerca {
    ...
    /**
     * Verifica che se il nome del tipo oggetto non è definito in input, allora non possono essere definiti i vari XML
     *
     * @param nmTipoObject
     *            nome tipo oggetto
     * @param xmlDatiSpecFiltri
     *            xml dati specifici (filtri)
     * @param xmlDatiSpecOutput
     *            xml dati specifici
    ...
    public RispostaControlli verificaNomeTipoObjectConXML(String nmTipoObject, String xmlDatiSpecFiltri,
            String xmlDatiSpecOutput, String xmlDatiSpecOrder) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        rispostaControlli.setrBoolean(true);
        if (StringUtils.isBlank(nmTipoObject) && (StringUtils.isNotBlank(xmlDatiSpecFiltri)
                || StringUtils.isNotBlank(xmlDatiSpecOutput) || StringUtils.isNotBlank(xmlDatiSpecOrder))) {
    ...
    /**
     * Verifica l'xml relativo ai filtri in maniera tale che ogni filtro sia previsto da almeno una versione dell'XSD
     * dei dati specifici per il dato tipo di oggetto e che lo stesso filtro possa essere utilizzato come tale. Se ogni
     * filtro ricavo anche il nome colonna corrispondente nel DB
     *
     * @param idTipoObject
     *            id tipo oggetto
     * @param listaDatiSpecFiltriConNomeColonna
    ...
    public RispostaControlli verificaXMLDatiSpecFiltri(long idTipoObject,
        ...
        /*
         * Creo tre liste per 3 controlli: una contenente gli eventuali filtri non trovati, una contenente gli eventuali
         * attributi non filtri ed una contenente gli eventuali attributi con nome colonna DB uguale a null
         */
        ...
        for (DatoSpecFiltroConNomeColonna filtroTypeConNomeColonna : listaDatiSpecFiltriConNomeColonna) {
            ...
            if (filtroTypeConNomeColonna.getFiltro().getFiltroUnValore() != null) {
                for (AttribDatiSpecBean ab : attribDatiSpecBean) {
                    if (filtroTypeConNomeColonna.getFiltro().getFiltroUnValore().getDatoSpecifico()
                        ...
                        if (ab.getFlFiltroDiario().equals("0")) {
                            ...
                            // Tutto OK: ricavo il nome colonna da DB, controllando che esso sia diverso da null
                ...
                if (!trovato) {
                    attributiNonTrovati
                            .add(filtroTypeConNomeColonna.getFiltro().getFiltroUnValore().getDatoSpecifico());
                }
                ...
                if (!trovato) {
                    attributiNonTrovati.add(filtroTypeConNomeColonna.getFiltro().getFiltroNullo().getDatoSpecifico());
                }
                ...
                if (!trovato) {
                    attributiNonTrovati
                            .add(filtroTypeConNomeColonna.getFiltro().getFiltroDueValori().getDatoSpecifico());
                }
            }
        }
        ...
        if (!attributiNonTrovati.isEmpty()) {
            ...
            rispostaControlli.setrString((entityManager.find(PigTipoObject.class, idTipoObject)).getNmTipoObject());
     ...
     */
    public RispostaControlli verificaXMLDatiSpecOutput(long idTipoObject, List<String> listaDatiSpecOutput,
        ...
        /*
         * Creo una LinkedHashMap che conterrà come chiave il dato specifico e come valore il suo nome colonna nel DB
         */
        // LinkedHashMap<String, String> hsDatiSpecOutput = new LinkedHashMap<>();

        /*
         * Creo una lista che conterr� il dato specifico che voglio in output con l'aggiunta del nome colonna del DB
         */
        ...
        } // ...e in caso affermativo restituisco i dati specifici di output con l'aggiunta del nome colonna del DB
        else {
     ...
     * @param idTipoObject
     *            id tipo oggetto
     * @param listaDatiSpecOrderConNomeColonna
    ...
    public RispostaControlli verificaXMLDatiSpecOrder(long idTipoObject,
        ...
        for (DatoSpecOrderConNomeColonna datoSpecOrder : listaDatiSpecOrderConNomeColonna) {
            boolean trovato = false;
            for (AttribDatiSpecBean ab : listaAttribDatiSpecBean) {
                if (datoSpecOrder.getDatoSpecificoOrder().getDatoSpecifico().equals(ab.getNmAttribDatiSpec())) {
                    trovato = true;
                    // Tutto OK: ricavo il nome colonna da DB, controllando che esso sia diverso da null
                    if (ab.getNomeColonna() != null) {
        ...
        if (attributiNonTrovati.size() > 0) {
            ...
            rispostaControlli.setrObject(attributiConCampoColonnaDBNullo);
            rispostaControlli.setrString(" - Il nome colonna del/dei filtro/i su DB � nullo ");
        } // ...e in caso affermativo restituisco i dati specifici per l'ordine con l'aggiunta del nome colonna del DB
        else {
    ...
    public List<AttribDatiSpecBean> getAttribDatiSpecBean(long idTipoObject) {
        ...
        try {
            ...
            log.error("Eccezione nel join tra le tabelle PigTipoObject, PigXsdDatiSpec e PigAttribDatiSpec", e);
        }
