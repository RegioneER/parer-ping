...
public class RicercaDiarioExecuteQueries {
	...
	try {
	    ...
	    rispostaWs.getRicercaDiarioRisposta().setDsErr(MessaggiWSBundle.getString(
     ...
     * @param cdKeyObject                  numreo oggetto
     * @param rispostaWs                   risposta ws
     *
     * @return ListaOggRicDiarioType, l'oggetto contenente la lista di oggetti, risultato della
     *         query ricerca
     */
    ...
    public ListaOggRicDiarioType eseguiQueryRicerca(RicercaDiarioExt rdExt,
	    ...
	    String cdKeyObject, RispostaWSRicercaDiario rispostaWs) {
	...
	try {
		...
		if (datiSpecOutputConNomeColonna != null) {
		    ...
		    obj.setXmlDatiSpecResult(
			    ricercaDiarioParser.parseDatiSpecResult(listaDS, rdExt, rispostaWs));

		    /*
		     * 2) Comincio controllando se il parametro in input CdKeyObject è stato
		     * valorizzato
		     */
		    if (cdKeyObject != null) {
			...
			obj.setChiaveUnitaDoc(
				"Il numero di unità documentarie generate dall'oggetto � > 1");
		    }
