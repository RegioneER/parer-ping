...
public class RicercaRecuperatiCheckHelper {
    ...
    public void checkSessione(RicercaRecuperatiExt ricercaRecuperatiExt, RispostaWSRicercaRecuperati rispostaWs) {
        RispostaControlli rispostaControlli = new RispostaControlli();
        // Verifica Nome Ambiente
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            rispostaControlli.reset();
            rispostaControlli = controlliWS
                    .verificaNomeAmbiente(ricercaRecuperatiExt.getRicercaRecuperatiInput().getNmAmbiente());
            if (!rispostaControlli.isrBoolean()) {
                if (rispostaControlli.getCodErr() == null) {
                    rispostaControlli.setCodErr(MessaggiWSBundle.PING_RICRECUP_001);
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_RICRECUP_001,
            ...
            rispostaWs.getRicercaRecuperatiRisposta()
                    .setNmAmbiente(ricercaRecuperatiExt.getRicercaRecuperatiInput().getNmAmbiente());
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            if (!rispostaControlli.isrBoolean()) {
                if (rispostaControlli.getCodErr() == null) {
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_RICRECUP_002,
                            ricercaRecuperatiExt.getRicercaRecuperatiInput().getNmVersatore()));
                    setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO, rispostaControlli);
                } else {
                    // Errore 666
            ...
            rispostaWs.getRicercaRecuperatiRisposta()
                    .setNmVersatore(ricercaRecuperatiExt.getRicercaRecuperatiInput().getNmVersatore());
        }
        ricercaRecuperatiExt.setIdVersatore(idVersatore);
    }

    private void setRispostaWsError(RispostaWSRicercaRecuperati rispostaWs, SeverityEnum sev, EsitoServizio esito,
