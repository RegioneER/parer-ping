...
public class RicercaRestituzioniOggettiQueriesBuilder {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(RicercaRestituzioniOggettiQueriesBuilder.class);
    RicercaRestituzioniOggettiExt ricercaRestituzioniOggettiExt;

    public RicercaRestituzioniOggettiQueriesBuilder(
	    RicercaRestituzioniOggettiExt ricercaRestituzioniOggettiExt) {
     ...
     *
     * @return objects, un array di object di 3 elementi contenenti nell'ordine: 1) la stringa della
     *         queryConta 2) la stringa della queryRicerca 3) la LinkedHashMap contenente i
     *         parametri da passare alla query in fase di sua esecuzione
     */
	...
	if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
		.getRicercaRestituzioniOggettiInput().getNmAmbiente())) {
	    whereCondition.append("AND sessione_recupero.nmAmbienteVers = :valore")
		    .append(contatoreValori).append(" ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getNmAmbiente());
	    contatoreValori++;
	}
	if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
		.getRicercaRestituzioniOggettiInput().getNmVersatore())) {
	    whereCondition.append("AND sessione_recupero.nmVers = :valore").append(contatoreValori)
		    .append(" ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getNmVersatore());
	    contatoreValori++;
	...
	if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
	    ...
	    whereCondition.append("AND oggetto.pigTipoObject.nmTipoObject = :valore")
		    .append(contatoreValori).append(" ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getNmTipoObject());
	    contatoreValori++;
	...
	if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
		.getRicercaRestituzioniOggettiInput().getCdKeyObject())) {
	    whereCondition.append("AND oggetto.cdKeyObject = :valore").append(contatoreValori)
		    .append(" ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getCdKeyObject());
	    contatoreValori++;
	...
	if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
		.getRicercaRestituzioniOggettiInput().getTiStatoSessione())) {
	    whereCondition.append("AND sessione_recupero.tiStato = :valore").append(contatoreValori)
		    .append(" ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getTiStatoSessione());
	    contatoreValori++;
	...
	if (ricercaRestituzioniOggettiExt.getRicercaRestituzioniOggettiInput()
		.getDtAperturaSessioneDa() != null
		&& ricercaRestituzioniOggettiExt.getRicercaRestituzioniOggettiInput()
			.getDtAperturaSessioneA() != null) {
	    whereCondition.append("AND sessione_recupero.dtApertura BETWEEN ").append(":valore")
		    .append(contatoreValori).append(" AND ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getDtAperturaSessioneDa());
	    contatoreValori++;
	    whereCondition.append(":valore").append(contatoreValori).append(" ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getDtAperturaSessioneA());
	    contatoreValori++;
	} else if (ricercaRestituzioniOggettiExt.getRicercaRestituzioniOggettiInput()
		.getDtAperturaSessioneDa() != null) {
	    whereCondition.append("AND sessione_recupero.dtApertura >= :valore")
		    .append(contatoreValori).append(" ");
	    valoriQuery.put(":valore" + contatoreValori, ricercaRestituzioniOggettiExt
		    .getRicercaRestituzioniOggettiInput().getDtAperturaSessioneDa());
	    contatoreValori++;
	...
	if (ricercaRestituzioniOggettiExt.getRicercaRestituzioniOggettiInput()
		...
		if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
			.getRicercaRestituzioniOggettiInput().getXmlDatiSpecFiltri())) {
			...
			// Ricavo il filtro dato specifico guardando quale dei 3 casi è diverso da
			// null
			...
			FiltroDueValori fDueValori = datoSpecFiltro.getFiltro()
				.getFiltroDueValori();
			...
			String nomeColonnaCammellata = QueriesUtils
				.toCamelCase(datoSpecFiltro.getColumnName());
			...
			if (fUnValore != null) {
				...
				queryRicerca.append("UPPER(info_dicom.")
					.append(nomeColonnaCammellata).append(") ");
				queryConta.append("UPPER(info_dicom.").append(nomeColonnaCammellata)
					.append(") ");
			    } else {
				...
				valoriQuery.put(":valore" + contatoreValori, Arrays
					.asList(fUnValore.getValore().toUpperCase().split(",")));
			    } else {
				// Se sto passando un filtro di tipo alfanumerico o numerico
				if (!dataType.equals(AttribDatiSpecDataType.DATA)
					&& !dataType.equals(AttribDatiSpecDataType.DATETIME)) {
				    valoriQuery.put(":valore" + contatoreValori, opEl[1]
					    + fUnValore.getValore().toUpperCase() + opEl[2]);
					...
					if (dataType.equals(AttribDatiSpecDataType.DATETIME)) {
					    ...
					    Date date = (Date) timeStampDf
						    .parse(fUnValore.getValore());
					...
					// FIXME: In caso di parseException, come lo gestisco? Forse
					// meglio lanciare un
					// 666
				    }
				}
			    ...
			    try {
				// Controllo il tipo di dato dell'attributo e lo gestisco tra data,
				// timestamp e stringa
				// Controllo che la data sotto forma di stringa, sia nel formato
				// corretto
				if (dataType.equals(AttribDatiSpecDataType.DATETIME)) {
				    // Ho passato come parametri dei timestamp
				    DateFormat timeStampDf = new SimpleDateFormat(
				    ...
				    queryRicerca.append("info_dicom.")
					    .append(nomeColonnaCammellata);
				    queryRicerca.append(" BETWEEN ").append(":valore")
					    .append(contatoreValori).append(" AND ");
				    queryConta.append("info_dicom.").append(nomeColonnaCammellata);
				    queryConta.append(" BETWEEN ").append(":valore")
					    .append(contatoreValori).append(" AND ");
				    ...
				    valoriQuery.put(":valore" + contatoreValori,
					    fDueValori.getValore1());
				    contatoreValori++;
				    queryRicerca.append(":valore").append(contatoreValori);
				    queryConta.append(":valore").append(contatoreValori);
				    valoriQuery.put(":valore" + contatoreValori,
					    fDueValori.getValore2());
				    contatoreValori++;
				}
			    } catch (ParseException ex) {
				log.error(ex.getMessage(), ex);
				// FIXME: In caso di parseException, come lo gestisco? Forse meglio
				// lanciare un 666
			    }
			}
		...
		if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
		    ...
		    // for (int i = 0; i < ricercaDiarioExt.getDatiSpecOrderConNomeColonna().size();
		    // i++) {
			...
			String nomeColonnaCammellata = QueriesUtils
				.toCamelCase(dato.getColumnName());
			queryRicerca.append("info_dicom.").append(nomeColonnaCammellata)
				.append(" ");
		...
		if (StringUtils.isNotBlank(ricercaRestituzioniOggettiExt
		    ...
		    // Scorro la lista dei dati specifici (nomeDatoSpec, nomeColonnaSuDB) da
		    // definire come output della
		    // query
		...
		// A questo punto ultimo la query di ricerca aggiungendo la parte iniziale di SELECT
		// con i parametri da
		// restituire in output
		queryRicerca = standardSelectForQueryRicerca.append(queryRicercaPart)
			.append(queryRicerca);
	    } // End If Studio Dicom
	    else {
		queryRicerca = new StringBuilder(
			"FROM PigObject oggetto JOIN oggetto.pigSessioneRecups sessione_recupero ");
		queryRicerca = standardSelectForQueryRicerca.append(queryRicerca)
