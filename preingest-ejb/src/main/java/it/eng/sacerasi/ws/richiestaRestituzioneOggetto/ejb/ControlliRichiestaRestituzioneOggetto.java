...
public class ControlliRichiestaRestituzioneOggetto {
    ...
    private static final Logger log = LoggerFactory.getLogger(ControlliRichiestaRestituzioneOggetto.class);
    ...
    public RispostaControlli verificaOggetto(Long idObject) {
        ...
        if (rispostaControlli.isrBoolean() && !obj.getTiStatoObject().equals(Constants.StatoOggetto.CHIUSO_OK.name())) {
            rispostaControlli.setrBoolean(false);
            rispostaControlli.setCodErr(MessaggiWSBundle.PING_RICHOBJ_007);
            rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_RICHOBJ_007));
        }
        if (rispostaControlli.isrBoolean()
                && !obj.getPigTipoObject().getTiVersFile().equals(Constants.TipoVersamento.NO_ZIP.name())) {
            rispostaControlli.setrBoolean(false);
            rispostaControlli.setCodErr(MessaggiWSBundle.PING_RICHOBJ_006);
            rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_RICHOBJ_006));
        }
        ...
        if (rispostaControlli.isrBoolean()) {
            ...
            try {
                ...
                if (count > 0) {
                    ...
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_RICHOBJ_008));
                }
            } catch (Exception e) {
                rispostaControlli.setrBoolean(false);
