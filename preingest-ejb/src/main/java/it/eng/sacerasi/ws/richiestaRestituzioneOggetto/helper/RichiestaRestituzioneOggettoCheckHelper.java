...
public class RichiestaRestituzioneOggettoCheckHelper {
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		if (rispostaControlli.getCodErr() == null) {
		    ...
		    rispostaControlli
			    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_RICHOBJ_001,
				    rroExt.getRichiestaRestituzioneOggettoInput().getNmAmbiente()));
		...
		rispostaWs.getRichiestaRestituzioneOggettoRisposta().setNmAmbiente(
			rroExt.getRichiestaRestituzioneOggettoInput().getNmAmbiente());
	    }
	}
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		if (rispostaControlli.getCodErr() == null) {
		    ...
		    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
			    MessaggiWSBundle.PING_RICHOBJ_002,
			    rroExt.getRichiestaRestituzioneOggettoInput().getNmVersatore()));
		    setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO,
			    rispostaControlli);
		} else {
		...
		rispostaWs.getRichiestaRestituzioneOggettoRisposta().setNmVersatore(
			rroExt.getRichiestaRestituzioneOggettoInput().getNmVersatore());
	    }
	}
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    ...
	    rispostaControlli = controlliWS.verificaNotBlankCdKeyObject(
		    ...
		    MessaggiWSBundle.PING_RICHOBJ_004);
	    if (!rispostaControlli.isrBoolean()) {
		setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO,
			rispostaControlli);
	    }
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    ...
	    rispostaControlli = controlliWS.verificaCdKeyObjectLength(
		    rroExt.getRichiestaRestituzioneOggettoInput().getCdKeyObject(),
		    MessaggiWSBundle.PING_RICHOBJ_009);
	    if (!rispostaControlli.isrBoolean()) {
		setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO,
			rispostaControlli);
	    }
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		...
		rispostaControlli
			.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.PING_RICHOBJ_005));
		setRispostaWsError(rispostaWs, SeverityEnum.ERROR, Constants.EsitoServizio.KO,
			rispostaControlli);
	    } else {
		rispostaWs.getRichiestaRestituzioneOggettoRisposta().setCdKeyObject(
			rroExt.getRichiestaRestituzioneOggettoInput().getCdKeyObject());
    ...
    private void setRispostaWsError(RispostaWSRichiestaRestituzioneOggetto rispostaWs,
	    SeverityEnum sev, Constants.EsitoServizio esito, RispostaControlli rispostaControlli) {
