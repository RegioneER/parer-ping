...
public class Util {
    ...
    public static void rimuoviDirRicorsivamente(File dirPath, XADiskConnection xadConn) throws FileNotExistsException,
            LockingFailedException, NoTransactionAssociatedException, InterruptedException,
            InsufficientPermissionOnFileException, DirectoryNotEmptyException, FileUnderUseException {
        if (xadConn.fileExistsAndIsDirectory(dirPath)) {
            ...
            log.info("***************************");
            log.warn("La directory da rimuovere non esiste, ignoro il problema: " + dirPath.toString());
            log.info("***************************");
        }
    }

    /*
     * Funzione ricorsiva che cancella tutti i file e le cartelle presenti a qualsiasi livello tranne la cartella
     * "dirPath" (solo se è al livello 1 della ricorsione) e tranne il file sotto "dirPath" (solo se è al livello 1
     * della ricorsione) con nome "cartellaPrincipale".zip.
     */
    public static void eliminaCartellaRicorsivamenteTranneSeStessa(File dirPath, XADiskConnection xadConn,
            String cartellaPrincipale, int livello) throws FileNotExistsException, LockingFailedException,
            NoTransactionAssociatedException, InterruptedException, InsufficientPermissionOnFileException,
            DirectoryNotEmptyException, FileUnderUseException {
        livello++;
        if (xadConn.fileExistsAndIsDirectory(dirPath)) {
            File[] elencoFile = Util.listFiles(xadConn, dirPath);
            if (elencoFile != null && elencoFile.length > 0) {
                for (File file : elencoFile) {
                    if (xadConn.fileExistsAndIsDirectory(file)) {
                        eliminaCartellaRicorsivamenteTranneSeStessa(file, xadConn, cartellaPrincipale, livello);
                    } else {
                        if (livello == 1) {
