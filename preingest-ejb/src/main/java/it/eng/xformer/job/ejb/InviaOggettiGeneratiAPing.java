...
public class InviaOggettiGeneratiAPing {
	...
	try {
	    ...
	    List<Long> pigObjectsIds = jobHelper
		    .selectPOIDFromQueue(Constants.Stato.TRASFORMATO.name());
	    for (Long poid : pigObjectsIds) {
		PigObject po = trasformazioniHelper.findById(PigObject.class, poid);
		logger.info("[InviaOggettiGeneratiAPing] inizio invio oggetto id: "
			+ po.getIdObject() + " nome: " + po.getCdKeyObject());

		List<PigObjectTrasf> pigObjectTrasfs = trasformazioniHelper
			.searchGeneratedPigObjects(po);

		int packagesCounter = pigObjectTrasfs.size();

		for (PigObjectTrasf pot : pigObjectTrasfs) {
		    PigObject oldPigObject = trasformazioniHelper
			    .searchPigObjectTrasfInPigObjects(pot);
		    // Se l'oggetto trasformato non esiste già in PING o è in errore, allora lo
		    // verso
		    if (!isPigObjectValid(oldPigObject)) {
			sendPigObjectTrasfToPing(pot, packagesCounter);
		    }
		}
		logger.info("[InviaOggettiGeneratiAPing] fine invio oggetto id: " + po.getIdObject()
	    ...
	    for (Long poid : pigObjectsIds) {
		PigObject po = trasformazioniHelper.findById(PigObject.class, poid);
		logger.info("[InviaOggettiGeneratiAPing] inizio notifica oggetto id: "
			+ po.getIdObject() + " nome: " + po.getCdKeyObject());
		...
		for (PigObjectTrasf pot : pigObjectTrasfs) {
		    PigObject oldPigObject = trasformazioniHelper
			    .searchPigObjectTrasfInPigObjects(pot);
		    if (oldPigObject != null && oldPigObject.getTiStatoObject()
			    .equals(Constants.Stato.IN_ATTESA_FILE.name())) {

			String dsPathTrasf = pot.getPigVer().getDsPathTrasf();
			String dsPathInputFtp = pot.getPigVer().getDsPathInputFtp();
			// quindi i path da dove prendere il paccchetto e il path dove mettere i
			// pacchetti generati.
			String inputFilename = configurationHelper.getValoreParamApplicByApplic(
				Constants.ROOT_TRASF) + File.separator + dsPathTrasf
				+ File.separator + pot.getCdKeyObjectTrasf() + File.separator
				+ pot.getCdKeyObjectTrasf() + Constants.STANDARD_PACKAGE_EXTENSION;
			String outputFilename = configurationHelper.getValoreParamApplicByApplic(
				Constants.ROOT_FTP) + File.separator + dsPathInputFtp
				+ File.separator + pot.getCdKeyObjectTrasf() + File.separator
				+ pot.getCdKeyObjectTrasf() + Constants.STANDARD_PACKAGE_EXTENSION;
			...
			if (pot.getPigTipoObject().getPigTipoFileObjects() == null
			    ...
			    setError(pot, "TRASFORMAZIONE_BAD_CONFIG",
				    "Nessun tipo file definito per il tipo oggetto "
					    + pot.getPigTipoObject().getNmTipoObject() + ".");

			    // segno in errore anche l'oggetto padre
			    jobHelper.changePigObjectAndSessionStateAtomic(
			...
			if (!inputFile.toFile().exists() || !inputFile.toFile().isFile()
			    ...
			    setError(pot, "TRASFORMAZIONE_FILE_NOT_FOUND",
				    "File " + inputFilename + " non trovato.");
			    // segno in errore anche l'oggetto padre
				...
				if (backendVersamento.isFile()) {
				    // MEV 30039 - cancello la cartella se esiste già per fare
				    // pulizia.
		...
		logger.info("[InviaOggettiGeneratiAPing] fine niotifica oggetto id: "
			+ po.getIdObject() + " nome: " + po.getCdKeyObject());
	    }
	    ...
	    for (Long poid : pigObjectsIds) {
		PigObject po = trasformazioniHelper.findById(PigObject.class, poid);
		logger.info("[InviaOggettiGeneratiAPing] inizio verifica versamento oggetto id: "
			+ po.getIdObject() + " nome: " + po.getCdKeyObject());
		...
		for (PigObjectTrasf pot : pigObjectTrasfs) {
		    PigObject oldPigObject = trasformazioniHelper
			    .searchPigObjectTrasfInPigObjects(pot);
		    if (oldPigObject == null || (!oldPigObject.getTiStatoObject()
			    .equals(Constants.Stato.IN_ATTESA_SCHED.name())
			    && !oldPigObject.getTiStatoObject()
				    .equals(Constants.Stato.CHIUSO_OK.name())
			    && !oldPigObject.getTiStatoObject()
				    .equals(Constants.Stato.IN_CODA_HASH.name()))) {
			completed = false;
		    }
		}
		...
		if (completed) {
		    jobHelper.changePigObjectAndSessionState(po,
			    Constants.Stato.VERSATO_A_PING.name());
		    ...
		    aggiornaStatoEventualeSisma(po.getCdKeyObject(),
			    PigSisma.TiStato.IN_TRASFORMAZIONE, PigSisma.TiStato.IN_VERSAMENTO,
			    null);
		    aggiornaStatoEventualeSisma(po.getCdKeyObject(),
			    PigSisma.TiStato.IN_TRASFORMAZIONE_SA,
			    PigSisma.TiStato.IN_VERSAMENTO_SA, null);
		}
		logger.info("[InviaOggettiGeneratiAPing] fine verifica versamento oggetto id: "
			+ po.getIdObject() + " nome: " + po.getCdKeyObject());
	    }
	} catch (Exception ex) {
    ...
    private void notifyFileTransfer(PigObjectTrasf pot, BackendStorage backendVersamento,
	    String nmFileOs) throws ObjectStorageException {
	try {
	    logger.debug("[notifyFileTransfer] inizio su oggetto figlio id: "
		    + pot.getIdObjectTrasf() + " nome: " + pot.getCdKeyObjectTrasf());
	    List<PigTipoFileObject> pigTipoFileObjects = pot.getPigTipoObject()
		    .getPigTipoFileObjects();
	    if (pigTipoFileObjects == null || pigTipoFileObjects.isEmpty()) {
		setError(pot, "BAD_CONFIG",
			"Per il tipo di oggetto generato non è definito alcun tipo file.");
		// segno in errore anche l'oggetto padre
	    ...
	    fileDepositato.setNmNomeFile(
		    pot.getCdKeyObjectTrasf() + Constants.STANDARD_PACKAGE_EXTENSION);
	    fileDepositato.setNmTipoFile(
		    pot.getPigTipoObject().getPigTipoFileObjects().get(0).getNmTipoFileObject());
	    ...
	    NotificaTrasferimentoRisposta notificaTrasferimentoRisposta = notificaTrasferimentoEjb
		    .notificaAvvenutoTrasferimentoFile(nmAmbiente, nmVersatore, cdKeyObject,
			    listaFileDepositati);

	    switch (notificaTrasferimentoRisposta.getCdEsito()) {
	    case "KO":

		setError(pot, notificaTrasferimentoRisposta.getCdErr(),
			notificaTrasferimentoRisposta.getDsErr());
	...
	logger.debug("[notifyFileTransfer] fine su oggetto figlio id: " + pot.getIdObjectTrasf()
		+ " nome: " + pot.getCdKeyObjectTrasf());
    }

    private void sendPigObjectTrasfToPing(PigObjectTrasf pot, int packagesCount) {
	try {
	    logger.debug("[sendPigObjectTrasfToPing] inizio su oggetto figlio id: "
		    + pot.getIdObjectTrasf() + " nome: " + pot.getCdKeyObjectTrasf());
	    ...
	    InvioOggettoAsincronoEstesoRisposta invioOggettoAsincronoEstesoRisposta = invioOggettoAsincronoEjb
		    .invioOggettoAsincronoEsteso(
			    configurationHelper.getValoreParamApplicByApplic(Constants.IOGP_USER),
			    nmAmbiente, nmVersatore, cdKeyObject, dsObject, nmTipoObject,
			    flFileCifrato, flForzaWarning, flForzaAccettazione, null, cdVersioneXml,
			    xml, nmAmbienteObjectPadre, nmVersatoreObjectPadre, cdKeyObjectPadre,
			    niTotObjectFigli, pgObjectFiglio, niUnitaDocAttese, null, null, null,
			    null);
		...
		if (!pot.getCdErr().equals("PING-SENDOBJ-OBJ-010")
			&& !pot.getCdErr().equals("666")) {
		    if (!pot.getPigObject().getTiStatoObject()
			    .equals(Constants.Stato.ERRORE_VERSAMENTO_A_PING.name())) {
			// segno in errore anche l'oggetto padre
			jobHelper.changePigObjectAndSessionState(pot.getPigObject(),
	...
	logger.debug("[sendPigObjectTrasfToPing] fine su oggetto figlio id: "
		+ pot.getIdObjectTrasf() + " nome: " + pot.getCdKeyObjectTrasf());
    }
