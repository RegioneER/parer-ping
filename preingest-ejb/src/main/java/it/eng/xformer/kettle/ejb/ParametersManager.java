...
public class ParametersManager {
    ...
    public long insertNewParametersSet(String name, String description, String type, long idParentTrasf) {
    ...
    public long insertNewParameter(String name, String description, String type, String defaultValue,
            long idParentParamsSet) {
        XfoSetParamTrasf xfoSetParamTrasf = helper.findById(XfoSetParamTrasf.class, idParentParamsSet);
    ...
    public XfoSetParamTrasfTableBean searchParametersSetsByTransformation(long idTrasf) throws ParerUserError {
        ...
        if (paramtersSets != null && !paramtersSets.isEmpty()) {
            try {
                ...
                throw new ParerUserError(
                        "Errore durante il recupero dei parametri: " + ExceptionUtils.getRootCauseMessage(ex));
            }
        }
    ...
    public XfoSetParamTrasfRowBean getXfoSetParamTrasfRowBean(long idSetParamTrasf) throws ParerUserError {
        XfoSetParamTrasfRowBean rowBean = null;

        XfoSetParamTrasf xfoSetParamTrasf = helper.findById(XfoSetParamTrasf.class, idSetParamTrasf);
    ...
    public void updateParametersSet(long idParametersSet, String name, String description, String type)
            throws ParerUserError {
        XfoSetParamTrasf xfoSetParamTrasf = helper.findById(XfoSetParamTrasf.class, idParametersSet);
    ...
    public void updateParameter(long idParameterTrasf, String newDescription, String newType, String newValue)
            throws ParerUserError {
    ...
    public void updateParameterByName(long idtrasf, String name, String newDescription, String newType, String newValue)
            throws ParerUserError {
    ...
    public void deleteParametersSet(long idSetParameterTrasf) throws ParerUserError {
        ...
        XfoSetParamTrasf xfoSetParamTrasf = helper.findById(XfoSetParamTrasf.class, idSetParameterTrasf);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void updateParametersInParametersSets(long idTrasf, Map<String, String> updatedParameters,
            String defaultSetName) throws ParerUserError {
        ...
        for (XfoParamTrasf param : parameters) {
            if (!updatedParameters.containsKey(param.getNmParamTrasf())) {
                ...
                logger.debug("updateParametersInParametersSet: rimosso parametro " + param.getNmParamTrasf());
            }
        }

        // aggiunge i parametri non presenti nel set esistente o aggiorna i valori standard se diversi da quelli attuali
        for (Map.Entry<String, String> updatedParam : updatedParameters.entrySet()) {
            if (isParameterAssigned(updatedParam.getKey(), idTrasf)) {
                updateParameterByName(idTrasf, updatedParam.getKey(), "--", "ALFANUMERICO", updatedParam.getValue());
                logger.debug("updateParametersInParametersSet: aggiornato parametro " + updatedParam.getKey()
                        + " con valore " + updatedParam.getValue());
            } else {
                XfoSetParamTrasf defaultParametersSet = getParametersSet(defaultSetName, idTrasf);

                if (defaultParametersSet == null) {
                    long idParametersSet = insertNewParametersSet(defaultSetName, "--", "1", idTrasf);
                    defaultParametersSet = helper.findById(XfoSetParamTrasf.class, idParametersSet);
                }

                insertNewParameter(updatedParam.getKey(), "--", "ALFANUMERICO", updatedParam.getValue(),
                        defaultParametersSet.getIdSetParamTrasf());
                logger.debug("updateParametersInParametersSet: inserito parametro " + updatedParam.getKey()
                        + " con valore " + updatedParam.getValue());
            }
        }
    }
