 ...
 */
@Entity
@Table(name = "MON_V_LIS_FASCICOLO_OBJECT")
@NamedQuery(name = "MonVLisFascicoloObject.findByPigObjectId", query = "SELECT m.cdErrSacer, min(m.dlErrSacer), min(m.cdConcatDlErrSacer) FROM MonVLisFascicoloObject m WHERE m.idObject=:idObject AND m.cdErrSacer IS NOT NULL GROUP BY m.cdErrSacer ORDER BY m.cdErrSacer")
public class MonVLisFascicoloObject implements Serializable {
