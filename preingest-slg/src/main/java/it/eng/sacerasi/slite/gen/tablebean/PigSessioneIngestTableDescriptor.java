/*
 * Engineering Ingegneria Informatica S.p.A.
 *
 * Copyright (C) 2023 Regione Emilia-Romagna <p/> This program is free software: you can
 * redistribute it and/or modify it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the License, or (at your option)
 * any later version. <p/> This program is distributed in the hope that it will be useful, but
 * WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A
 * PARTICULAR PURPOSE. See the GNU Affero General Public License for more details. <p/> You should
 * have received a copy of the GNU Affero General Public License along with this program. If not,
 * see <https://www.gnu.org/licenses/>.
 */

package it.eng.sacerasi.slite.gen.tablebean;

import java.sql.Types;
import java.util.LinkedHashMap;
import java.util.Map;

import it.eng.spagoLite.db.oracle.bean.column.ColumnDescriptor;
import it.eng.spagoLite.db.oracle.bean.column.TableDescriptor;

/**
 * @author Sloth
 *
 *         Bean per la tabella Pig_Sessione_Ingest
 *
 */
public class PigSessioneIngestTableDescriptor extends TableDescriptor {

    /*
     * @Generated( value = "it.eg.dbtool.db.oracle.beangen.Oracle4JPAClientBeanGen$TableBeanWriter",
     * comments = "This class was generated by OraTool", date = "Monday, 17 March 2014 15:12" )
     */

    public static final String SELECT = "Select * from Pig_Sessione_Ingest /**/";
    public static final String TABLE_NAME = "Pig_Sessione_Ingest";
    public static final String COL_ID_SESSIONE_INGEST = "id_sessione_ingest";
    public static final String COL_NM_AMBIENTE_VERS = "nm_ambiente_vers";
    public static final String COL_NM_VERS = "nm_vers";
    public static final String COL_CD_KEY_OBJECT = "cd_key_object";
    public static final String COL_ID_VERS = "id_vers";
    public static final String COL_DT_APERTURA = "dt_apertura";
    public static final String COL_DT_CHIUSURA = "dt_chiusura";
    public static final String COL_NM_TIPO_OBJECT = "nm_tipo_object";
    public static final String COL_FL_FILE_CIFRATO = "fl_file_cifrato";
    public static final String COL_FL_FORZA_ACCETTAZIONE = "fl_forza_accettazione";
    public static final String COL_DL_MOTIVO_FORZA_ACCETTAZIONE = "dl_motivo_forza_accettazione";
    public static final String COL_CD_VERSIONE_XML_VERS = "cd_versione_xml_vers";
    public static final String COL_TI_STATO = "ti_stato";
    public static final String COL_ID_OBJECT = "id_object";
    public static final String COL_NI_UNITA_DOC_DA_VERS = "ni_unita_doc_da_vers";
    public static final String COL_NI_UNITA_DOC_VERS = "ni_unita_doc_vers";
    public static final String COL_CD_ERR = "cd_err";
    public static final String COL_DL_ERR = "dl_err";
    public static final String COL_DL_MOTIVO_CHIUSO_WARNING = "dl_motivo_chiuso_warning";
    public static final String COL_NI_UNITA_DOC_VERS_OK = "ni_unita_doc_vers_ok";
    public static final String COL_NI_UNITA_DOC_VERS_ERR = "ni_unita_doc_vers_err";
    public static final String COL_FL_FORZA_WARNING = "fl_forza_warning";
    public static final String COL_FL_SES_ERR_VERIF = "fl_ses_err_verif";
    public static final String COL_FL_SES_ERR_NON_RISOLUB = "fl_ses_err_non_risolub";
    public static final String COL_NI_UNITA_DOC_VERS_TIMEOUT = "ni_unita_doc_vers_timeout";
    public static final String COL_TI_STATO_VERIFICA_HASH = "ti_stato_verifica_hash";

    private static Map<String, ColumnDescriptor> map = new LinkedHashMap<String, ColumnDescriptor>();

    static {
	map.put(COL_ID_SESSIONE_INGEST,
		new ColumnDescriptor(COL_ID_SESSIONE_INGEST, Types.DECIMAL, 22, true));
	map.put(COL_NM_AMBIENTE_VERS,
		new ColumnDescriptor(COL_NM_AMBIENTE_VERS, Types.VARCHAR, 100, false));
	map.put(COL_NM_VERS, new ColumnDescriptor(COL_NM_VERS, Types.VARCHAR, 100, false));
	map.put(COL_CD_KEY_OBJECT,
		new ColumnDescriptor(COL_CD_KEY_OBJECT, Types.VARCHAR, 100, false));
	map.put(COL_ID_VERS, new ColumnDescriptor(COL_ID_VERS, Types.DECIMAL, 22, false));
	map.put(COL_DT_APERTURA, new ColumnDescriptor(COL_DT_APERTURA, Types.TIMESTAMP, 7, false));
	map.put(COL_DT_CHIUSURA, new ColumnDescriptor(COL_DT_CHIUSURA, Types.TIMESTAMP, 7, false));
	map.put(COL_NM_TIPO_OBJECT,
		new ColumnDescriptor(COL_NM_TIPO_OBJECT, Types.VARCHAR, 100, false));
	map.put(COL_FL_FILE_CIFRATO,
		new ColumnDescriptor(COL_FL_FILE_CIFRATO, Types.VARCHAR, 1, false));
	map.put(COL_FL_FORZA_ACCETTAZIONE,
		new ColumnDescriptor(COL_FL_FORZA_ACCETTAZIONE, Types.VARCHAR, 1, false));
	map.put(COL_DL_MOTIVO_FORZA_ACCETTAZIONE,
		new ColumnDescriptor(COL_DL_MOTIVO_FORZA_ACCETTAZIONE, Types.VARCHAR, 1024, false));
	map.put(COL_CD_VERSIONE_XML_VERS,
		new ColumnDescriptor(COL_CD_VERSIONE_XML_VERS, Types.VARCHAR, 100, false));
	map.put(COL_TI_STATO, new ColumnDescriptor(COL_TI_STATO, Types.VARCHAR, 30, false));
	map.put(COL_ID_OBJECT, new ColumnDescriptor(COL_ID_OBJECT, Types.DECIMAL, 22, false));
	map.put(COL_NI_UNITA_DOC_DA_VERS,
		new ColumnDescriptor(COL_NI_UNITA_DOC_DA_VERS, Types.DECIMAL, 22, false));
	map.put(COL_NI_UNITA_DOC_VERS,
		new ColumnDescriptor(COL_NI_UNITA_DOC_VERS, Types.DECIMAL, 22, false));
	map.put(COL_CD_ERR, new ColumnDescriptor(COL_CD_ERR, Types.VARCHAR, 100, false));
	map.put(COL_DL_ERR, new ColumnDescriptor(COL_DL_ERR, Types.VARCHAR, 1024, false));
	map.put(COL_DL_MOTIVO_CHIUSO_WARNING,
		new ColumnDescriptor(COL_DL_MOTIVO_CHIUSO_WARNING, Types.VARCHAR, 1024, false));
	map.put(COL_NI_UNITA_DOC_VERS_OK,
		new ColumnDescriptor(COL_NI_UNITA_DOC_VERS_OK, Types.DECIMAL, 22, false));
	map.put(COL_NI_UNITA_DOC_VERS_ERR,
		new ColumnDescriptor(COL_NI_UNITA_DOC_VERS_ERR, Types.DECIMAL, 22, false));
	map.put(COL_FL_FORZA_WARNING,
		new ColumnDescriptor(COL_FL_FORZA_WARNING, Types.VARCHAR, 1, false));
	map.put(COL_FL_SES_ERR_VERIF,
		new ColumnDescriptor(COL_FL_SES_ERR_VERIF, Types.VARCHAR, 1, false));
	map.put(COL_FL_SES_ERR_NON_RISOLUB,
		new ColumnDescriptor(COL_FL_SES_ERR_NON_RISOLUB, Types.VARCHAR, 1, false));
	map.put(COL_NI_UNITA_DOC_VERS_TIMEOUT,
		new ColumnDescriptor(COL_NI_UNITA_DOC_VERS_TIMEOUT, Types.DECIMAL, 22, false));
	map.put(COL_TI_STATO_VERIFICA_HASH,
		new ColumnDescriptor(COL_TI_STATO_VERIFICA_HASH, Types.VARCHAR, 20, false));
    }

    public Map<String, ColumnDescriptor> getColumnMap() {
	return map;
    }

    public String getTableName() {
	return TABLE_NAME;
    }

    public String getStatement() {
	return SELECT;
    }

}
