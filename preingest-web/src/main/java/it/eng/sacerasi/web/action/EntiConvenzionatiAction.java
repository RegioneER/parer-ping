...
public class EntiConvenzionatiAction extends EntiConvenzionatiAbstractAction {
    ...
    public static String checkDeleteEnteConvenzOrg(int numeroEntiConvenz) {
        return numeroEntiConvenz <= 1
                ? "Il versatore è associato ad un solo ente siam. Non è possibile eseguire l'eliminazione" : null;
    }
        ...
        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
            ...
            if (getTableName().equals(getForm().getEnteConvenzOrgList().getName())) {
                ...
                if (enteConvenzOrgRowBean.getString("convenzionato").equals("1")) {
                    // Recupero tutti gli enti convenzionati a cui è abilitato e setto quello specifico
                    BigDecimal idAmbienteEnteConvenz = enteConvenzOrgRowBean.getBigDecimal("id_ambiente_ente_convenz");
                    ...
                    getForm().getEnteConvenzOrg().getId_ambiente_ente_convenz().setValue("" + idAmbienteEnteConvenz);
                } else {
                ...
                if (getForm().getEnteConvenzOrg().postAndValidate(getRequest(), getMessageBox())) {
                    ...
                    if (!getMessageBox().hasError()) {
                        ...
                        if (getForm().getEnteConvenzOrg().getStatus().equals(BaseElements.Status.insert)) {
                            ...
                            BigDecimal idEnteConvenzOrg = amministrazioneEjb.insertEnteConvenzOrg(param, idVers,
                            ...
                            // dell'associazione
                            BigDecimal idEnteConvenzOrgNew = amministrazioneEjb.updateEnteConvenzOrg(param,
                                    idEnteConvenzOrg, idEnteConvenz, idVers, dtIniVal, dtFineVal);
    ...
    public JSONObject triggerEnteConvenzOrgId_ambiente_ente_convenzOnTrigger() throws EMFError {
        ...
        if (idAmbienteEnteConvenz != null) {
            OrgVRicEnteConvenzByEsternoTableBean ricEnteConvenz = amministrazioneEjb.getOrgVRicEnteConvenzAbilTableBean(
    ...
    public void deleteEnteConvenzOrg() throws EMFError {
        ...
        BigDecimal idEnteConvenzOrgFromList = ((SIOrgEnteConvenzOrgRowBean) getForm().getEnteConvenzOrgList().getTable()
                .getCurrentRow()).getIdEnteConvenzOrg();
        ...
        if (getLastPublisher().equals(Application.Publisher.ENTE_CONVENZIONATO_DETAIL)) {
            if (!idEnteConvenzOrg.equals(idEnteConvenzOrgFromList)) {
                getMessageBox().addError("Eccezione imprevista nell'eliminazione dell'associazione all'ente siam");
            }
        }
        String error = checkDeleteEnteConvenzOrg(getForm().getEnteConvenzOrgList().getTable().size());
        if (error != null) {
    ...
    public JSONObject triggerEnteConvenzOrgId_ente_convenzOnTrigger() throws EMFError {
        ...
        if (idEnteSiam != null) {
            Date[] dateAssociazione = amministrazioneEjb.getDatePerAssociazioneEnteVersatore(idVers, idEnteSiam);
            DateFormat formato = new SimpleDateFormat("dd/MM/yyyy");
            getForm().getEnteConvenzOrg().getDt_ini_val().setValue(formato.format(dateAssociazione[0]));
            getForm().getEnteConvenzOrg().getDt_fine_val().setValue(formato.format(dateAssociazione[1]));
        }
        return getForm().getEnteConvenzOrg().asJSON();
    }
}
