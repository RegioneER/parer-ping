...
public class HomeAction extends HomeAbstractAction {
	...
	try {
		...
		int numGiorni = Integer.parseInt(configHelper
			.getValoreParamApplicByApplic(Constants.NUM_GIORNI_ESPONI_SCAD_PSW));
	...
	if (resp.getListaNews() != null) {
		...
		String dateFormatted = fmt
			.format(row.getDtIniPubblic().toGregorianCalendar().getTime());
	...
	this.getResponse().sendRedirect(
		configHelper.getValoreParamApplicByApplic(Constants.URL_MODIFICA_PASSWORD) + "?r="
			+ retURL + "&h=" + hmac + "&s=" + salt);
