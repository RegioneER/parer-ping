...
public class SceltaOrganizzazioneAction extends SceltaOrganizzazioneAbstractAction {
    ...
    private static Logger logger = LoggerFactory.getLogger(SceltaOrganizzazioneAction.class.getName());
    ...
    public void process() throws EMFError {
        ...
        if (utente.isUtenteDaAssociare()) {
            ...
            PigAmbienteVersTableBean ambientiTb = comboHelper.getAmbienteVersatoreFromUtente(getUser().getIdUtente());
            ...
            if (ambientiTb.size() == 1) {
                ...
                PigVersTableBean versatoriTb = comboHelper.getVersatoreFromAmbienteVersatore(getUser().getIdUtente(),
                        ambientiTb.getRow(0).getIdAmbienteVers());
                ...
                if (versatoriTb.size() == 1) {
                    getForm().getVersatori().getNm_vers().setValue(versatoriTb.getRow(0).getIdVers().toString());
                }
            } else {
    ...
    private void gestisciUtenteDaAssociare(User utente) throws EMFError {
        ...
        sacerLogHelper.insertEventoLoginUser(username, getIpClient(), new Date(),
                ...
                "SACERPING - " + ConstLogEventoLoginUser.DS_EVENTO_BAD_CF_SPID, utente.getCognome(), utente.getNome(),
        ...
        final String url = configHelper.getValoreParamApplicByApplic(Constants.URL_ASSOCIAZIONE_UTENTE_CF);
        try {
            this.getResponse().sendRedirect(url + "?r=" + Base64.encodeBase64URLSafeString(retURL.getBytes()) + "&h="
    ...
    public void selezionaVersatore() throws EMFError {
        ...
        DecodeMap mappaTipoVersatore = (DecodeMap) getForm().getVersatori().getNm_vers().getDecodeMap();
