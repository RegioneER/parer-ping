...
public class MultipartFileUploadToS3ForVersamentoOggettoServlet extends HttpServlet {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(MultipartFileUploadToS3ForVersamentoOggettoServlet.class);
	...
	if (isMultipart) {
		...
		while (iter.hasNext()) {
			...
			if (chunk == 0) {
			    ...
			    inviaOggetto(user, nomeFile, nm_ambiente_vers, nm_vers, nm_tipo_object,
				    ds_hash_file_vers, ds_object, cd_vers_gen,
				    ti_gest_oggetti_figli, xml_to_upload_string, cd_versione_xml,
				    nm_ambiente_vers_padre, nm_vers_padre, nm_tipo_object_padre,
				    cd_key_object_padre, ds_object_padre, ni_tot_object_trasf,
				    pg_oggetto_trasf, ti_priorita_versamento);

			    nmFileOs = versamentoOggettoEjb.computeOsFileKey(
		...
		if (input != null) {
			...
			log.error("MultipartFileUploadToS3ForVersamentoOggettoServlet", ioex);
		    }
		}
	    } finally {
		if (input != null) {
    ...
    private void inviaOggetto(User user, String nomeFile, String nm_ambiente_vers, String nm_vers,
	    ...
	    String cd_key_object_padre, String ds_object_padre, String ni_tot_object_trasf,
	    String pg_oggetto_trasf, String ti_priorita_versamento) throws ParerUserError {
	...
	form.getVersamentoOggettoDetail().getTi_gest_oggetti_figli()
		.setValue(ti_gest_oggetti_figli);
	form.getVersamentoOggettoDetail().getCd_versione_xml().setValue(cd_versione_xml);

	form.getVersamentoOggettoDetail().getNm_ambiente_vers_padre()
		.setValue(nm_ambiente_vers_padre);
	...
	try {
	    ...
	    String dsHashFileVers = form.getVersamentoOggettoDetail().getDs_hash_file_vers()
		    .parse();
	    ...
	    BigDecimal idObjectPadre = form.getVersamentoOggettoDetail().getCd_key_object_padre()
		    .parse();
	    String cdKeyObjectPadre = null;
	    String ambientePadre = StringUtils.trimToNull(form.getVersamentoOggettoDetail()
		    .getNm_ambiente_vers_padre().getDecodedValue());
	    String versatorePadre = StringUtils.trimToNull(
		    form.getVersamentoOggettoDetail().getNm_vers_padre().getDecodedValue());
	    String cdVersGen = form.getVersamentoOggettoDetail().getCd_vers_gen().parse();
	    BigDecimal niTotObjectTrasf = form.getVersamentoOggettoDetail().getNi_tot_object_trasf()
		    .parse();
	    BigDecimal pgOggettoTrasf = form.getVersamentoOggettoDetail().getPg_oggetto_trasf()
		    .parse();
	    ...
	    WebService annotationNotif = NotificaTrasferimento.class
		    .getAnnotation(WebService.class);
	    wsHelper.checkAuthorizations(ambiente, versatore, user.getUsername(),
	    ...
	    // imposto la decode map per recuperare il nome del tipo oggetto (si può recuperare da
	    // db anche volendo...)
	    PigTipoObjectTableBean tipoObjectTableBean = comboHelper.getTipoObjectFromVersatore(
		    user.getIdUtente(), idVers, Constants.TipoVersamento.DA_TRASFORMARE.name(),
		    Constants.TipoVersamento.ZIP_CON_XML_SACER.name());
	    form.getVersamentoOggettoDetail().getNm_tipo_object().setDecodeMap(DecodeMap.Factory
		...
		throw new ParerUserError(
			"Il tipo oggetto prevede che sia controllato l'hash del file");
	    }
	    ...
	    } else if (StringUtils.isBlank(xml_to_upload_string)
		...
		cdKeyObjectPadre = form.getVersamentoOggettoDetail().getCd_key_object_padre()
			.getDecodedValue();
		...
		BigDecimal idTipoObjectPadre = form.getVersamentoOggettoDetail()
		...
		if (idTipoObjectPadre != null) {
		    ...
		    if (pigVersTipoObjectDaTrasfRowBean == null || !pigVersTipoObjectDaTrasfRowBean
			    .getIdTipoObjectGen().equals(idTipoObject)) {
			throw new ParerUserError(
		...
		if (StringUtils.isNotBlank(dsHashFileVers)) {
			...
			if (tiHashFileVers.equals(Constants.TipiHash.SCONOSCIUTO.descrivi())) {
			    throw new ParerUserError(
				    "Il formato dell'hash non è tra quelli supportati ("
		...
		if (idObjTrasf == null) {
		    ...
		    long idObjTrasfLong = amministrazioneEjb.createPigObjectTrasf(idObjectPadre,
			    ...
			    Constants.TipiEncBinari.HEX_BINARY.descrivi(), pgOggettoTrasf,
			    cdVersioneXml, xml_to_upload_string);
	    ...
	    String tipoObject = form.getVersamentoOggettoDetail().getNm_tipo_object()
		    .getDecodedValue();

	    InvioOggettoAsincronoRisposta invioRisposta = invioOggettoAsincronoEjb
		    .invioOggettoAsincronoEsteso(user.getUsername(), ambiente, versatore,
			    cdKeyObject, dsObject, tipoObject, false, false, false, null,
			    cdVersioneXml, xml_to_upload_string, ambientePadre, versatorePadre,
			    cdKeyObjectPadre, niTotObjectTrasf, pgOggettoTrasf, null, cdVersGen,
			    ti_gest_oggetti_figli, tiPriorita, tiPrioritaVersamento);
		...
		throw new ParerUserError(
			"Errore " + invioRisposta.getCdErr() + ": " + invioRisposta.getDsErr());
	    }
	...
	switch (Constants.TipiHash
		.evaluateByLenght(BinEncUtility.decodeUTF8HexString(hash).length)) {
	case MD5:
	...
	try {
	    dim = configurationHelper
		    .getValoreParamApplicByApplic(Constants.DIM_MAX_FILE_DA_VERSARE_OS);
	} catch (Exception ex) {
	    throw new ParerUserError(
