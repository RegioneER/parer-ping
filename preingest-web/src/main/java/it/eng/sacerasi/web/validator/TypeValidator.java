...
public class TypeValidator {
    ...
    public Date validateSingleDate(Date data, BigDecimal ore, BigDecimal minuti, String nomeCampo) {
        Date validDate = null;
        if (data != null || ore != null || minuti != null) {
            // Verifico che i campi data, ora e minuti siano validi
            isDateValid(data, ore, minuti, nomeCampo);

            // Controllo che i campi degli orari e minuti siano validi (23 ore 59 minuti)
            isTimeValid(ore, minuti, "Ora");

            // Comincio ad inserire, in caso, i valori di default
            Calendar dataEOrario = Calendar.getInstance();
            dataEOrario.set(Calendar.HOUR_OF_DAY, 0);
            dataEOrario.set(Calendar.MINUTE, 0);
            dataEOrario.set(Calendar.SECOND, 0);
            dataEOrario.set(Calendar.MILLISECOND, 0);

            if (data == null) {
                dataEOrario.set(Calendar.YEAR, 2000);
                dataEOrario.set(Calendar.MONTH, 0);
                dataEOrario.set(Calendar.DAY_OF_MONTH, 1);
            } else {
                dataEOrario.setTime(data);
            }
            ...
            if (ore == null || minuti == null) {
                ...
                dataEOrario.set(Calendar.HOUR_OF_DAY, ore.intValue());
                dataEOrario.set(Calendar.MINUTE, minuti.intValue());
                dataEOrario.set(Calendar.SECOND, 0);
            }

            validDate = dataEOrario.getTime();
        }

        // Ritorno la data validata
        return validDate;
    }
